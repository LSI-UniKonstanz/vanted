...
public class QPRectanglePlacement extends Observable implements RectanglePlacement, Observer {
	 ...
	 * maintaining a set of open blocks sorted by x placement. For each opening edge
	 * of a block x, we determine x's neighbours (l and r) and if necessary, set x,l
	 * and r's neighbour pointers. When we process a closing edge of a block x, we
	 * create 0,1 or 2 constraints between x and it's neighbours, resetting the
	 * neighbours' neighbour pointers. Generates up to 2n constraints, O(n log n)
	 * time.
	 */
	...
	void initVarsAndConstraintsMinimal(Chunk[] chunks) {
		...
		for (ChunkEdge e : edges) {
			...
			if (e.isStart) {
				open.add(v);
				v.setNeighbours(getLeftNeighbours(open, v), getRightNeighbours(open, v));
			} else {
				for (Iterator<Chunk> i = v.leftNeighbours.iterator(); i.hasNext();) {
					Chunk u = i.next();
					constraints.add(new Constraint(u.v, v.v, u.getLength()));
					u.rightNeighbours.remove(v);
				}
				for (Iterator<Chunk> i = v.rightNeighbours.iterator(); i.hasNext();) {
					Chunk u = i.next();
	 ...
	 * the conjugate axis but not in this axis - there is overlap in this axis and
	...
	boolean needConstraint(Chunk l, Chunk r, boolean all) {
		...
		boolean conjOverlapGreaterThanThis = l.overlap(r) > 0 && l.overlap(r) < l.conjugateOverlap(r);
		if (all && conjOverlap)
			return true;
