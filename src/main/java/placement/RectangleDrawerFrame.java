		...
		cleanupButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (f.algorithm == Algorithm.FSA) {
					...
					QPRectanglePlacement r = new QPRectanglePlacement(f.algorithm == Algorithm.ACTIVESET_SPLIT,
							f.completeConstraints, f.orthogonalOrderingConstraints, f.algorithm == Algorithm.MOSEK,
							xGap, yGap, f.animate);
					if (f.animate) {
						r.addObserver(d);
					}
