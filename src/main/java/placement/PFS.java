...
public class PFS implements RectanglePlacement {
	...
	void horizontal() {
		...
		while (i < n) {
			...
			for (int j = k + 1; j < n; j++) {
				...
				r.setRect(r.getMinX() + delta, r.getMinY(), r.getWidth(), r.getHeight());
			}
			i = k + 1;
		}
	...
	void vertical() {
		...
		while (i < n) {
			...
			for (int j = k + 1; j < n; j++) {
				Rectangle2D r = rectangles.get(j);
				r.setRect(r.getMinX(), r.getMinY() + delta, r.getWidth(), r.getHeight());
			}
			i = k + 1;
		}
	...
	public void place(ArrayList<Rectangle2D> rectangles) {
		...
		for (Rectangle2D r : rectangles) {
			r.setRect(r.getMinX(), r.getMinY(), r.getWidth() + xgap, r.getHeight() + ygap);
		}
		long t0 = System.currentTimeMillis();
		double cost = horizontalImproved();
		long t1 = System.currentTimeMillis();
		System.out.println("PFS horizontal: cost=" + cost + " time=" + (t1 - t0));
		cost = verticalImproved();
		long t2 = System.currentTimeMillis();
		System.out.println("PFS vertical: cost=" + cost + " time=" + (t2 - t1));
