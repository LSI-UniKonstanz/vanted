...
class Result {
	...
	public static void writeHeader() {
		...
		try {
			...
			output = new BufferedWriter(new FileWriter(f, true));
			output.write(
					"size,k,as_time,as_disp,as_split_time,as_split_disp,as_oo_time,as_oo_disp,as_split_oo_time,as_split_oo_disp,mosek_time,mosek_disp,mosek_oo_time,mosek_oo_disp,fsa_time,fsa_disp\n");
			output.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
...
public class PerformanceTest {
	...
	PerformanceTest() {
		...
		for (int size = 10; size <= 2000; size += 10) {
			...
			result.size = size;
			ArrayList<Rectangle2D> rs = generateRandom(result, 100, 100, Math.sqrt(size) / 5);
			result.fsa = run(new PFS(0, 0), rs);
			result.as = run(new QPRectanglePlacement(false, false, false, false, 0, 0, false), rs);
			result.mosek = run(new QPRectanglePlacement(false, false, false, true, 0, 0, false), rs);
			result.as_split = run(new QPRectanglePlacement(true, false, false, false, 0, 0, false), rs);
			result.as_oo = run(new QPRectanglePlacement(false, false, true, false, 0, 0, false), rs);
			result.as_split_oo = run(new QPRectanglePlacement(true, false, true, false, 0, 0, false), rs);
			result.mosek_oo = run(new QPRectanglePlacement(false, false, true, true, 0, 0, false), rs);
			results.add(result);
			result.writeToFile();
		}
		System.out.println(
	...
	static ArrayList<Rectangle2D> generateRandom(Result result, double w, double h, double rSize) {
		...
		for (int i = 0; i < result.size; i++) {
			Rectangle2D r = new Rectangle2D.Double(w * rand.nextDouble(), h * rand.nextDouble(),
					(w / rSize) * rand.nextDouble(), (h / rSize) * rand.nextDouble());
			rectangles.add(r);
		}
		int overlapCount = 0;
