...
public class AdaptagramsLibrary {
	...
	/**
	 * Return the names of the native Adaptagrams libraries for the different operating systems. There is a 32 bit and 64 bit version of the library for Windows
	 * and Linux. There is only one library necessary for MacOS.
	 * 
	...
	public static String loadLibrary(String libraryName, String libraryPath) {
		...
		if (!osName.toLowerCase().contains(availableOSs[0]) &&
				!osName.toLowerCase().contains(availableOSs[1]) &&
				!osName.toLowerCase().contains(availableOSs[2]))
			return "Layout library not available for " + osName + "!";
		
		// on Windows and on Linux two versions of the native library exist (one for 32 bit and one for 64 bit)
		if (osName.toLowerCase().contains(availableOSs[0])) {
			ext32 = windowsExt32;
			ext64 = windowsExt64;
		}
		...
		if (!ext32.isEmpty() && !ext64.isEmpty() &&
				!(new File(libraryPath + libraryName + ext32)).exists() &&
			...
			if (!ext.isEmpty() &&
					!(new File(libraryPath + libraryName + ext)).exists())
				return "Could not find " + libraryName + ext + " in<br>" + libraryPath + "!";
		
		// on Windows and on Linux it is a bit hard to figure out if the Java runtime is 32 bit or 64 bit
