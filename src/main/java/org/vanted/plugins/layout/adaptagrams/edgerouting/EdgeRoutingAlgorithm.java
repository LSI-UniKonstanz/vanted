...
public class EdgeRoutingAlgorithm extends AbstractAlgorithm {
	...
	"Building orthogonal visibility graph in x-dimension ...",
			"Building orthogonal visibility graph in y-dimension ...",
			"Searching initial paths for edges ...",
			"Detecting crossing edges ...",
			"Rerouting crossing edges ...",
			"Nudging apart edge segments in x-dimension ...",
			"Nudging apart edge segments in y-dimension ...",
			"Finishing ..."
	};
	...
	private boolean segmentPenalty = true; // if set to true the number of segments is minimised during orthogonal routing
	private boolean nudgeOrthogonalSegments = false; // if set to true the first and the last segments of edges are nudged apart
	private boolean ignoreNodesWithoutEdges = false; // if set to true nodes without edges are ignored as obstacle for the edges
	
	/**
	 * Check preconditions:
	 * (1) a graph with edges,
	 * (2) native layout library can be loaded.
	 */
	@Override
	...
	public Parameter[] getParameters() {
		...
		return new Parameter[] {
				new ObjectListParameter(this.routingStr, "<html>Edge Routing", "<html>Polyline or orthogonal edge routing", getRoutings()),
				new DoubleParameter(this.shapeBufferDistance, "<html>Node Distance", "<html>Distance between nodes and edges"),
				new DoubleParameter(this.radius, "<html>Bend Radius", "<html>Radius for edge smoothing around edge bends"),
	 ...
	 * 
	 * @return collection of <code>String</code> containing the different routing types
	 */
	...
	public void execute() {
		...
		switch (this.routingStr) {
			...
			case polylineStr: // polyline routing
				...
				backgroundTaskStatusProvider.setCurrentStatusText2(getStatusTextPhaseDescription(polylineRoutingFirstPhase));
				break;
			case orthogonalStr: // orthogonal routing
				...
				backgroundTaskStatusProvider.setCurrentStatusText2(getStatusTextPhaseDescription(orthogonalRoutingFirstPhase));
				break;
			default:
				return;
		}
		...
		// define Adaptagrams shapes for all visible nodes and depending on settings ignore nodes without edges
		...
		// create a finish swing task to get the bend points from the Adaptagrams router for each edge
		Runnable finishSwingTask = createFinishSwingTask(edges, hmConnRefs, this.radius, view, router);
		BackgroundTaskHelper.issueSimpleTask("Adaptagrams Edge Routing", "", backgroundTask, finishSwingTask, backgroundTaskStatusProvider);
		...
		Runnable backgroundTask = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					// router.outputInstanceToSVG("libavoid-debug-" + simpleDateFormat.format(date));
	...
	/**
	 * Create finish swing task to get the bend points from the Adaptagrams router for each edge.
	 * 
	 * @param edges
	 *           the edges
	 * @param hmConnRefs
	 *           <code>HashMap</code> containing the edges and the Adaptagrams connectors
	 * @param radius
	 ...
	 * @return runnable
	 */
	private static Runnable createFinishSwingTask(final Collection<Edge> edges, final HashMap<Edge, ConnRef> hmConnRefs, final double radius,
			final View view, final Router router) {
		...
		Runnable finishSwingTask = new Runnable() {
			...
			public void run() {
				...
				// for each edge get the according Adaptagrams connector and return the bend points from the routing
				...
				for (Edge edge : edges) {
					...
					if (connRef != null) {
						...
						// check for triples of bend points where x1 == x2 == x3 or y1 == y2 == y3 and remove bend point (x2, y2)
						...
						for (int k = 1; k < edgeBends.size() - 1; k++) {
							...
							if ((Math.abs(bend_pre_k.x - bend_k.x) < 0.0001 && Math.abs(bend_post_k.x - bend_k.x) < 0.0001) ||
									(Math.abs(bend_pre_k.y - bend_k.y) < 0.0001 && Math.abs(bend_post_k.y - bend_k.y) < 0.0001))
						...
						if (edgeBends.size() > 0) {
							...
							EdgeGraphicAttribute edgeGraphicAttribute = (EdgeGraphicAttribute) edge.getAttribute(GraphicAttributeConstants.GRAPHICS);
							...
							if (srcDocking != null && srcDocking.indexOf(";") > 0) {
								// edges (or edge segments) which run horizontally or vertically might not be perfectly horizontal or vertical
								// due to the docking position (it's often difficult to set the exact docking position for a perfectly horizontal or
								// vertical edge (segment))
								// orthogonal edge routing might therefore introduce bend points in close proximity to the docking position
								...
								AdaptagramsRouting.addBendPointForNodePosition(edgeBends, 0, 0, AttributeHelper.getPositionVec2d(srcNode));
								// do we have more bend points than the start point and the end point of the edge?
							...
							if (tgtDocking != null && tgtDocking.indexOf(";") > 0) {
								...
								for (int k = edgeBends.size() - 2; k >= 0; k--) {
									double deltaX = Math.abs(edgeBends.get(edgeBends.size() - 1).x - edgeBends.get(k).x);
									double deltaY = Math.abs(edgeBends.get(edgeBends.size() - 1).y - edgeBends.get(k).y);
									if ((deltaX < 0.0001 && deltaY < 1.0001) || (deltaX < 1.0001 && deltaY < 0.0001))
										edgeBends.remove(k);
									else
										break;
								...
								AdaptagramsRouting.addBendPointForNodePosition(edgeBends, edgeBends.size() - 1, edgeBends.size(),
										AttributeHelper.getPositionVec2d(tgtNode));
								// after fixing the source bend points do we have more bend points than the end point of the edge?
