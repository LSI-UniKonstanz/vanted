...
public class AdaptagramsRouting {
	...
	public static ConnRef defineConnRef(Router router, View view, Edge edge, int routingType, AvoidCheckpoints avoidCheckpoints) {
		...
		ConnRef connRef = defineConnRef(router, view, edge, ConnDirFlag.ConnDirAll, ConnDirFlag.ConnDirAll, routingType);
	...
	public static void addBendPointForNodePosition(ArrayList<Vector2d> edgeBends, int compareIndex, int addIndex, Vector2d nodePos) {
		...
		// check whether 'edgeBends' already contains a bend point with same coordinates as node position
		// otherwise add bend point for the node position
		if (Double.compare(edgeBends.get(compareIndex).x, nodePos.x) != 0 || Double.compare(edgeBends.get(compareIndex).y, nodePos.y) != 0)
	...
	public static void fixSourceBendPoints(ArrayList<Vector2d> edgeBends, View view, Node node) {
		...
		if (nodeShape != null)
			for (int k = 0; k <= edgeBends.size() - 2; k++) {
				Point2D intersectionPoint = nodeShape.getIntersection(
						new Line2D.Double(edgeBends.get(k).x, edgeBends.get(k).y, edgeBends.get(k + 1).x, edgeBends.get(k + 1).y));
	...
	public static void fixTargetBendPoints(ArrayList<Vector2d> edgeBends, View view, Node node) {
		...
		if (nodeShape != null)
			for (int k = 0; k <= edgeBends.size() - 2; k++) {
				...
				Point2D intersectionPoint = nodeShape.getIntersection(
						new Line2D.Double(edgeBends.get(l).x, edgeBends.get(l).y, edgeBends.get(l + 1).x, edgeBends.get(l + 1).y));
				if (intersectionPoint != null) {
	...
	private static Vector2d calculateBendPoint(Vector2d pos, Point2D intersectionPoint) {
		...
		if (Math.abs(deltaX) > Math.abs(deltaY))
			...
			deltaX = 0;
		return new Vector2d(intersectionPoint.getX() + Math.signum(deltaX) * 0.5, intersectionPoint.getY() + Math.signum(deltaY) * 0.5);
