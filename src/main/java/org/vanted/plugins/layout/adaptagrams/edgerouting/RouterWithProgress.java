...
public class RouterWithProgress extends Router {
	 ...
	 * Sets the status text and the progress bar of the BackgroundTaskStatusProviderSupportingExternalCall components.
	 * Sets logger information.
	 * 
	 * @param elapsedTime
	...
	public boolean shouldContinueTransactionWithProgress(long elapsedTime, long phaseNumber, long totalPhases, double proportion) {
		...
		if (this.backgroundTaskStatusProvider != null) {
			this.backgroundTaskStatusProvider.setCurrentStatusText1(EdgeRoutingAlgorithm.getStatusTextPhaseNumber((int) phaseNumber));
			this.backgroundTaskStatusProvider.setCurrentStatusText2(EdgeRoutingAlgorithm.getStatusTextPhaseDescription((int) phaseNumber));
			if (proportion < 0.01)
		...
		int hours = (int) Math.ceil(elapsedTime / 3600000);
		int minutes = (int) Math.ceil(elapsedTime / 60000) - hours * 60;
		int seconds = (int) Math.ceil(elapsedTime / 1000) - minutes * 60 - hours * 3600;
		long milliseconds = elapsedTime - seconds * 1000 - minutes * 60000 - hours * 3600000;
		String strElapsedTime = String.format("%02d:%02d:%02d.%03d", new Integer(hours), new Integer(minutes), new Integer(seconds), new Long(milliseconds));
		logger.info("Adaptagrams Edge Routing: running for " + strElapsedTime + ", phase " + phaseNumber + " of " + totalPhases + " ... " +
				String.format(Locale.ENGLISH, "%1$,.2f", new Double(proportion * 100.0)) + "%");
		return true;
