...
public class LayoutAlgorithmWrapper {
	 ...
	 *
	 * @param guiName
	 *           The name the algorithm has in the GUI. This can be different from
	 ...
	 * @param algorithm
	 *           The {@link Algorithm} to wrap. Must not be {@code null} and must be a layout algorithm
	 *           (i.e. {@link Algorithm#isLayoutAlgorithm()} must return {@code true}.
	 * @param useThreadSafeGUI
	 *           Some algorithms such as {@link de.ipk_gatersleben.ag_nw.graffiti.plugins.layouters.pattern_springembedder.PatternSpringembedder}
	...
	LayoutAlgorithmWrapper(String guiName, Algorithm algorithm, boolean useThreadSafeGUI) {
		...
		this.guiName = guiName == null ? Objects.toString(algorithm.getName(), algorithm.getClass().getSimpleName()) : guiName;
	...
	public static Map<String, LayoutAlgorithmWrapper> getLayoutAlgorithms() {
		try {
			return Stream.concat(
					...
					getPluginLayoutAlgs().entrySet().stream()).collect(
							Collectors.toMap(
									Map.Entry::getKey,
									Map.Entry::getValue));
	 ...
	 *
	 * @param graph
	 *           The {@link Graph}. Must not be {@code null}.
	 * @param selection
	 ...
	 *
	 * @param alg
	 *           The {@link ThreadSafeAlgorithm}. Must not be {@code null}.
	 * @param options
	 *           The {@link ThreadSafeOptions} object where the options will be stored. Must not be {@code null}.
	 ...
	 * @see de.ipk_gatersleben.ag_nw.graffiti.plugins.gui.plugin_settings.PreferencesDialog#initAlgorithmPreferencesPanel(JDialog, Algorithm, Graph, Selection,
	 *      HandlesAlgorithmData, boolean)
	 */
