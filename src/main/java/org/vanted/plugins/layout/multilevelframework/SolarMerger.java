...
public class SolarMerger implements Merger {
		...
		for (Parameter parameter : parameters) {
			switch (parameter.getName()) {
				case MAX_LEVEL_FACTOR_NAME:
					final int maxLevelFactor = ((IntegerParameter) parameter).getInteger();
					validateNumber(maxLevelFactor, 0, Integer.MAX_VALUE, MAX_LEVEL_FACTOR_NAME);
					this.maxLevelFactor = maxLevelFactor;
					break;
				case MIN_NODES_NAME:
					final int minNodes = ((IntegerParameter) parameter).getInteger();
					validateNumber(minNodes, 0, Integer.MAX_VALUE, MIN_NODES_NAME);
	 ...
	 *
	 * @param multilevelGraph
	 *           the MultilevelGraph which buildCoarseningLevels works on
	 ...
	 *
	 * @param baseLevel
	 *           the level current of the multilevel framework
