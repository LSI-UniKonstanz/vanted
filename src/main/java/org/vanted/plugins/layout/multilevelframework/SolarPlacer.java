...
public class SolarPlacer implements Placer {
	...
	public void reduceCoarseningLevel(MultilevelGraph multilevelGraph) {
		...
		@SuppressWarnings("unchecked")
		Set<Node> allPlanets = (Set<Node>) getElement(top, SolarMerger.PLANETS_KEY);
		if (allPlanets == null) {
			allPlanets = Collections.emptySet();
		}
		@SuppressWarnings("unchecked")
		Map<Node, Set<Node>> sunToPlanets = (Map<Node, Set<Node>>) getElement(top,
				SolarMerger.SUN_TO_PLANETS_KEY);
		if (sunToPlanets == null) {
			sunToPlanets = Collections.emptyMap();
		}
		@SuppressWarnings("unchecked")
		Map<Node, Set<Node>> planetToMoons = (Map<Node, Set<Node>>) getElement(top,
				SolarMerger.PLANET_TO_MOONS_KEY);
		if (planetToMoons == null) {
			planetToMoons = Collections.emptyMap();
		}
	 ...
	 *
	 * @param levelGraph
	 *           The {@link LevelGraph} to get the attribute from.
