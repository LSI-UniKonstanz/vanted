...
public class MultilevelFrameworkLayout extends ThreadSafeAlgorithm {
	...
	/**
	 * Setting this attribute on a graph will indicate that it is currently part of
	 * an MLF run. This is to prevent invoking the algorithm twice on the same
	 * graph.
	 */
	final static String WORKING_ATTRIBUTE_PATH = "MLF_EXECUTING";
	
	// Default names of the attributes that indicate to the algorithms that the
	// graphs they work on
	// coarsened graph instead of the original graph. The algorithm can use this for
	// optimizations.
	...
	private final static String COARSENING_BOTTOM_LEVEL_INDICATOR_ATTRIBUTE_PATH = "GRAPH_IS_MLF_COARSENING_BOTTOM_LEVEL";
	
	// add the default mergers and placers
	static {
		MultilevelFrameworkLayout.mergers.add(new SolarMerger());
		MultilevelFrameworkLayout.mergers.add(new RandomMerger());
		MultilevelFrameworkLayout.placers.add(new RandomPlacer());
		MultilevelFrameworkLayout.placers.add(new SolarPlacer());
	}
	
	/**
	 * Set this to true to make execute() block until it is finished and use the
	 * specified mergers instead of the ones selected through the GUI.
	 */
	...
	/**
	 * Contains the Swing UI components for the configuration (a.k.a "parameters").
	 * At the same time, these are used as a "model" (in the MVC sense).
	 */
	 ...
	 */
	static String makeStatusMessage(int totalLevels, int currentLevel, int totalComponents, int currentComponent,
			String graphName) {
		return "Laying out level " + currentLevel + " (out of " + totalLevels + ")" + " of connected component "
				+ currentComponent + " (out of " + totalComponents + ") of graph \"" + graphName + "\"";
	}
	
	/**
	 * Create a percentage for the GUI. Calculates the progress for the current
	 * connected component.
	 */
	static double calculateProgress(MultilevelGraph mlg, int numComponents, int currentIndex,
			int numberOfLevelsAtStart) {
		if (numberOfLevelsAtStart == 0 || numComponents == 0) {
			return -1;
		}
		// calculate the progress as the percentage of nodes and connected components
		// already processed
		return 100.0 * (1 - (double) mlg.getNumberOfLevels() / numberOfLevelsAtStart) * (currentIndex + 1.0)
				/ numComponents;
	}
	
	/**
	 * Does the same thing as {@link AbstractAlgorithm#getSelectedOrAllNodes()}
	 * which cannot be used here because that uses instance variables which is not
	 * safe it the user runs the algorithm multiples times at the same time on
	 * different graphs with different settings.
	 *
	 * @param graph
	 *           The {@link Graph} to use. Must not be {@code null}.
	 * @param selection
	 *           The {@link Selection} to use. May be {@code null}.
	 * @return All nodes if the selection is empty or {@code null}, or the selected
	 *         nodes otherwise.
	 ...
	 *            invocation or the number of nodes is zero or
	 *            negative
	 */
	@Override
	...
	/**
	 * Performs the layout. Can be executed on multiple graphs at the same time (by
	 * calling attach, execute, attach, execute...), but only if the
	 * {@link Algorithm} used for the levels supports thread safe execution (i.e.
	 * multiple execute methods running at the same time using the same instance).
	 * This is only possible if the algorithm's execute method copies the parameter
	 * values or the {@link org.graffiti.plugin.algorithm.ThreadSafeOptions} object
	 * in case of a {@link org.graffiti.plugin.algorithm.ThreadSafeAlgorithm}.
	 * Unless you can guarantee that the algorithm you're using does this, it is not
	 * recommended to run this on multiple graphs at the same time.
	 * 
	 * @author Gordian
	 */
	public void execute() {
		// need to save old session and view, because they need to be restored for
		...
		// check if the graph is currently being worked on by another MLF
		// instance/thread
		// this is necessary to prevent the user from double clicking the layout button
		// to start the layout twice
		...
		// remember the base level (of each induced component) to later access its nodes
		// and, in
		...
		final Runnable mainTask = () -> {
			...
			List<IndexedComponent> components = IndexedGraphOperations
					.getComponents(IndexedNodeSet.setOfAllIn(getSelectedOrAllNodes(graph, selection)));
			...
			for (IndexedComponent component : components) {
				...
				MultilevelGraph componentMLG = new MultilevelGraph(componentBaseLevel);
				
				merger.buildCoarseningLevels(componentMLG);
				
				// keep track of how many nodes coarsening levels there were at the start (for
				// the progress bar)
				final int numLevelsAtStart = componentMLG.getNumberOfLevels();
				// indicate that this is a coarsened graph to allow for optimizations in the
				// level layouter
		...
		final Runnable finishSwingTask = () -> {
			...
			// center graph layout
			GravistoService.getInstance().runAlgorithm(new CenterLayouterAlgorithm(), graph, selection, null);
			
			// remove space between components / remove overlapping
			// do not run as regular algorithm, since that triggers a gui dialogue
			// this means however, that we are also changing the positions of non selected
			// nodes
		...
		BackgroundTaskHelper.issueSimpleTask(this.getName(), "Multilevel Framework is running", attributeSafeTask,
				finishSwingTask, bts);
	}
	...
	/**
	 * Add a new {@link Merger} that will be available for the user to choose. This
	 * method is meant to be called by other VANTED add-ons that seek to extend the
	 * MLF with new {@link Merger}s.
	 * 
	 * @param merger
	...
	public static void addMerger(Merger merger) {
		...
		if (!MultilevelFrameworkLayout.mergers.contains(merger)) {
			MultilevelFrameworkLayout.mergers.add(merger);
		}
	}
	
	/**
	 * @return the currently registered {@link Merger}s as an unmodifiable
	 *         {@link Collection}.
	 * @author Gordian
	 */
	public static Collection<Merger> getMergers() {
		return Collections.unmodifiableList(MultilevelFrameworkLayout.mergers);
	}
	
	/**
	 * Add a new {@link Placer} that will be available for the user to choose. This
	 * method is meant to be called by other VANTED add-ons that seek to extend the
	 * MLF with new {@link Placer}s.
	 * 
	 * @param placer
	...
	public static void addPlacer(Placer placer) {
		...
		if (!MultilevelFrameworkLayout.placers.contains(placer)) {
			MultilevelFrameworkLayout.placers.add(placer);
		}
	}
	
	/**
	 * @return the currently registered {@link Placer}s as an unmodifiable
	 *         {@link Collection}.
	 * @author Gordian
	 */
	public static Collection<Placer> getPlacers() {
		return Collections.unmodifiableList(MultilevelFrameworkLayout.placers);
	}
	
	/**
	 * Calls {@link GraphHelper#diplayGraph(Graph)} in the event dispatcher thread
	 * (it throws exceptions if called from other threads).
	 * 
	 * @param graph
	...
	private void reportStatus(MLFBackgroundTaskStatus bts, int numLevelsAtStart, MultilevelGraph componentMLG,
			List<IndexedComponent> components, int currentComponentIndex) {
		bts.statusMessage = makeStatusMessage(numLevelsAtStart, componentMLG.getNumberOfLevels(), components.size(),
				currentComponentIndex, graph.getName());
		bts.status = calculateProgress(componentMLG, components.size(), currentComponentIndex, numLevelsAtStart);
	}
	
	/**
	 * @return the {@link Merger} selected by the user and set it up using the
	 *         parameters specified by the user through the GUI.
	 * @author Gordian
	 */
	...
	/**
	 * @return the {@link Placer} selected by the user and set it up using the
	 *         parameters specified by the user through the GUI.
	 * @author Gordian
	 */
...
class MLFBackgroundTaskStatus implements BackgroundTaskStatusProvider {
	...
	public int getCurrentStatusValue() {
		return (int) Math.ceil(this.getCurrentStatusValueFine());
	}
