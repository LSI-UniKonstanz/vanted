...
public class StressMinimizationLayout extends BackgroundAlgorithm {
	...
	public void execute() {
		...
		for (IndexedComponent component : components) {
			...
			int nodeMovementThreshold = (params.terminationCheckboxGroup.nodeThresholdActive()) ? params.terminationCheckboxGroup.getNodeMovementThreshold() : 0;
			int iterThreshold = (params.terminationCheckboxGroup.iterThresholdActive()) ? params.terminationCheckboxGroup.getMaxIterations() : Integer.MAX_VALUE;
	...
	void setIterationProgress(double newStress, double initialStress, int iterationCount) {
		...
		setStatusDescription(
				"Stress Minimization: optimizing layout - iteration: " + iterationCount + "; current stress reduction: " + stressProgress * 100 + "%");
	...
	void setFinalProgress(double finalStress, double initialStress, int iterationCount) {
		...
		setProgress(1.0);
		setStatusDescription(
				"Stress Minimization: optimization finished - iterations: " + iterationCount + "; total stress reduction: " + stressProgress * 100 + "%");
