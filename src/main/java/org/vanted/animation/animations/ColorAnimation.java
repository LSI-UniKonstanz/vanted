...
public abstract class ColorAnimation extends ContinuousAnimation<ColorTimePoint> {
	...
	/**
	 * @param colorMode
	 * Specifies which colour-interpolation mode the interpolator should use<br>
	 * RGB: Maintains brightness but not saturation during interpolation<br>
	 * HSB: Maintains saturation but not perceived-brightness during interpolation<br>
	...
	public ColorAnimation(Attributable attributable,List<ColorTimePoint> dataPoints, double loopDuration, double startTime, int noLoops,
			Looper looper, Interpolator interpolator,ColorMode colorMode) {
