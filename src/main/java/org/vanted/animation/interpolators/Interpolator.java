...
public abstract class Interpolator {
	...
	/**
	 * In the event that you need to perform linear interpolation, you can call this
	 * method to save you from implementing the method yourself
	 */
	...
	/**
	 * Calculates how far the {@code time} is from the next data point relative to
	 * the previous pooint as a percentage. <br>
	 * Eg. If there are 4 points with times: 3,6,7,9 and {@code time} = 6.5 then
	 ...
	 */
	protected <V, T extends InterpolatableTimePoint<V>> double getNormalizedTime(double time, double loopDuration,
			List<T> dataPoints, List<T> pointsUsed, Looper looper) {
		return looper.getNormalizedTime(time, loopDuration, dataPoints, pointsUsed, pointsUsed.get(getPointsBefore()),
				pointsUsed.get(getPointsBefore() + 1));
	}

	/**
	 * Colors can interpolate in special ways.<br>
	 * It is recommended that you call this method instead of interpolate() to
	 ...
	 *            between.
	 * @param looper
	 *            Used to figure out which points are needed at a particular time
	 *            for interpolation.
	 * @param transitionMode
	 *            Specifies what kind of the color interpolation the interpolator is
	 *            using.
	 * @return The interpolated color.
	 */
	public Color interpolateColor(double time, double loopDuration, int previousIndex, List<ColorTimePoint> dataPoints,
			Looper looper, ColorMode transitionMode) {
		List<ColorTimePoint> pointsUsed = looper.getPointsUsed(dataPoints, previousIndex, getPointsBefore(),
				getPointsAfter());
	 ...
	 * 
	 * @param t
	 *            A value between 0 and 1. Typically represents how close you are to
	 *            a given point with respect to time.<br>
	 *            t=0 Means the elapsed time value is equal to the time value of the
	 *            previous data point. <br>
	 *            t=1 Means the elapsed time value is equal the time value of the
	 *            next data point. <br>
	 * @param y
	...
	/**
	 * This method calls {@link #interpolate(double, double...)} for each array in
	 ...
	 *            next data point. <br>
	 * @param pointsUsed
	 *            The data points being used for interpolation at this instance in
	 *            time.
	 * @return An interpolated value.
	 */
	 ...
	 *            The type of data point that the interpolator is interpolating for.
	 * @param y
	 *            A list of data points that the interpolator needs to interpolate
	 *            between
	 * @return A 2D array:<br>
	 *         Each array accessed in the first dimension represents an individual
	 *         set of data values to interpolate between.<br>
