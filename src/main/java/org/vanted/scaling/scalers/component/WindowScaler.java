...
public class WindowScaler extends ComponentScaler {
	 ...
	 * up to a given percentage out of the full scaling factor. Default system
	 * windows (through usage of {@link WindowScaler#attachSystemWindowResizer()})
	...
	private static float getScalebackFactor(float ratio) {
		if (ratio == previousRatio) {
			if (ratio > 1)
				...
				return 1;
		} else { // currently unused, because we scale back only impersistent windows
					// (dialog-like)
			if (ratio > previousRatio) {
				previousRatio = ratio;
				return SCALEBACK_DECREASE;
