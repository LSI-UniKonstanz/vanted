...
public class ScalingCoordinator {
	 ...
	 * 
	 * @param factor
	 *           the required new DPI Factor, from which a scaling ratio would
	 *           be constructed.
	 */
	 ...
	 * <b>Important Note:</b> To scale any added defaults, these must be added to
	 * the {@link UIManager#getLookAndFeelDefaults()}.
	 * 
	 * @param delegate
	 */
	public void adjustDefaults(Scaler delegate) {
		/*
		 * We use getLookAndFeelDefaults() instead of just getDefaults(), because the
		 * developer defaults contain doubled references and thus result in doubled
		 * scaling.
		 */
	...
	public static void refreshGUI(final Container main) {
		if (main != null)
			/**
			 * repaint() is thread-safe only on OpenJDK, therefore we cannot rely on that
			 * and call EDT explicitly.
			 */
