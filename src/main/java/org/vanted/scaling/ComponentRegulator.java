...
public class ComponentRegulator {	
	...
	/**
	 * Scan and scale the components specifics. This are taken from the 
	 * specified parent {@link Container} <code>container</code>. <p>
	 * 
	 * <b>Important:</b> Register any newly implemented  component scalers
	 *  before calling this method!
	 *  (see: {@linkplain ComponentRegulator#registerNewScaler()}}
	 * 
	 * @param container, whose components are to be modified
	 * 
	 * @throws OutOfMemoryError when the allocated heap memory is not enough
	 * to hold all largely scaled components, such as Icons. 
	 */
	...
	/**
	 * Registers the necessary scalers to avoid instance creation explosion
	 * (one pro Component) and spare memory. Due to the supporting structure
	 * the inserting order matters.
	 */
	 ...
	 * its appropriate scaler. It is responsibility of the matched scaler to have
	 * implemented such method. <p>
	 * 
	 * <b>Note:</b> No need to explicitly call this method, since it is part of 
	 * the {@linkplain ComponentRegulator#init(Container)} workflow. Useful when
		...
		for (Entry<Class<?>, ComponentScaler> entry : scalers.entrySet()) {
			if (matches(component, entry.getKey())) {
				...
				try {
					...
					Method htmlModifier = 
							scaler.getClass().getDeclaredMethod(
									"coscaleHTML", JComponent.class);
	...
	/**
	 * Checks if interface contract between interface and supposed implementation
	 * is respected.
	 * 
	...
	/**
	 * Registers new ComponentScaler to use. Does not overwrite previous 
	 * mappings and just returns. Please, use before 
	 * {@linkplain ComponentRegulator#init(Container)}.<p>
	 * 
	 ...
	 * 
	 * @param superclass the super class, or possibly even direct
	 ...
	 * his scaling specifics - Font, Icon, Insets; scaled.  
	 * 
