...
public class UNCFileLocationCheck {
	...
	/**
	 * Checks, if the given file is to be stored at a UNC network path. If not, it
	 * returns JOptionPane.OK_OPTION If yes, it opens a dialog and let the user
	 * decide, if the UNC location is ok.
	 * 
	 * @param file
	 *           The File to check
	...
	/**
	 * Checks, if the given path is a UNC network path. If not, it returns
	 * JOptionPane.OK_OPTION If yes, it opens a dialog and let the user decide, if
	 * the UNC location is ok.
	 * 
	 * @param path
	 *           The path to check
	 * @return UNCFileLocationCheck.CONFIRM if UNC location is accepted
	...
	public static int showUNCPathConfirmDialogForPath(String path) {
		if (UNCFileLocationCheck.isUNCFilePath(path))
			return JOptionPane.showConfirmDialog(MainFrame.getInstance(), message, "Storing location Question",
