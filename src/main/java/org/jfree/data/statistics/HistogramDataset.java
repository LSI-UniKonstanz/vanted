 ...
 * $Id: HistogramDataset.java,v 1.2 2010/12/14 07:02:02 morla Exp $
 * Changes
 * -------
 ...
 * <p>
 * See the <code>HistogramDemo.java</code> file in the JFreeChart distribution
 * for an example.
 * 
...
public class HistogramDataset extends AbstractIntervalXYDataset implements IntervalXYDataset, Cloneable, Serializable {
	...
	/**
	 * Adds a series to the dataset. Any data value falling on a bin boundary will
	 * be assigned to the lower value bin, with the exception of the lower bound of
	 * the bin range which is always assigned to the first bin.
	 * 
	 * @param name
	 *            the series name (<code>null</code> not permitted).
	 * @param values
	 ...
	 */
	public int[] addSeries(final String name, final double[] values, final int bins, final double minimum,
	...
	/**
	 * Returns the y-value for a bin (calculated to take into account the histogram
	 * type).
	 * 
	 * @param series
	 *            the series index (zero based).
	...
	/**
	 * Returns the start y-value for a bin (which is the same as the y-value, this
