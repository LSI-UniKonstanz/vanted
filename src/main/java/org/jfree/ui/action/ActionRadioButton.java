 ...
 * $Id: ActionRadioButton.java,v 1.2 2010/12/14 07:02:06 morla Exp $
 * ChangeLog
 * ---------
		...
		public void propertyChange(final PropertyChangeEvent event) {
			try {
				if (event.getPropertyName().equals("enabled")) {
					...
					if (event.getPropertyName().equals(Action.SMALL_ICON)) {
						...
						if (event.getPropertyName().equals(Action.NAME)) {
							...
							if (event.getPropertyName().equals(Action.SHORT_DESCRIPTION)) {
				...
				if (event.getPropertyName().equals(ActionDowngrade.ACCELERATOR_KEY)) {
					...
					if (event.getPropertyName().equals(ActionDowngrade.MNEMONIC_KEY)) {
