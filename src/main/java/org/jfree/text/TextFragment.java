 ...
 * Contributor(s): -;
 * $Id: TextFragment.java,v 1.2 2010/12/14 07:02:06 morla Exp $
 * Changes
 * -------
...
public class TextFragment implements Serializable {
	...
	public Size2D calculateDimensions(Graphics2D g2) {
		...
		double width = fm.stringWidth(this.text);
		double height = fm.getHeight();
		if (logger.isDebugEnabled()) {
			logger.debug("width = " + width + ", height = " + height);
		}
		Size2D result = new Size2D(width, height);
		// END WORKAROUND
		return result;
	}
	...
	public float calculateBaselineOffset(Graphics2D g2, TextAnchor anchor) {
		...
		if (anchor == TextAnchor.TOP_LEFT || anchor == TextAnchor.TOP_CENTER
			...
			if (anchor == TextAnchor.BOTTOM_LEFT || anchor == TextAnchor.BOTTOM_CENTER
