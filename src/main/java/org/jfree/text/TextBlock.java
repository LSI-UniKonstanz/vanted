 ...
 * Contributor(s): -;
 * $Id: TextBlock.java,v 1.2 2010/12/14 07:02:06 morla Exp $
 * Changes
 * -------
...
public class TextBlock implements Serializable {
	...
	protected static final LogContext logger = Log.createContext(TextBlock.class);
	...
	public Shape calculateBounds(final Graphics2D g2,
											final float anchorX, final float anchorY, final TextBlockAnchor anchor,
		...
		final Shape rotatedBounds = ShapeUtils.rotateShape(bounds, angle, rotateX, rotateY);
	...
	/**
	 * Draws the text block, aligning it with the specified anchor point and rotating it about
	 * the specified rotation point.
	 * 
	 * @param g2
	 *           the graphics device.
	 * @param anchorX
	 ...
	 *           the y-coordinate for the anchor point.
	 * @param anchor
	 *           the point on the text block that is aligned to the anchor point.
	 * @param rotateX
	 *           the x-coordinate for the rotation point.
	 * @param rotateY
	 *           the x-coordinate for the rotation point.
	...
	public void draw(final Graphics2D g2,
		...
		while (iterator.hasNext()) {
			final TextLine line = (TextLine) iterator.next();
			line.draw(
								g2, anchorX + offsets[0], anchorY + offsets[1] + yCursor,
	...
	/**
	 * Calculates the x and y offsets required to align the text block with the specified
	 * anchor point. This assumes that the top left of the text block is at (0.0, 0.0).
	 * 
	 * @param anchor
	 *           the anchor position.
	 * @param width
	...
	private float[] calculateOffsets(final TextBlockAnchor anchor, final double width, final double height) {
		...
		if (anchor == TextBlockAnchor.TOP_CENTER
			...
			if (anchor == TextBlockAnchor.TOP_RIGHT
		...
		if (anchor == TextBlockAnchor.TOP_LEFT
			...
			if (anchor == TextBlockAnchor.CENTER_LEFT
								|| anchor == TextBlockAnchor.CENTER
								|| anchor == TextBlockAnchor.CENTER_RIGHT) {
				...
				if (anchor == TextBlockAnchor.BOTTOM_LEFT
									|| anchor == TextBlockAnchor.BOTTOM_CENTER
									|| anchor == TextBlockAnchor.BOTTOM_RIGHT) {
