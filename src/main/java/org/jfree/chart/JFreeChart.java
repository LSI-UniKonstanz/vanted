 ...
 * Klaus Rheinwald;
 * Nicolas Brodu;
 * $Id: JFreeChart.java,v 1.4 2010/12/14 09:27:16 morla Exp $
 ...
 * 03-Dec-2003 : Legends are now registered by this class instead of using the old constructor
 * way. (TM);
 ...
 * 27-May-2004 : Moved constants from JFreeChartConstants.java back to this class (DG);
 */
 ...
 * {@link Legend}, a {@link Plot} and a {@link org.jfree.data.Dataset} (the plot
...
public class JFreeChart
		implements Drawable, TitleChangeListener, LegendChangeListener, PlotChangeListener, Serializable, Cloneable {
	...
	private static final LogContext LOGGER = Log.createContext(JFreeChart.class);
	 ...
	 *            the rendering hints (<code>null</code> not permitted).
	 */
	...
	/**
	 * Sets a flag that controls whether or not a border is drawn around the outside
	 * of the chart.
	 * 
	 * @param visible
	 *            the flag.
	...
	/**
	 * Sets the title for the chart. If you do not want a title for the chart, set
	 ...
	 * 
	 * @param title
	 *            the new title (<code>null</code> permitted).
	 */
	...
	/**
	 * Adds a chart subtitle, and notifies registered listeners that the chart has
	 * been modified.
	 * 
	 * @param subtitle
	 *            the subtitle.
	...
	/**
	 * Sets the chart legend. Registered listeners are notified that the chart has
	 * been modified. The legends chart reference is updated.
	 * 
	 * @param legend
	 ...
	 * coordinating the visual representation of the data, including the axes (if
	 * any).
	 * 
	 * @return the plot.
	 */
	...
	/**
	 * Returns a flag that indicates whether or not anti-aliasing is used when the
	 * chart is drawn.
	 * 
	 * @return the flag.
	 */
	...
	public void setAntiAlias(boolean flag) {
		if (renderingHints == null) {
			Object setting = flag ? RenderingHints.VALUE_ANTIALIAS_ON : RenderingHints.VALUE_ANTIALIAS_OFF;
			renderingHints = new RenderingHints(RenderingHints.KEY_ANTIALIASING, setting);
		}
		Object o = this.renderingHints.get(RenderingHints.KEY_ANTIALIASING);
		if (o == null) {
	...
	/**
	 * Returns the background image for the chart, or <code>null</code> if there is
	 * no image.
	 * 
	...
	/**
	 * Sets the background image for the chart and sends a {@link ChartChangeEvent}
	 ...
	 * the <code>com.jrefinery.ui.Align</code> class in the JCommon class library.
	 * 
	...
	/**
	 * Draws a title. The title should be drawn at the top, bottom, left or right of
	 * the specified area, and the area should be updated to reflect the amount of
	 * space used by the title.
	 * 
	 * @param title
	 ...
	 * @param area
	 *            the chart area, excluding any existing titles (<code>null</code>
	 *            not permitted).
	 */
	public void drawTitle(Title title, Graphics2D g2, Rectangle2D area) {
		...
		if (position == RectangleEdge.TOP) {
			...
			area.setRect(area.getX(), Math.min(area.getY() + availableHeight, area.getMaxY()), availableWidth,
					Math.max(area.getHeight() - availableHeight, 0));
	 ...
	 *            the height.
	 * @param info
	 *            carries back chart state information (<code>null</code>
	 *            permitted).
	 * @return a buffered image.
	 */
	 ...
	 *            the image height.
	 * @param drawWidth
	 *            the width for drawing the chart (will be scaled to fit image).
	 * @param drawHeight
	 ...
	 * needs to capture the click event and pass it onto the JFreeChart object. If
	 * you are not using JFreeChart in a client application, then this method is not
	 * required (and hopefully it doesn't get in the way).
	 * 
	 * @param x
	...
	/**
	 * Receives notification that a chart title has changed, and passes this on to
	 * registered listeners.
	 * 
	 * @param event
	...
	/**
	 * Receives notification that the chart legend has changed, and passes this on
	...
	/**
	 * Clones the object, and takes care of listeners. Note: caller shall register
	 * its own listeners on cloned graph.
	 * 
	 * @return A clone.
...
class JFreeChartInfo extends ProjectInfo {
	...
	public JFreeChartInfo() {
		...
		setContributors(Arrays.asList(new Contributor[] { new Contributor("Eric Alexander", "-"),
				...
				new Contributor("Nicolas Brodu", "-"), new Contributor("David Browning", "-"),
				new Contributor("S�ren Caspersen", "-"), new Contributor("Chuanhao Chiu", "-"),
				new Contributor("Pascal Collet", "-"), new Contributor("Martin Cordova", "-"),
				new Contributor("Paolo Cova", "-"), new Contributor("Mike Duffy", "-"),
				new Contributor("Jonathan Gabbai", "-"),
				new Contributor("David Gilbert", "david.gilbert@object-refinery.com"),
				new Contributor("Serge V. Grachov", "-"), new Contributor("Joao Guilherme Del Valle", "-"),
				new Contributor("Hans-Jurgen Greiner", "-"), new Contributor("Aiman Han", "-"),
				new Contributor("Jon Iles", "-"), new Contributor("Wolfgang Irler", "-"),
				new Contributor("Xun Kang", "-"), new Contributor("Bill Kelemen", "-"),
				new Contributor("Norbert Kiesel", "-"), new Contributor("Gideon Krause", "-"),
				new Contributor("Arnaud Lelievre", "-"), new Contributor("Wolfgang Lenhard", "-"),
				new Contributor("David Li", "-"), new Contributor("Tin Luu", "-"),
				...
				new Contributor("Tomer Peretz", "-"), new Contributor("Andrzej Porebski", "-"),
				new Contributor("Xavier Poinsard", "-"), new Contributor("Viktor Rajewski", "-"),
				new Contributor("Eduardo Ramalho", "-"), new Contributor("Michael Rauch", "-"),
				new Contributor("Cameron Riley", "-"), new Contributor("Dan Rivett", "d.rivett@ukonline.co.uk"),
				new Contributor("Scott Sams", "-"), new Contributor("Michel Santos", "-"),
				new Contributor("Thierry Saura", "-"), new Contributor("Andreas Schneider", "-"),
				new Contributor("Jean-Luc SCHWAB", "-"), new Contributor("Bryan Scott", "-"),
				new Contributor("Greg Steckman", "-"), new Contributor("Roger Studner", "-"),
				new Contributor("Irv Thomae", "-"), new Contributor("Eric Thomas", "-"),
				new Contributor("Rich Unger", "-"), new Contributor("Daniel van Enckevort", "-"),
				new Contributor("Laurence Vanhelsuwe", "-"), new Contributor("Sylvain Vieujot", "-"),
				new Contributor("Mark Watson", "www.markwatson.com"), new Contributor("Alex Weber", "-"),
				new Contributor("Matthew Wright", "-"), new Contributor("Benoit Xhenseval", "-"),
				new Contributor("Christian W. Zuckschwerdt", "Christian.Zuckschwerdt@Informatik.Uni-Oldenburg.de"),
