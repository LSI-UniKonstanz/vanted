 ...
 * Richard Atkinson;
 * $Id: XYItemRenderer.java,v 1.2 2010/12/14 07:02:04 morla Exp $
 * Changes
 * -------
...
public interface XYItemRenderer {
	...
	/**
	 * Sets the base item label generator and sends a {@link RendererChangeEvent} to
	 * all registered listeners.
	 * 
	 * @param generator
	 *           the generator (<code>null</code> permitted).
	...
	/**
	 * Sets the base tool tip generator and sends a {@link RendererChangeEvent} to
	 * all registered listeners.
	 * 
	 * @param generator
	 *           the generator (<code>null</code> permitted).
	 ...
	 * <p>
	 * The {@link XYPlot} can make multiple passes through the dataset, depending on
	 * the value returned by the renderer's initialise() method.
	 * 
	 * @param g2
	 *           the graphics device.
	 * @param state
	...
	/**
	 * Fills a band between two values on the axis. This can be used to color bands
	...
	/**
	 * Fills a band between two values on the range axis. This can be used to color
	...
	/**
	 * Returns the range type for the renderer. The plot needs to know this
	 * information in order to determine an appropriate axis range (when the axis
	 * auto-range calculation is on).
	 * <P>
	 * Two types are recognised:
	 * <ul>
	 * <li><code>STANDARD</code> - data items are plotted individually, so the axis
	 * range should extend from the smallest value to the largest value;</li>
	 * <li><code>STACKED</code> - data items are stacked on top of one another, so
	 * to determine the axis range, all the items in a series need to be summed
	 * together.</li>
	 * </ul>
