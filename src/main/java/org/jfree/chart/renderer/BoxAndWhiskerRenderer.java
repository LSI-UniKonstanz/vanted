		...
		if (yOutliers != null) {
			for (int i = 0; i < yOutliers.size(); i++) {
				...
				if (outlier > bawDataset.getMaxOutlier(row, column).doubleValue()) {
					...
					if (outlier < bawDataset.getMinOutlier(row, column).doubleValue()) {
						outlierListCollection.setLowFarOut(true);
					} else
						if (outlier > bawDataset.getMaxRegularValue(row, column).doubleValue()) {
							yyOutlier = rangeAxis.valueToJava2D(outlier, dataArea, location);
							outliers.add(new Outlier(xx + state.getBarWidth() / 2.0, yyOutlier, oRadius));
						} else
							if (outlier < bawDataset.getMinRegularValue(row, column).doubleValue()) {
								yyOutlier = rangeAxis.valueToJava2D(outlier, dataArea, location);
								outliers.add(new Outlier(xx + state.getBarWidth() / 2.0, yyOutlier, oRadius));
							}
