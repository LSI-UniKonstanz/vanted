 ...
 * $Id: PolarChartPanel.java,v 1.2 2010/12/14 07:02:07 morla Exp $
 * Changes
 * -------
...
/**
 * <code>PolarChartPanel</code> is the top level object for using the {@link PolarPlot}.
 * Since this class has a <code>JPanel</code> in the inheritance hierarchy, one uses this class
 * to integrate the Polar plot into their application.
 * <p>
 * The main modification to <code>ChartPanel</code> is the popup menu. It removes <code>ChartPanel</code>'s versions of:
 * <ul>
...
public class PolarChartPanel extends ChartPanel {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (command.equals(POLAR_ZOOM_IN_ACTION_COMMAND)) {
			...
			if (command.equals(POLAR_ZOOM_OUT_ACTION_COMMAND)) {
				PolarPlot plot = (PolarPlot) getChart().getPlot();
				plot.zoom(2.0);
			} else
				if (command.equals(POLAR_AUTO_RANGE_ACTION_COMMAND)) {
					PolarPlot plot = (PolarPlot) getChart().getPlot();
	 ...
	 *           the title.
	 * @param data
	 *           the dataset for the chart where each series is a collection
