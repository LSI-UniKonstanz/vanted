 ...
 * $Id: ValueAxis.java,v 1.3 2010/12/14 07:02:00 morla Exp $
...
public abstract class ValueAxis extends Axis implements Cloneable, PublicCloneable, Serializable {
	...
	protected AxisState drawTickMarksAndLabels(Graphics2D g2,
		...
		List<ValueTick> ticks = refreshTicks(g2, state, plotArea, dataArea, edge);
	...
	protected double findMaximumTickLabelWidth(List ticks,
		...
		if (!vertical) {
			...
			while (iterator.hasNext()) {
				Tick tick = (Tick) iterator.next();
				String s = tick.getText();
				// System.out.print(s+", ");
				Rectangle2D labelBounds = TextUtilities.getTextBounds(s, g2, fm);
			...
			LineMetrics metrics = font.getLineMetrics("0ABCxyz", g2.getFontRenderContext());
			maxWidth = metrics.getHeight() + insets.top + insets.bottom;
		}
		// System.out.println(maxWidthText+" : "+maxWidth);
		return maxWidth;
