 ...
 * Contributor(s): -;
 * $Id: CategoryTick.java,v 1.2 2010/12/14 07:02:00 morla Exp $
 * Changes
 * -------
...
public class CategoryTick extends Tick {
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof CategoryTick && super.equals(obj)) {
			CategoryTick ct = (CategoryTick) obj;
			if (!ObjectUtils.equal(this.category, ct.category)) {
				return false;
			}
			if (!ObjectUtils.equal(this.label, ct.label)) {
				return false;
			}
			if (!ObjectUtils.equal(this.labelAnchor, ct.labelAnchor)) {
				return false;
			}
			return true;
		}
