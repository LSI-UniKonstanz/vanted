 ...
 * Contributor(s): -;
 * $Id: SerialDate.java,v 1.2 2010/12/14 07:02:02 morla Exp $
 ...
 * <P>
 * Why not just use java.util.Date? We will, when it makes sense. At times, java.util.Date can be *too* precise - it represents an instant in time, accurate to
 * 1/1000th of a second (with the date itself depending on the time-zone). Sometimes we just want to represent a particular day (e.g. 21 January 2015) without
 * concerning ourselves about the time of day, or the time-zone, or anything else. That's what we've defined SerialDate for.
 * <P>
...
public abstract class SerialDate implements Comparable,
	 ...
	 * <P>
	 * This method will return one of the constants JANUARY, FEBRUARY, ..., DECEMBER that corresponds to the string. If the string is not recognised, this method
	 * returns -1.
	 * 
	 * @param s
	 *           the string to parse.
	...
	public static SerialDate addYears(final int years, final SerialDate base) {
		...
		final int targetD = Math.min(baseD, SerialDate.lastDayOfMonth(baseM, baseY));
	 ...
	 * 
	 * @param description
	 *           the new description for the date.
	 */
