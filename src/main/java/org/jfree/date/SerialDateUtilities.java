 ...
 * Contributor(s): -;
 * $Id: SerialDateUtilities.java,v 1.2 2010/12/14 07:02:02 morla Exp $
 ...
 * <p>
 * [1] some of the methods assume that the default Calendar is a GregorianCalendar (used mostly to determine leap years) - so the code won&rsquo;t work if some
 * other Calendar is the default. I'm not sure how to handle this properly?
 * <p>
 * [2] a whole bunch of static methods isn't very object-oriented - but I couldn't think of a good way to extend the Date and Calendar classes to add the
 * functions I required, so static methods are doing the job for now.
 */
public class SerialDateUtilities {
	...
	public int stringToWeekday(final String s) {
		...
		if (s.equals(this.weekdays[Calendar.SATURDAY])) {
			...
			if (s.equals(this.weekdays[Calendar.SUNDAY])) {
				return SerialDate.SUNDAY;
			} else
				if (s.equals(this.weekdays[Calendar.MONDAY])) {
					return SerialDate.MONDAY;
				} else
					if (s.equals(this.weekdays[Calendar.TUESDAY])) {
						return SerialDate.TUESDAY;
					} else
						if (s.equals(this.weekdays[Calendar.WEDNESDAY])) {
							return SerialDate.WEDNESDAY;
						} else
							if (s.equals(this.weekdays[Calendar.THURSDAY])) {
