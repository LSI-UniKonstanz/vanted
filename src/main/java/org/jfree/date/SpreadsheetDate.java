 ...
 * Contributor(s): -;
 * $Id: SpreadsheetDate.java,v 1.2 2010/12/14 07:02:02 morla Exp $
 * Changes
 * -------
 ...
 * <P>
 * Be aware that there is a deliberate bug in Excel that recognises the year 1900 as a leap year when in fact it is not a leap year. You can find more
 * information on the Microsoft website in article Q181370:
 * <P>
 * http://support.microsoft.com/support/kb/articles/Q181/3/70.asp
 * <P>
 * Excel uses the convention that 1-Jan-1900 = 1. This class uses the convention 1-Jan-1900 = 2. The result is that the day number in this class will be
 * different to the Excel figure for January and February 1900...but then Excel adds in an extra day (29-Feb-1900 which does not actually exist!) and from that
 * point forward the day numbers will match.
 */
...
public class SpreadsheetDate extends SerialDate implements Serializable {
	...
	public SpreadsheetDate(final int day, final int month, final int year) {
		...
		if ((day >= 1) && (day <= SerialDate.lastDayOfMonth(month, year))) {
			...
			throw new IllegalArgumentException("The 'day' argument is invalid.");
		}
	...
	/**
	 * Returns the description that is attached to the date. It is not required that a date
	 * have a description, but for some applications it is useful.
	 * 
	 ...
	 * <P>
	 * The codes are defined in the {@link SerialDate} class as: <code>SUNDAY</code>, <code>MONDAY</code>, <code>TUESDAY</code>, <code>WEDNESDAY</code>,
	 ...
	 * <P>
	 * This method will return true ONLY if the object is an instance of the {@link SerialDate} base class, and it represents the same day as this
	 ...
	 * @param include
	 *           a code that controls whether or not the start and end dates are
	...
	public boolean isInRange(final SerialDate d1, final SerialDate d2, final int include) {
		...
		if (include == SerialDate.INCLUDE_BOTH) {
			...
			if (include == SerialDate.INCLUDE_FIRST) {
				return (s >= start && s < end);
			} else
				if (include == SerialDate.INCLUDE_SECOND) {
					return (s > start && s <= end);
				} else {
					return (s > start && s < end);
				}
	 ...
	 */
	private int calcSerial(final int day, final int month, final int year) {

		final int y = ((year - 1900) * 365) + SerialDate.leapYearCount(year - 1);
