 ...
 * <ul>
 * <li>The LU decomposition is stored and reused on subsequent calls. If matrix data are modified using any of the public setXxx methods, the saved
 * decomposition is discarded. If data are modified via references to the underlying array obtained using <code>getDataRef()</code>, then the stored LU
 * decomposition will not be discarded. In this case, you need to explicitly invoke <code>LUDecompose()</code> to recompute the decomposition before using any
 * of the methods above.</li>
 * <li>As specified in the {@link BigMatrix} interface, matrix element indexing is 0-based -- e.g., <code>getEntry(0, 0)</code> returns the element in the first
 * row, first column of the matrix.</li>
 * </ul>
 * 
 * @version $Revision: 1.2 $ $Date: 2010/12/14 07:02:05 $
 */
public class BigMatrixImpl implements BigMatrix, Serializable {
	...
	/**
	 * Create a new BigMatrix using the values represented by the strings in <code>data</code> as the underlying data array.
	 * 
	 * @param d
	 *           data for new matrix
	 * @throws IllegalArgumentException
	 ...
	 * Returns matrix entries as a two-dimensional array.
	 * <p>
	 * Makes a fresh copy of the underlying data converted to <code>double</code> values.
	 * 
	 * @return 2-dimensional array of entries
	 */
	...
	/**
	 * Returns the entries in column number <code>column</code> as a column matrix. Column indices start at 0.
	 * 
	 * @param column
	 ...
	 * <p>
	 * Row indices start at 0. A <code>MatrixIndexException</code> is thrown unless <code>0 <= row < rowDimension.</code>
	 * 
	 * @param row
	 *           the row to be fetched
	 ...
	 * <p>
	 * Column indices start at 0. A <code>MatrixIndexException</code> is thrown unless <code>0 <= column < columnDimension.</code>
	 * 
	 * @param col
	 *           the column to be fetched
	 ...
	 * coefficient matrix = this and constant vectors = columns of <code>b</code>.
	 * 
	 * @param b
	 ...
	 * This method should rarely be invoked directly. Its only use is to force recomputation of the LU decomposition when changes have been made to the
	 * underlying data using direct array references. Changes made using setXxx methods will trigger recomputation when needed automatically.
	 * 
	 * @throws InvalidMatrixException
	...
	/**
	 * Returns true iff <code>object</code> is a <code>BigMatrixImpl</code> instance with the same dimensions as this
	 ...
	 * <p>
	 * The matrix returned is a compact representation of the LU decomposition. Elements below the main diagonal correspond to entries of the "L" matrix;
	 * elements on and above the main diagonal correspond to entries of the "U" matrix.
	 * <p>
	 * Example:
	 * 
	 * <pre>
	 ...
	 * where permuteRows reorders the rows of the matrix to follow the order determined by the <a href=#getPermutation()>permutation</a> property.
	 * 
