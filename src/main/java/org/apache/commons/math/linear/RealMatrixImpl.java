 ...
 * <ul>
 * <li>The LU decomposition is cached and reused on subsequent calls. If data are modified via references to the underlying array obtained using
 * <code>getDataRef()</code>, then the stored LU decomposition will not be discarded. In this case, you need to explicitly invoke <code>LUDecompose()</code> to
 * recompute the decomposition before using any of the methods above.</li>
 * <li>As specified in the {@link RealMatrix} interface, matrix element indexing is 0-based -- e.g., <code>getEntry(0, 0)</code> returns the element in the
 * first row, first column of the matrix.</li>
 * </ul>
 * 
 * @version $Revision: 1.3 $ $Date: 2010/12/14 07:02:05 $
 */
public class RealMatrixImpl implements RealMatrix, Serializable {
	...
	/**
	 * Returns true iff <code>object</code> is a <code>RealMatrixImpl</code> instance with the same dimensions as this
	...
	public boolean equals(Object object) {
		...
		for (int row = 0; row < nRows; row++) {
			for (int col = 0; col < nCols; col++) {
				if (Double.doubleToLongBits(data[row][col]) != Double.doubleToLongBits(m.getEntry(row, col))) {
					return false;
				}
			}
		}
