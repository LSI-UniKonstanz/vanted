 ...
 * 
 * @version $Revision: 1.2 $ $Date: 2010/12/14 07:02:04 $
 */
public interface IntegerDistribution extends DiscreteDistribution {
	 ...
	 * <ul>
	 * <li>If there is a minimum value, <code>m</code>, with postive probablility under (the density of) X, then <code>m - 1</code> is returned by
	 * <code>inverseCumulativeProbability(0).</code> If there is no such value <code>m,  Integer.MIN_VALUE</code> is returned.</li>
	 * <li>If there is a maximum value, <code>M</code>, such that P(X &le; M) =1, then <code>M</code> is returned by
	 * <code>inverseCumulativeProbability(1).</code> If there is no such value, <code>M, Integer.MAX_VALUE</code> is returned.</li>
	 * </ul>
	 * 
	 * @param p
