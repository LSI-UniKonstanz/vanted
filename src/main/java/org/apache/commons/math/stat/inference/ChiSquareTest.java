...
public interface ChiSquareTest {
	 ...
	 * Chi-Square statistic</a> comparing <code>observed</code> and <code>expected</code> freqeuncy counts.
	 * <p>
	 ...
	 * Chi-square goodness of fit test</a> comparing the <code>observed</code> frequency counts to those in the <code>expected</code> array.
	 * <p>
	 * The number returned is the smallest significance level at which one can reject the null hypothesis that the observed counts conform to the frequency
	 ...
	 * <strong>Preconditions</strong>:
	 * <ul>
	 * <li>All counts must be >= 0.</li>
	 * <li>The count array must be rectangular (i.e. all count[i] subarrays must have the same length).</li>
	 * <li>The 2-way table represented by <code>counts</code> must have at least 2 columns and at least 2 rows.</li> </li>
	 * </ul>
	 * <p>
	 * If any of the preconditions are not met, an <code>IllegalArgumentException</code> is thrown.
	 * 
	 ...
	 * <p>
	 * <strong>Example:</strong><br>
	 * To test the null hypothesis that the counts in <code>count[0], ... , count[count.length - 1] </code> all correspond to the same underlying probability
	 * distribution at the 99% level, use
	 * <p>
