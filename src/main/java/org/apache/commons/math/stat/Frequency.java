 ...
 * <p>
 * Accepts int, long, char or Object values. New values added must be comparable to those that have been added, otherwise the add method will throw an
 * IllegalArgumentException.
 * <p>
 * Integer values (int, long, Integer, Long) are not distinguished by type -- i.e. <code>addValue(new Long(2)), addValue(2), addValue(2l)</code> all have the
 * same effect (similarly for arguments to <code>getCount,</code> etc.).
 * <p>
 * The values are ordered using the default (natural order), unless a <code>Comparator</code> is supplied in the constructor.
 * 
...
public class Frequency implements Serializable {
	 ...
	 * <p>
	 * If added values are itegral (i.e., integers, longs, Integers, or Longs), they are converted to Longs when they are added, so the objects returned by the
	 ...
	 * (as a proportion between 0 and 1).
	 * <p>
	 * Returns <code>Double.NaN</code> if no values have been added. Returns 0 if at least one value has been added, but v is not comparable to the values set.
	 * 
	 * @param v
	 *           the value to lookup
