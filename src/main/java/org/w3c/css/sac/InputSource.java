 ...
 * $Id: InputSource.java,v 1.2 2010/12/14 07:02:03 morla Exp $
 */
 ...
 * <p>
 * This class allows a CSS application to encapsulate information about an input source in a single object, which may include a URI, a byte stream (possibly
 * with a specified encoding), and/or a character stream.
 * </p>
 * <p>
 * The CSS parser will use the InputSource object to determine how to read CSS input. If there is a character stream available, the parser will read that stream
 * directly; if not, the parser will use a byte stream, if available; if neither a character stream nor a byte stream is available, the parser will attempt to
 * open a URI connection to the resource identified by the URI.
 * </p>
 * <p>
...
public class InputSource {
	 ...
	 * <p>
	 * The URI is optional if there is a byte stream or a character stream, but it is still useful to provide one, since the application can use it to resolve
	 * relative URIs and can include it in error messages and warnings (the parser will attempt to open a connection to the URI only if there is no byte stream
	 ...
	 * <p>
	 * The SAX parser will ignore this if there is also a character stream specified, but it will use a byte stream in preference to opening a URI connection
	 * itself.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param byteStream
	 *           A byte stream containing an CSS document or
	 *           other entity.
	 * @see #setEncoding
	 * @see #getByteStream
	 * @see #getEncoding
	 ...
	 * 
	 * @param title
	 *           The advisory title. See the title attribute definition
	...
	/**
	 * Returns the media associated to the input source or <code>null</code> if media are currently unknown.
	 * 
