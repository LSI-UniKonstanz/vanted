				...
				if (!FileSaveAsAction.this.isTextFieldFileNameSearchDone
					...
					if (FileSaveAsAction.this.fileTypeDescription != null) {
						for (FileFilter filterFilter : fileChooser.getChoosableFileFilters())
							if (((GenericFileFilter) filterFilter).getDescription()
									.startsWith(FileSaveAsAction.this.fileTypeDescription)
					...
					FileSaveAsAction.this.jTextFieldFileName = getTextFieldFileName(fileChooser.getComponents(),
							fileName);
					FileSaveAsAction.this.isTextFieldFileNameSearchDone = true;
					// FileSaveAsAction.this.jTextFieldFileName = null;
				}
				if (FileSaveAsAction.this.jTextFieldFileName != null && (propertyChangeEvent.getPropertyName()
			...
			try {
				...
				if (os == null)
					MainFrame.getInstance()
							.showMessageDialog("Output serializer unknown for file extension '" + ext + "'.");
				else {
					MainFrame.showMessage("Save network to file " + file.getAbsolutePath() + "...",
							MessageType.PERMANENT_INFO);
					// revision 12/11/2015
					// graph name and file type description should be set before the writer is
					// called
	...
	JTextField getTextFieldFileName(Component[] components, String fileName) {
		...
		for (Component component : components)
			if (component instanceof JPanel) {
				...
				if (jTextField != null)
					break;
			} else if (component instanceof JTextField && fileName.equals(((JTextField) component).getText())) {
