...
public class ParameterEditPanel extends JPanel {
	...
	public ParameterEditPanel(Parameter[] parameters, Map<?, ?> editTypes, Selection selection, String title,
			boolean fillSurroundingStyle, String heading) {
		this(parameters, editTypes, selection, title, fillSurroundingStyle, heading, null);
	}
	
	public ParameterEditPanel(Parameter[] parameters, Map<?, ?> editTypes, Selection selection, String title,
			boolean fillSurroundingStyle, String heading, JComponent descComponent) {
		super();
		...
		double[][] size = new double[][] { { TableLayoutConstants.FILL }, { TableLayoutConstants.FILL } };
		...
		if (paramCnt > 0) {
			jc = new JScrollPane(myPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		if (descComponent != null) {
			add(TableLayout.getSplit(descComponent, jc, TableLayoutConstants.PREFERRED, TableLayoutConstants.PREFERRED),
					"0,0");
		} else
			add(jc, "0,0");
	...
	private void addRow(FolderPanel myPanel, Parameter parameter, Class<?> ecClass) {
		...
		try {
			editComp = (ValueEditComponent) InstanceLoader.createInstance(ecClass, "org.graffiti.plugin.Displayable",
					parameter);
			if (multiLine)
				editComp.setParameter("multiline", true);
			ToolTipHelper.addToolTip(editComp.getComponent(), parameter.getDescription());
		} catch (InstanceCreationException ice) {
			ErrorMsg.addErrorMessage(ice);
			throw new RuntimeException("Could not create an instance of a ValueEditComponent class. " + ice);
		}
		...
		if (parameter != null && ((parameter instanceof AbstractSingleParameter
				&& ((AbstractSingleParameter) parameter).isLeftAligned()))) {
			if (parameter instanceof BooleanParameter) {
				myPanel.addGuiComponentRow(editCompComp, null, false);
				editCompComp.setToolTipText(parameter.getDescription());
			} else
				myPanel.addGuiComponentRow(TableLayout.get3Split(editCompComp, null, descLabel, TableLayout.PREFERRED,
						5, TableLayout.PREFERRED), null, false);
		} else
	...
	private void addValueEditComponents(FolderPanel myPanel, Selection selection) {
		if (parameters != null)
			for (int i = 0; i < parameters.length; i++) {
				...
				/*
				 * check whether there exists a ValueEditComponent, if not use standard edit
				 * component
				 */
				Class<?> ecClass = null;
