...
//
// ==============================================================================
// $Id: DefaultParameterDialog.java,v 1.25 2011/06/30 06:55:30 morla Exp $
...
public class DefaultParameterDialog extends AbstractParameterDialog implements ActionListener, WindowListener {
	 ...
	 * @return DOCUMENT ME!
	 */
	private JPanel createValueEditContainer(Parameter[] parameters, Selection selection, String title, String heading,
			JComponent descComponent) {
	...
	private void defineLayout(boolean okOnly, boolean noButton, boolean allowMultipleGraphTargets) {
		...
		if (!noButton)
			getContentPane().add(TableLayout.get3Split(sessionselpan, TableLayout.getSplitVertical(null,
					TableLayout.get3Split(ok, null, okOnly ? null : cancel, TableLayoutConstants.PREFERRED, border,
							TableLayoutConstants.PREFERRED),
					TableLayout.FILL, TableLayoutConstants.PREFERRED), null, TableLayout.FILL,
	...
	public static Object[] getInput(Object description, String title, Object... parameters) {
		...
		for (int i = 0; i < p.length; i++) {
			...
			if (param instanceof JComponent) {
				...
				Set<?> val = (Set<?>) param;
				String name = (String) desc;
				ObjectListParameter ip = new ObjectListParameter(val.size() > 0 ? val.iterator().next() : null, name,
						name, val);
				p[i] = ip;
		...
		if (description != null && description instanceof String && ((String) description).startsWith("["))
			...
			} else if (((String) description).indexOf("nonmodal") > 0) {
				modal = false;
				description = StringManipulationTools.stringReplace((String) description, "nonmodal", "");
			}
		...
		if (description != null && description instanceof String) {
			if (((String) description).indexOf("<") >= 0 && ((String) description).indexOf(">") >= 0
