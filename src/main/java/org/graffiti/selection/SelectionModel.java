...
public class SelectionModel {
	...
	/**
	 * Sets the active selection to the given value. Informs all listeners about the
	 * change.
	 * 
	 * @param selectionName
	...
	public void setActiveSelection(Selection sel) {
		/*
		 * this method fires two selection changed events 1. event to clear previously
		 * selected elements 2. event to select the new elements Needs revamping of the
		 * code.. not optimal
		 */
		if (this.activeSelection != null) {
			Selection oldSel = this.selections.get(this.activeSelection);
			oldSel.clear();
			/*
			 * fire selection change for current active selection, to unselect all elements
			 * in this old selection old selection has been emptied.
			 */
			this.selectionChanged();
		}
