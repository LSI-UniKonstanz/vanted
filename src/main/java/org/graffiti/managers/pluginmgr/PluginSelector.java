...
public class PluginSelector extends JDialog
	...
	/**
	 * Constructs a new plugin selector from the given plugin description collector.
	 * 
	 * @param parent
	 *            the parent dialog.
	...
	public PluginSelector(PluginManagerDialog parent, PluginDescriptionCollector pluginDescriptionCollector) {
		super(parent, true);
	 ...
	 * @see PluginEntry
	 */
	private List<?> createPluginDescriptionList(PluginDescriptionCollector collector) {
	...
	static class PluginDescriptionCellRenderer extends DefaultListCellRenderer {
		...
		public static String displayToolTip(PluginEntry e) {
			...
			return "<html>Version:     " + d.getVersion() + "<br>Description: " + d.getDescription()
					+ "<br>Available:   " + d.getAvailable() + "<br>Main class:  " + d.getMain() + "<br>Author:      "
					+ d.getAuthor() + "<br><br>" + "Loaded from: " + e.getFileName() + "</html>";
		}
	}
	...
	class EntryComparator implements Comparator<Object> {
		/**
		 * Compares to entries via <code>getDescription().getName()</code>. A String
		 * starting with an underscore ("_") precedes all other strings.
		 * 
