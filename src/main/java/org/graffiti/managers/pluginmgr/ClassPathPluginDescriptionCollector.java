...
public class ClassPathPluginDescriptionCollector implements PluginDescriptionCollector {
	 ...
	 * 
	 * @return An enumeration of all plugin description files from the given class
	 *         path.
	 */
	 ...
	 * @param acc
	 *            the accumulator, which will contain the result of the recursive
	 *            search.
	 */
		...
		if (root.isFile()) {
				...
				try {
					...
					while (entries.hasMoreElements()) {
						...
						if (isPluginDescription(jarEntry.getName())) {
							acc.add("jar:file:" + classRoot.getAbsolutePath() + "!/" + jarEntry.getName());
						}
					}
				} catch (IOException e) {
			...
			if (contents != null) {
				for (int i = 0; i < contents.length; i++) {
					gatherFiles(classRoot, fileName + File.separatorChar + contents[i], acc);
				}
			}
		}
	}
