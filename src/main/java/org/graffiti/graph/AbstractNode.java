...
//
// ==============================================================================
// $Id: AbstractNode.java,v 1.12 2010/12/22 13:05:33 klukas Exp $
...
public abstract class AbstractNode extends AbstractGraphElement implements Node {
	 ...
	 * the number returned by <code>getInDegree()</code>, due to the fact that
	 * multiple edges between the same two nodes are possible but the corresponding
	 ...
	 * target of either an ingoing, outgoing or an undirected <code>Edge</code> of
	 * this <code>Node</code>.
	 * 
	 ...
	 * number of Elements returned by this function might be less than the number
	 * returned by <code>getOutDegree()</code>, due to the fact that multiple edges
	 * between the same two nodes are possible but the corresponding
	 * <code>Node</code> is only inserted once.
	 * 
	 * @return a collection containing all the neighbor nodes of the current
