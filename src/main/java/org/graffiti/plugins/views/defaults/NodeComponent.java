...
public class NodeComponent extends AbstractGraphElementComponent implements NodeComponentInterface {
	...
	public synchronized void graphicAttributeChanged(Attribute attr) throws ShapeNotFoundException {
		if (attr != null) {
			logger.debug("graphicAttributeChanged for node id:" + getGraphElement().getID() + " attribute "
					+ attr.getName());
			 ...
			 * components, which can easily result in multiple recreation of dep. components
			 * (like edges). This slows transaction very much down. This new approach goes
			 * that the View will collect all depenend components once and update them in
			 * one go.
			 */
	...
	protected void drawShape(Graphics g) {
		...
		// logger.debug("graph id " + getGraphElement().getGraph().getName() + ", node
		// id:" + getGraphElement().getID() + ", border:" + getBorder().toString());
		// super.drawShape(g);
	...
	protected void recreate() throws ShapeNotFoundException {
		...
		if (nodeAttr.getUseGradient() > epsilon)
			rgp = new RoundGradientPaint(nodeAttr.getDimension().getWidth() * nodeAttr.getUseGradient(),
					nodeAttr.getDimension().getHeight() * nodeAttr.getUseGradient(), nodeAttr.getFillcolor().getColor(),
					new Point2D.Double(0, maxR), nodeAttr.getFramecolor().getColor());
		else if (nodeAttr.getUseGradient() < -1) {
			Point2D p1 = new Point2D.Double(0,
					nodeAttr.getDimension().getHeight() * (1 + 1 + nodeAttr.getUseGradient()));
			...
			Point2D p2 = new Point2D.Double(0,
					nodeAttr.getDimension().getHeight() * (1 + epsilon + nodeAttr.getUseGradient()));
			Color c1 = nodeAttr.getFillcolor().getColor();
			Color c2 = nodeAttr.getFramecolor().getColor();
			rgp = new GradientPaint(p1, c1, p2, c2);
		} else
	...
	private void updateStroke() {
		...
		if (frameThickness > 0) {
			...
			stroke = new BasicStroke(lastStrokeWidth, DEFAULT_CAP_R, DEFAULT_JOIN, DEFAULT_MITER,
		...
		synchronized (dependentComponents) {
			for (Iterator<?> it = dependentComponents.iterator(); it.hasNext();) {
				...
				try {
					...
					ErrorMsg.addErrorMessage("Only EdgeComponents should be registered as dependent components. "
