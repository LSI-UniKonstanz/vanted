...
//
// ==============================================================================
// $Id: PolygonalNodeShape.java,v 1.12 2010/12/22 13:06:19 klukas Exp $
...
public class PolygonalNodeShape extends RectangularNodeShape {
	...
	public Point2D getIntersection(Line2D line) {
		Polygon workPolygon = new Polygon(polygon.xpoints, polygon.ypoints, polygon.npoints);
		...
		for (int pt = 0; pt < (nr - 1); pt++) {
			if (ignorePoints != null && ignorePoints.contains(pt))
				continue;
			polyLine = new Line2D.Double(workPolygon.xpoints[pt], workPolygon.ypoints[pt], workPolygon.xpoints[pt + 1],
		...
		// for closed polygon:
		polyLine = new Line2D.Double(workPolygon.xpoints[nr - 1], workPolygon.ypoints[nr - 1], workPolygon.xpoints[0],
		...
		// if (minIntPoint!=null)
		// minIntPoint = new
		// Point2D.Double(minIntPoint.getX()+bounds.getX()-Math.floor(bounds.getX()),
