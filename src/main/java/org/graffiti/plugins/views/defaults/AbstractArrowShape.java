...
public abstract class AbstractArrowShape implements ArrowShape {
	 ...
	 * larger than the edge but is only enlarged the moment the edge gets broader
	 * than the arrow. In this case, <code>SCALE_FACTOR</code> is used.
	 */
	protected final boolean DOSCALE = true;
	
	/**
	 * If line is larger than arrow, then arrow is scaled to be <code>factor</code>
	 * times larger than the line.
	 */
	...
	public static Rectangle2D addThickness(Rectangle2D b, double lineWidth) {
		...
		return new Rectangle2D.Double(b.getX() - thick, b.getY() - thick, b.getWidth() + thick * 2,
				b.getHeight() + thick * 2);
	}
	 ...
	 * given by the connection between points target and other. Ensures that the
	 * arrow is always larger than the line.
	 * 
	 * @param target
	 *           the point where the arrow should be put.
	 * @param other
	 *           needed to calculate the direction in which the arrow should point
	 *           to. The line is given by the two points target and other.
	 * @param lineWidth
	 *           The total width of the line. May be used to scale the arrow so as
	 *           to be larger than the line.
