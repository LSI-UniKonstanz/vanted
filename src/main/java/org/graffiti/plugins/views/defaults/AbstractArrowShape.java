...
//
// ==============================================================================
// $Id: AbstractArrowShape.java,v 1.10 2010/12/22 13:06:19 klukas Exp $
 ...
 * @version $Revision: 1.10 $ $Date: 2010/12/22 13:06:19 $
 */
public abstract class AbstractArrowShape
	 ...
	 * (before rotation) point horizontally from left to right. The size of
	 * the arrow is therefore its height.
	 */
	 ...
	 * arrow). If <code>false</code>, it is (only) guaranteed, that the arrow
	 * is larger than the edge but is only enlarged the moment the edge gets
	 * broader than the arrow. In this case, <code>SCALE_FACTOR</code> is
	 * used.
	 */
	protected final boolean DOSCALE = true;
	
	/**
	 * If line is larger than arrow, then arrow is scaled to be <code>factor</code> times larger than the line.
	 */
	protected float SCALE_FACTOR = 3f;
	...
	public void updateSize(double sz) {
		SIZE = (float) sz;
		anchor = new Point2D.Double(0, SIZE / 2d);
		head = new Point2D.Double(SIZE, SIZE / 2d);
	}
	...
	public Rectangle2D getBounds2D() {
		return arrowShape.getBounds2D();
	}
	
	public static Rectangle2D addThickness(Rectangle2D b, double lineWidth) {
		double thick = lineWidth / 2;
		return new Rectangle2D.Double(
							b.getX() - thick, b.getY() - thick,
							b.getWidth() + thick * 2, b.getHeight() + thick * 2);
	}
	 ...
	 * given by the connection between points target and other. Ensures that
	 * the arrow is always larger than the line.
	 * 
	 * @param target
	 *           the point where the arrow should be put.
	 * @param other
	 *           needed to calculate the direction in which the arrow should
	 *           point to. The line is given by the two points target and other.
	 * @param lineWidth
	 *           The total width of the line. May be used to scale the
	 *           arrow so as to be larger than the line.
	...
	public Shape affix(Point2D target, Point2D other, double lineWidth) {
		...
		if (arrowWidth > 0 && this.arrowWidth < lineWidth && Math.abs(SIZE) <= 1.000001 && Math.abs(SIZE) >= 0.999999) {
