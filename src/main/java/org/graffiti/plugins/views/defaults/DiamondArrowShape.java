...
public class DiamondArrowShape extends AbstractArrowShape implements SupportsHollowDrawing {
	...
	public void updateSize(double size) {
		 ...
		 * arrow.lineTo(ns*1f/6f, ns / 2f+lineWidth/2f); arrow.lineTo(ns / 2f, ns /
		 * 6f+lineWidth/2f); arrow.lineTo(ns*5f/6f, ns / 2f+lineWidth/2f); } else {
		 * arrow.moveTo(ns / 2f, ns * 5f / 6f); arrow.lineTo(ns*1f/6f, ns / 2f);
		 * arrow.lineTo(ns / 2f, ns / 6f); arrow.lineTo(ns*5f/6f, ns / 2f); }
		 */
		float offX = -ns;
		float offY = -ns / 2f;
