...
public abstract class AbstractGraphElementComponent extends GraphElementComponent
	 ...
	 * and their position is dependent on the position (and size) of this
	 ...
	 * then treated as dependent on the position, size etc. of this
	...
	public void attributeChanged(Attribute attr) throws ShapeNotFoundException {
		...
		if (attr.getPath().startsWith(Attribute.SEPARATOR + GraphicAttributeConstants.GRAPHICS)
			...
			/*
			 * a hack due to old implementation where adding another experiment just fires a
			 * max_charts_per_row attribute change event but we need to re-adjust the
			 * graphics for that as well to make it visible
			 */
			graphicAttributeChanged(attr);
		} else
	...
	public boolean contains(int x, int y) {
		return false;
		/*
		 * AffineTransform zoom = getZoom(); Point2D p = null; try { p =
		 * zoom.inverseTransform(new Point2D.Double(x + getX(), y + getY()), null); }
		 * catch(NoninvertibleTransformException e) { } x = (int) (p.getX() - getX()); y
		 * = (int) (p.getY() - getY()); return (super.contains(x, y) &&
	...
	public void paintComponent(Graphics g) {
		...
		if (isDebuggingLevel) {
			// boolean drawFrames =
			// PreferenceManager.getPreferenceForClass(VantedPreferences.class).getBoolean(VantedPreferences.PREFERENCE_DEBUG_SHOWPANELFRAMES,
			// false);
			...
			if (drawFrames) {
				...
				/*
				 * we have to set those quirky coordinates due to rounding problems in the
				 * graphics library, so the the actual outline with height or height would not
			...
			if (drawFrames) {
				...
				String bounds = "[" + getBounds().getX() + ", " + getBounds().getY() + ", "
						+ (getBounds().getX() + getBounds().getWidth()) + ", "
						+ (getBounds().getY() + getBounds().getHeight()) + "]";
				g.drawString(bounds, 5, 5);
				bounds = "[" + getBounds().getX() + ", " + getBounds().getY() + ", " + getBounds().getWidth() + ", "
						+ getBounds().getHeight() + "]";
	 ...
	 * coordinates and local coordinates relativ to this graphcomponent so x and y
	 * are always 0 to create the correct repaint region we call the parent frame
	 * with new coordinates, which regard the zoom the parent will then call its
	 * repaintmanager, which will have then the correct coordinates to paint. This
	 * component will then be repainted, because it falls in the clipping bounds
	 * with regard to the current zoom
	 */
	@Override
	public void repaint(long tm, int x, int y, int width, int height) {
		...
		if (parent != null && parent instanceof Zoomable) {
			...
			/*
			 * adjusting the width and height with a constant, because somehow the marking
			 * border on edges doesn't disappear (gets not completely removed). So the
			 * redraw region is widened
			 *
			 */
