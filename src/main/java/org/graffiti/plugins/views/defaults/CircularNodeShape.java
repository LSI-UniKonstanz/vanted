...
//
// ==============================================================================
// $Id: CircularNodeShape.java,v 1.8 2010/12/22 13:06:19 klukas Exp $
...
public abstract class CircularNodeShape extends RectangularNodeShape {
	...
	public static Point2D getIntersectionWithCircle(Ellipse2D circle, Line2D intLine) {
		if (circle.getWidth() != circle.getHeight()) {
			throw new IllegalArgumentException("First parameter must be a circle, i.e. height and width "
					+ "must be equal. Were: width=" + circle.getWidth() + "  height=" + circle.getHeight());
		}
		...
		double c = ((cx * cx) + (cy * cy) + (sx * sx) + (sy * sy)) - (2 * ((cx * sx) + (cy * sy))) - (rad * rad);
	...
	private static Point2D calculatePointOnLine(double u, Point2D s, Point2D t) {
		...
		return new Point2D.Double(s.getX() + (u * diffx), s.getY() + (u * diffy));
	}
}
