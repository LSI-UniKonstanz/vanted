...
//
// ==============================================================================
// $Id: SelectionMenu.java,v 1.6 2010/12/22 13:05:58 klukas Exp $
...
public class SelectionMenu extends GraffitiMenu implements GraffitiComponent, SelectionListener, SessionListener {
	...
	public SelectionMenu() {
		...
		saveAction = new SelectionSaveAction(sBundle.getString("menu.saveselection"));
		saveAction.setEnabled(true);
		saveAction.putValue(Action.NAME, sBundle.getString("menu.saveselection"));
		saveItem = new GraffitiMenuItem(sBundle.getString("menu.selections"), saveAction);
		saveItem.setToolTipText(
				"Adds the current selection to the list " + "of loadable selections in this menu.\nOnly available till "
	...
	public void selectionChanged(SelectionEvent e) {
		activeSession = GravistoService.getInstance().getMainFrame().getActiveEditorSession();
		...
		SelectionChangeAction selAction = new SelectionChangeAction(clonedLastSel, activeSession);
		...
		GraffitiMenuItem item = new GraffitiMenuItem(sBundle.getString("menu.selections"), selAction);
		insert(item, 1);

		nameItemMap.put(sBundle.getString("activeSelection"), new ItemPositionPair(item, 1));
	...
	public void selectionListChanged(SelectionEvent e) {
		...
		if (nameItemMap == null) {
			...
			add(saveItem);
			nameItemMap.put(saveItem.getActionCommand(), new ItemPositionPair(saveItem, this.getItemCount() - 1));
		}

		ItemPositionPair itp = (ItemPositionPair) nameItemMap.get(e.getSelection().getName());
		Component comp = null;
		...
		if (e.toBeAdded()) {
			...
			Action selAction = new SelectionChangeAction(clonedSel, activeSession);
			selAction.setEnabled(true);
			selAction.putValue(Action.NAME, clonedSel.getName());

			GraffitiMenuItem item = new GraffitiMenuItem(sBundle.getString("menu.selections"), selAction);
			this.add(item);
			nameItemMap.put(clonedSel.getName(), new ItemPositionPair(item, this.getItemCount() - 1));
