...
public abstract class AbstractTab extends InspectorTab
	...
	public AbstractTab() {
		...
		delayThreadAttributeChanged = new DelayThread(new DelayThread.DelayedCallback() {
			...
			public void call(AttributeEvent e) {
				...
				if (e != null)
					/*
					 * until i find a better solution, this will be it. The problem is, that
					 * recreating a ChartAttribute (through rebuildTreeAction) triggers an
					 * attributechanged event.. that will trigger again this update and so we get an
					 * infinite update loop This doesn't happen when we only update the table But..
					 * Node and Edgetabs don't work properly when we only update the table
					 * especially labels will not change, if updated using the LabelEditor dialog
					 * So. for now.. we check, which tab acutally this is (Node/Edge/Graph tab are
					 * children of Abstracttab) and trigger the action appropriately
					 */
	...
	protected void rebuildTreeAction() {
		...
		this.rootNode = new DefaultMutableTreeNode(new BooledAttribute(newAttr, true));
		synchronized (this.rootNode) {
			...
			DefaultMutableTreeNode selectedNode = fillNode(this.rootNode, newAttr, attributables, oldMarkedPath);
	...
	private DefaultMutableTreeNode fillNode(DefaultMutableTreeNode treeNode, Attribute attr,
		 ...
		 * or collection which is compared as object not identical
		 */
