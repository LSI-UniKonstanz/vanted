...
public class InspectorContainer extends JTabbedPane implements GraffitiContainer {
	...
	/**
	 * adds a given tab into the hierarchy of tabs. If this hierachy doesn't exist
	 * yet it is created The hierarchy is given by the tabs category string. This
	 * category string is a '.' separated multi token string where each token
	 * represents a hierarchy level
	 */
		...
		if (nestedTabPath == null) {
			...
			switch (tab.getPreferredTabPosition()) {
				...
				case InspectorTab.TAB_LEADING:
					insertTab(tab.getTitle(), icon, tab, null, 0);
					break;
			...
			while (pathtokenizer.hasMoreTokens()) {
				...
				for (InspectorTab curTab : curListInspectorTabs) { // check all tabs of the current level
					if (curTab.getTitle().equals(curHierarchyTabName)) {
						if (curTab instanceof SubtabHostTab) {
							...
							throw new RuntimeException("Cannot add a " + tab.getTitle() + " into hierarchy, since "
									+ curTab.getTitle() + " is not of type SubtabHostTab");
						}
				...
				if (curHierarchySubHostTab == null) {
					...
					curListInspectorTabs.add(curHierarchySubHostTab); // add new subtabhosttab to current list of of
					// tabs of current level
					logger.debug("adding subhosttab:" + curHierarchySubHostTab.getTitle() + " to "
							+ curTabbedPane.toString());
					curTabbedPane.addTab(curHierarchySubHostTab.getTitle(), curHierarchySubHostTab.getIcon(),
							curHierarchySubHostTab);
					curTabbedPane = curHierarchySubHostTab.getTabbedPane();
				}
				curListInspectorTabs = curHierarchySubHostTab.getTabs(); // for the next 'recursive' call into the
				// hierarchy
			}
