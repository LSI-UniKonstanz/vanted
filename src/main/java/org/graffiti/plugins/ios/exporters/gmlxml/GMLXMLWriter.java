...
//
// ==============================================================================
// $Id: GMLXMLWriter.java,v 1.8 2011/01/03 12:35:17 klukas Exp $
...
public class GMLXMLWriter implements OutputSerializer {
	 ...
	 * Sensible e.g. for attributes that have already been given special treatment.
	 */
	...
	public void write(OutputStream o, Graph g) {
		...
		if (g.isDirected()) {
			p.println("  directed 1");
		} else {
			p.println("  directed 0");
		}
	 ...
	 * structures. An example is the coordinate attribute: All sub attributes (x, y)
	 * of coordinate have already been written somewhere else. Therefore, the
	 * coordinate attribute itself will be empty and can be ommitted. This can be
	...
	private StringBuffer getWrittenAttributeHierarchy(Attribute a, int level) {
		...
		if (a instanceof CollectionAttribute) {
			...
			if (c instanceof LabelAttribute) {
				try {
					sb.append(createTabs(level) + "label \"" + ((LabelAttribute) c).getLabel() + "\"");
					sb.append(eol);
			...
			if (colMapping.containsKey(c.getId())) {
				...
				if ("graphics".equals(c.getId())) {
					try {
						sb.append(createTabs(level + 1) + "x " + c.getAttribute("coordinate.x").getValue());
						sb.append(eol);
					} catch (AttributeNotFoundException anfe) {
						// warning(anfe.getMessage());
					}
					...
					try {
						sb.append(createTabs(level + 1) + "fill \""
								+ colToHex(c.getAttribute(GraphicAttributeConstants.FILLCOLOR)) + "\"");
						sb.append(eol);
					} catch (AttributeNotFoundException anfe) {
						// warning(anfe.getMessage());
					}
			...
			if (attMapping.containsKey(a.getId())) {
				if ("shape".equals(a.getId())) {
					...
					// }
					// else
					if ("org.graffiti.plugins.views.defaults.SmoothLineEdgeShape".equals(a.getValue())) {
						sb.append(createTabs(level) + "smooth 1" + eol);
					} else {
						if ("org.graffiti.plugins.views.defaults.RectangleNodeShape".equals(a.getValue())) {
							sb.append(createTabs(level) + attMapping.get(a.getId()) + " ");
							sb.append("\"rectangle\"");
							sb.append(eol);
						} else if ("org.graffiti.plugins.views.defaults.CircleNodeShape".equals(a.getValue())) {
							sb.append(createTabs(level) + attMapping.get(a.getId()) + " ");
							sb.append("\"oval\"");
							sb.append(eol);
						} else if ("org.graffiti.plugins.views.defaults.EllipseNodeShape".equals(a.getValue())) {
							sb.append(createTabs(level) + attMapping.get(a.getId()) + " ");
							sb.append("\"oval\"");
							sb.append(eol);
						}
					...
					if (!a.getValue().equals("")) {
						...
						try {
							if (((StringAttribute) a.getParent().getAttribute(GraphicAttributeConstants.ARROWTAIL))
									.getString().equals("")) {
					...
					if (!a.getValue().equals("")) {
						try {
							if (((StringAttribute) a.getParent().getAttribute(GraphicAttributeConstants.ARROWHEAD))
									.getString().equals("")) {
								sb.append(createTabs(level));
			...
			if (attMapping.containsKey(a.getId())) {
				...
				sb.append(createTabs(level));
				sb.append(a.getId() + " " + (((BooleanAttribute) a).getBoolean() ? 1 : 0));
				sb.append(eol);
			} else if (a instanceof ByteAttribute || a instanceof IntegerAttribute || a instanceof LongAttribute
				...
				sb.append(createTabs(level));
				sb.append(a.getId() + "_XML_ " + a.toXMLString());
				sb.append(eol);
				
				warning("wrote complex attribute: " + a.getPath() + "as XML.");
			}
		}
	...
	private void writeEdges(PrintStream p, Graph g, ArrayList nodeIds) {
		...
		for (Iterator i = g.getEdgesIterator(); i.hasNext();) {
			...
			// edgeIds.indexOf(e) is inefficient
			p.println(createTabs(2) + "id " + edgeIds.indexOf(e));
			
			p.println(createTabs(2) + "source " + nodeIds.indexOf(e.getSource()));
			
			p.println(createTabs(2) + "target " + nodeIds.indexOf(e.getTarget()));
	...
	private ArrayList writeNodes(PrintStream p, Graph g) {
		...
		for (Iterator i = g.getNodesIterator(); i.hasNext();) {
			...
			// nodeIds.indexOf(n) is inefficient
