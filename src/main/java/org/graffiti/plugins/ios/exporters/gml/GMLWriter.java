...
//
// ==============================================================================
// $Id: GMLWriter.java,v 1.32 2011/06/30 06:55:28 morla Exp $
...
public class GMLWriter implements OutputSerializer, SupportsWriterOutput {
	...
	private StringBuffer getWrittenAttributeHierarchy(Attribute a, int level) {
		...
		if (a instanceof CollectionAttribute) {
			...
			if (c instanceof LabelAttribute) {
				try {
					...
					if (la.getName().equalsIgnoreCase("labelgraphics")) {
						if (la.getLabel() != null) {
							sb.append(createTabs(level) + "label \"" + la.getLabel() + "\"");
							sb.append(eol);
						}
					}
			...
			if (a instanceof BooleanAttribute) {
				sb.append(createTabs(level));
				sb.append(a.getId() + " " + (((BooleanAttribute) a).getBoolean() ? "\"true\"" : "\"false\""));
				sb.append(eol);
				...
				sb.append(eol);
			} else if (a instanceof LineModeAttribute) {
	...
	private void writeShapeAttribute(Attribute a, int level, StringBuffer sb) {
		...
		} else if ("org.graffiti.plugins.views.defaults.QuadCurveEdgeShape".equals(a.getValue())) {
			sb.append(createTabs(level) + "quadcurve 1" + eol);
		} else if ("org.graffiti.plugins.views.defaults.StraightLineEdgeShape".equals(a.getValue())) {
	...
	private void writeEdges(PrintStream p, Graph g, HashMap<Node, Long> node2id) {
		...
		for (Edge e : g.getEdges()) {
			if (e.getSource() == null || e.getTarget() == null || !node2id.containsKey(e.getSource())
					|| !node2id.containsKey(e.getTarget())) {
				ErrorMsg.addErrorMessage("Internal error: Invalid graph edge: " + e.toString()
