...
//
// ==============================================================================
// $Id: MegaCreateTool.java,v 1.43.6.1 2014/12/03 05:35:07 klapperipk Exp $
...
public class MegaCreateTool extends MegaTools {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!selection.isEmpty()) {
					if (AdvancedLabelTool.getEditDeleteAction() != null)
						...
						GraphElementsDeletionEdit edit = new GraphElementsDeletionEdit(selection.getElements(), graph,
								geMap);
						unmarkAll();
						fireSelectionChanged();
						edit.execute();
	...
	public void activate() {
		...
		try {
			...
			MainFrame.showMessage("<html>" + "Use the mouse to create nodes and/or edges and "
	...
	public void deactivate() {
		...
		if (keyComponent != null) {
			...
			keyComponent.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
					.remove(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_ESCAPE, 0));
			keyComponent.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
	...
	public void mouseMoved(MouseEvent e) {
		// MainFrame.showMesssage("Create Edge? "+creatingEdge,
	...
	private void startAddEdge(MouseEvent e, Node clickedNode) {
		...
		dummyEdge = graph.addEdge(clickedNode, dummyNode, true, dummyEdgeCol);
	...
	private void processEdgeCreation(MouseEvent e, Node clickedNode) {
		...
		graphics.getFramecolor().setColor(new Color(red, green, blue, alpha));
		...
		// if edge visualisation is suppressed by a global view setting or isHidden of
		// the view, suppress the error,
		// because the edge will be created but not shown
		boolean invisibleEdge = tempGeComp == null && v instanceof GraffitiView
				&& (((GraffitiView) v).getBlockEdges() || ((GraffitiView) v).isHidden(edge));
			...
			public void redo() throws CannotRedoException {
				...
				try {
					...
					if (creatingEdge) {
						...
						for (int k = 0; k < 100; k++)
							if (AttributeHelper.hasAttribute(newNode,
					...
					docking = (String) AttributeHelper.getAttributeValue(newEdge1,
							GraphicAttributeConstants.DOCKING_PATH, GraphicAttributeConstants.SOURCE, "",
							new String(""), true);
					if (docking.length() > 0) {
						Vector2d size = AttributeHelper.getSize(newEdge1.getSource());
						Vector2d dockingpos = new Vector2d(
								Double.parseDouble(docking.substring(0, docking.indexOf(";"))),
					...
					docking = (String) AttributeHelper.getAttributeValue(newEdge1,
							GraphicAttributeConstants.DOCKING_PATH, GraphicAttributeConstants.TARGET, "",
							new String(""), true);
					if (docking.length() > 0) {
					...
					if (!edgebends.isEmpty()) {
						...
						tmp = Math.sqrt(
								Math.pow(edgebends.get(0).x - pos1.x, 2) + Math.pow(edgebends.get(0).y - pos1.y, 2));
						lensrc = len;
						lensection = tmp;
						...
						for (int k = 0; k < edgebends.size() - 1; k++) {
							tmp = Math.sqrt(Math.pow(edgebends.get(k + 1).x - edgebends.get(k).x, 2)
									+ Math.pow(edgebends.get(k + 1).y - edgebends.get(k + 1).y, 2));
							if (edgebends.get(k + 1).x == edgebends.get(k).x)
								tmp_d = Math.abs(pos3.x - edgebends.get(k).x);
							else {
								m = (edgebends.get(k + 1).y - edgebends.get(k).y)
										/ (edgebends.get(k + 1).x - edgebends.get(k).x);
								tmp_d = Math.abs((m * (pos3.x - edgebends.get(k).x) - pos3.y + edgebends.get(k).y)
										/ Math.sqrt(m * m + 1));
							}
							if (tmp_d < d) {
								lensrc = len;
						...
						tmp = Math.sqrt(Math.pow(pos2.x - edgebends.get(edgebends.size() - 1).x, 2)
								+ Math.pow(pos2.y - edgebends.get(edgebends.size() - 1).y, 2));
						if (pos2.x == edgebends.get(edgebends.size() - 1).x)
							tmp_d = Math.abs(pos3.x - edgebends.get(edgebends.size() - 1).x);
						else {
							m = (pos2.y - edgebends.get(edgebends.size() - 1).y)
									/ (pos2.x - edgebends.get(edgebends.size() - 1).x);
							tmp_d = Math.abs((m * (pos3.x - edgebends.get(edgebends.size() - 1).x) - pos3.y
									+ edgebends.get(edgebends.size() - 1).y) / Math.sqrt(m * m + 1));
						}
						if (tmp_d < d) {
							lensrc = len;
							lensection = tmp;
					...
					if (lensrc < len * 0.5
							&& AttributeHelper.hasAttribute(newEdge1, GraphicAttributeConstants.LABELGRAPHICS))
					...
					if (lentgt < len * 0.5
							&& AttributeHelper.hasAttribute(newEdge2, GraphicAttributeConstants.LABELGRAPHICS))
					...
					docking = (String) AttributeHelper.getAttributeValue(newEdge2,
							GraphicAttributeConstants.DOCKING_PATH, GraphicAttributeConstants.SOURCE, "",
							new String(""), true);
					if (docking.length() > 0)
