...
public class HashMapAttribute extends AbstractCollectionAttribute implements CollectionAttribute, Comparable<Object> {
		...
		if (getAttributable() == null) {
			while (it.hasNext()) {
				...
				if (o != null)
					...
					ErrorMsg.addErrorMessage("Can't copy attribute: id:" + attr.getId() + ", desc:"
							+ attr.getDescription() + ", val:" + attr.getValue());
			}
		} else {
			while (it.hasNext()) {
				Attribute attr = (Attribute) it.next();
	...
	public Object copy() {
		if (this.getClass() == HashMapAttribute.class) {
			...
			try {
				Constructor<? extends HashMapAttribute> con = this.getClass()
				...
				for (Attribute attr : values) {
					// System.out.println("Copy SubAttr: "+attr.getClass().getSimpleName()+":
					// "+attr.getId());
