...
public interface Attribute extends Displayable, DeepCopy {
	 ...
	 * <code>null</code> before. Normaly only <code>addAttribute()</code> methods
	 * should call it.
	 * 
	 * @param parent
	 ...
	 * x==y, i.e. some particular implementations can provide x==y but this
	 ...
	 * att.getValue()</code> it is only guaranteed that x.equals(y) not x==y, i.e.
	 * some particular implementations can provide x==y but this behaviour is not
	 * general, in contrast x.equals(y) must be always guaranteed. <b>See
