...
public abstract class AbstractCollectionAttribute extends AbstractAttribute implements CollectionAttribute {
	...
	/**
	 * The <code>Attributable</code> of this <code>Attribute</code>. This reference
	 * is <code>null</code> except for the root.
	 */
	 ...
	 *               if the given id contains a seperator. This is checked for in
	 ...
	 * <p>
	 * <b>Implementation Notes:</b> This method should only be called once and only
	 * by an <code>addAttribute()</code> method call! The attributable property may
	...
	public void setAttributable(Attributable att) throws FieldAlreadySetException {
		...
		assert this.getParent() == null : "Only the root attribute has a reference to the attributable "
				+ " the hierarchy belongs to. Only call setAttributable on " + "attributes where parent == null.";
	 ...
	 * <code>getCollection().isEmpty()</code> would yield, but this method should be
	 * faster since the map is not copied.
	 * 
	...
	/**
	 * Returns the value of this attribute, i.e. map between contained attributes'
	 * ids and these attributes. The behaviour of this method depends on
	 * implementation of method <code>getCollection()</code> in concret classes
	 * which inherit this one. See documentation of concret classes for more
	 * information.
	 * 
	 ...
	 * <code>ListenerManager</code> about the addition when <code>inform</code> is
	 * set to true.
	 * 
	 * @param a
	 *           the new attribute to add to the list.
	 * @param inform
	 *           when true, <code>ListenerManager</code> gets informed otherwise
	 *           not
	 * @exception AttributeExistsException
	 *               if there is already an attribute with the id of a.
	 * @exception FieldAlreadySetException
		...
		if (attrId.indexOf(Attribute.SEPARATOR) != -1) {
			...
			if (attr == null) {
				throw new AttributeNotFoundException(
						"Attribute with ID " + attrId + "does not exist in " + "this HashMapAttribute");
			} else {
	 ...
	 * <code>remove(String id)</code> with the attribute's id as parameter. Notifies
	 * <code>ListenerManager</code> with an AttributeRemoved event when the
	 * attribute hierarchy is attached to an <code>Attributable</code>.
	 * 
	 * @param attr
	...
	public String toXMLString() {
		...
		valString.append(XMLHelper.spc(4) + "<subAttributes>" + XMLHelper.getDelimiter());
