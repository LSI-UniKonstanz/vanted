...
public class ThreadSafeOptions implements HelperClass {
	...
	/**
	 * Init parameter. This parameter needs to be removed later to make this class
	 * more general.
	 */
	...
	/**
	 * Parameter which is evaluated by the springEmbedder layouter at the end of the
	 * run. This parameter needs to be removed later to make this class more
	 * general.
	 */
	...
	/**
	 * If true, the algorithm should auto-update the view. If false, the algorithm
	 * should only redraw, if the value <code>redraw</code> is true.
	 */
	...
	/**
	 * Can be interpreted by the algorithm like desired. 0 should be "not started",
	 * 1 should be "running", 2 should be "idle", 3 should be "finished". Check or
	 * overwrite <code>getRunStatus</code> so that the descriptions match.
	 */
	 ...
	 * @param defaultValue
	 *            In case the parameter is not yet set, this value will be returned.
	 ...
	 * value. It is up to the plugin that uses the options to make sure, that each
	 * time the right index for storing and restoring the parameter value is used.
	 * 
	 * @param index
	...
	/**
	 * Returns a stored boolean parameter value. If the parameter has not yet been
	 * stored the <code>defaultValue</code> is returned.
	 * 
	 * @param index
