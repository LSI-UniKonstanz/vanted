...
//
// ==============================================================================
// $Id: AbstractTool.java,v 1.20.8.1 2014/12/08 00:43:24 klapperipk Exp $
...
public abstract class AbstractTool extends MouseInputAdapter implements Tool, SessionListener, SelectionListener {
	...
	public void activate() {
		...
		// ZoomListener zoomView =
		// MainFrame.getInstance().getActiveSession().getActiveView();
	...
	public void highlight(Component comp, MouseEvent e) {
		...
		if (comp != null) {
			if (comp instanceof GraphElementComponentInterface) {
				...
				try {
					ReceiveHighlightInfo hi = (ReceiveHighlightInfo) MainFrame.getInstance().getActiveEditorSession()
							.getActiveView();
					hi.isHighlighted(n);
					processed = true;
				} catch (Exception err) {
