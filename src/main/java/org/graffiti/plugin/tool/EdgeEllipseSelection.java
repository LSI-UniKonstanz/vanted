...
/**
 * Optional edge selection that would be implemented soon. 
 * There are multiple TODOs for now!
 * 
 * @author dim8
 *
 */
class EdgeEllipseSelection {
	...
	/**
	 * This method encapsulates all the work for creating ellipse selections over an edge.
	 * After initializing the class and passing an edgeComponent instance among others, one 
	 * needs here only to specify the graphics, where the ellipses should be painted.   
	 * 
	 * @param cg the graphics to paint on
	 */
	...
	void paintEllipses(Graphics2D cg) {
		...
		float[] ellFactors = calcFactors((float) factor, geShape.getPathIterator(null), theta,
				eWidth, eHeight, mComponent.getWidth());
