...
public abstract class AbstractView extends JComponent implements View {
	...
	/**
	 * A list that stores all components (node, edge, attribute) of a view.
	 * This is basically a copy of the Component.java components list but 
	 * We need to get access to the indexOf(Object component) method of that list, which we don't.
	 * We need the index to accurately add additional components (such as charts) in the correct drawing order (behind the node (list-wise)).
	 * If we don't do that, later added charts (or other components) would appear e.g. over other nodes
	 */
