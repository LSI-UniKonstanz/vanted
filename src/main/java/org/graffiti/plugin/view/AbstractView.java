...
//
// ==============================================================================
// $Id: AbstractView.java,v 1.22 2010/12/22 13:05:53 klukas Exp $
...
public abstract class AbstractView extends JComponent implements View {
	...
	/**
	 * The <code>AttributeComponentManager</code> that tells the view what class to
	 * use as <code>AttributeComponent</code>s for attributes
	 */
	...
	/**
	 * A list that stores all components (node, edge, attribute) of a view. This is
	 * basically a copy of the Component.java components list but We need to get
	 * access to the indexOf(Object component) method of that list, which we don't.
	 * We need the index to accurately add additional components (such as charts) in
	 * the correct drawing order (behind the node (list-wise)). If we don't do that,
	 * later added charts (or other components) would appear e.g. over other nodes
	 */
	protected List<JComponent> listCopyComponents;

	/**
	 * Contains MessageListeners which are interesting for messages generated by the
	 * view i.e. if the painting of graph elements failed.
	 */
	...
	/**
	 * Extracts the name of the view class. It has to be overridden by all extended
