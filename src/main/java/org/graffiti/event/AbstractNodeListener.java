 ...
 * objects.
 * <p>
 * Extend this class to create a <code>NodeEvent</code> listener and override the methods for the events of interest. (If you implement the
 * <code>NodeListener</code> interface, you have to define all of the methods in it. This abstract class defines <code>null</code> methods for them all, so you
 * can only have to define methods for events you care about.)
 * </p>
 * <p>
 * Create a listener object using the extended class and then register it with a component using the component's <code>addNodeEventListener</code> method. When
 * a node is added, removed or the edge of a node is added ore removed, a transaction of graph structure changes is started or finished, the relevant method in
