...
public class GraphEvent extends AbstractEvent {
	...
	/**
	 * The graph that has been changed by the event. <code>Null</code> if no graph
	 * is associated with the changed graph element (e.g. after an element has been
	 * deleted) or if a different source has been specified.
	 */
	private Graph graph = null;
	
	/**
	 * The node that has been changed by the event. <code>Null</code> if no node is
	 * concerned.
	 */
	...
	/**
	 * Constructs a graph event object with the graph of the first node as source
	 * and saves the two given nodes.
	 * 
	 * @param node
	 *           the node that originated the event.
	 * @param secondNode
	...
	/**
	 * Returns the node that is concerned with that event, e.g.: the node that has
	 * been / is to be removed. Might return <code>null</code> if no node is
	 * concerned.
	 * 
	 * @return the edge that is concerned with the event.
	 */
