...
public interface SortableTableData {
	 ...
	 * <li><code>Click</code> (without <code>Ctrl</code> or <code>Alt</code>): The clicked column becomes the only sorted column. If the clicked column already
	 * was sorted, change sorting direction.
	 * </ul>
	 * 
	 * @return <code>true</code> if the table model supports
	 *         multiple sorted columns, <code>false</code> otherwise
	 ...
	 * <p>
	 * Note for implementors: If your data is dynamically changing you should think about storing copies of the arguments so you can re-sort data after each
	 * change. It may also be a good idea to call <code>fireTableDataChanged()</code> after sorting (this makes sure that the table is updated).
	 * 
	 * @param columns
	 ...
	 *           <ul>
	 *           <li><code>SORT_ASCENDING</code> - sort column data in ascending order, or <li><code>SORT_DESCENDING</code> - sort column data in descending
	 *           order
	 *           </ul>
	 * @param table
