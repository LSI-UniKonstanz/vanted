...
public class TinyRootPaneUI extends BasicRootPaneUI implements FocusListener {
	 ...
	 * metal look and feel implementation of <code>RootPaneUI</code>. If the <code>windowDecorationStyle</code> property of the <code>JRootPane</code> is other
	 * than <code>JRootPane.NONE</code>,
	 * this will add a custom <code>Component</code> to render the widgets to <code>JRootPane</code>, as well as installing a custom <code>Border</code> and
	 ...
	 * If a <code>Component</code> has been added to the <code>JRootPane</code> to render the window decoration style, this
	 * method will remove it. Similarly, this will revert the Border and
	 * LayoutManager of the <code>JRootPane</code> to what it was before <code>installUI</code> was invoked.
	 * 
	 * @param c
	 ...
	 * installed.
	 * <p>
	 * NOTE: This may be called if you haven't installed client decorations yet (ie before <code>installClientDecorations</code> has been invoked).
	 */
	 ...
	 * primarily interested in events originating from the <code>JRootPane</code> it has been installed on identifying the
	 * property <code>windowDecorationStyle</code>. If the <code>windowDecorationStyle</code> has changed to a value other than <code>JRootPane.NONE</code>, this
	 * will add a <code>Component</code> to the <code>JRootPane</code> to render the window decorations, as well
	 * as installing a <code>Border</code> on the <code>JRootPane</code>.
	 * On the other hand, if the <code>windowDecorationStyle</code> has
	 * changed to <code>JRootPane.NONE</code>, this will remove the <code>Component</code> that has been added to the <code>JRootPane</code> as well resetting
	 * the Border to what it was
	...
	private class MenuActions extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (CANCEL.equals(name)) {
				...
				if (SELECT_PARENT.equals(name)) {
					selectParentChild(PARENT);
				} else
					if (SELECT_CHILD.equals(name)) {
		...
		private void selectItem(boolean direction) {
			...
			if (path[0] instanceof JMenuBar && path[1] instanceof JMenu && len == 2) {
				...
				if (path[len - 1] instanceof JPopupMenu && path[len - 2] instanceof JMenu) {
