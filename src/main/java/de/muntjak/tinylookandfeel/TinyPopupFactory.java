...
public class TinyPopupFactory extends PopupFactory {
	...
	public static void addDialog(JDialog dialog) {
		...
		if (!dialog.isModal()) {
			...
			if (activationListener == null) {
				activationListener = new WindowAdapter() {
					...
					public void windowClosed(WindowEvent e) {
						...
						DIALOGS.remove(w);
						if (DEBUG_DIALOGS)
							System.out.println("windowClosed - " + DIALOGS.size() + " dialogs");
					}
				};
			}
			...
			if (DEBUG_DIALOGS)
				System.out.println("addDialog - " + DIALOGS.size() + " dialogs");
		}
	}
	...
	public static boolean isPopupShadowEnabled() {
		return !(TinyUtils.isOSMac() ||
							!Theme.menuPopupShadow.getValue() || TinyLookAndFeel.ROBOT == null);
	}
	...
	private static class ShadowPopup extends Popup {
		...
		static ShadowPopup getInstance(Component owner, Popup delegate, JPopupMenu contents, int x, int y) {
			...
			if (DEBUG)
				System.out.println("ShadowPopup.getInstance, cache.size=" + SHADOW_POPUP_CACHE.size() +
		...
		private void uninstall() {
			...
			if (DEBUG)
				System.out.println("ShadowPopup.uninstall, cache.size=" + SHADOW_POPUP_CACHE.size());
		}
	}
}
