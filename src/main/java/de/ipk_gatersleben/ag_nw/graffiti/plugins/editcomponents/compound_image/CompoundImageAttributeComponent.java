	...
	public void attributeChanged(Attribute attr) throws ShapeNotFoundException {
		...
		if (ge instanceof Node) {
			...
			if (attr.getName().equals(COORDINATE)) {
				...
				// System.out.println("\nStep 1 (attr changed):
				// "+AttributeHelper.getSubstanceName(n, "unknown")+" --> "+(String)
				// attr.getValue());
				recreate();
				// setSize(getPreferredSize());
			} else if (attr.getPath().startsWith(Attribute.SEPARATOR + GRAPHICS + Attribute.SEPARATOR + COORDINATE)) {
	...
	public void recreate() throws ShapeNotFoundException {
		...
		if (ge instanceof Node) {
			...
			imageComponent = CompoundImageAttributeEditor.getCompoundImageComponent(imageComponent,
					checkAndChangePath(attr.getValue().toString(), attr), false); // true
			if (imageComponent != null && !position.equals(GraphicAttributeConstants.HIDDEN)) {
				...
				if (position.equals(GraphicAttributeConstants.CENTERED)
						|| position.equals(GraphicAttributeConstants.CENTERED_FIT)) {
					offX = (nodeSize.x - imgWidth) / 2;
					offY = (nodeSize.y - imgHeight) / 2;
				} else if (position.equals(GraphicAttributeConstants.LEFT)) {
					offX = -imgWidth - off;
					offY = (nodeSize.y - imgHeight) / 2;
				} else if (position.equals(GraphicAttributeConstants.RIGHT)) {
					offX = nodeSize.x + off;
					offY = (nodeSize.y - imgHeight) / 2;
				} else if (position.equals(GraphicAttributeConstants.ABOVE)) {
					offX = (nodeSize.x - imgWidth) / 2;
					offY = -imgHeight - off;
			...
			boolean centerfitresize = (Double.compare(ns.x, imgWidth + 2) != 0
					|| Double.compare(ns.y, imgHeight + 2) != 0)
					&& CompoundImagePositionAttributeEditor.isCenteredFitPosition(n);
			boolean centerresize = (ns.x < imgWidth || ns.y < imgHeight)
					&& CompoundImagePositionAttributeEditor.isCenteredPosition(n);
