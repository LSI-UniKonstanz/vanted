...
public class CircleSearchAndLayoutAlgorithm extends AbstractAlgorithm {
	...
	public String getDescription() {
		return "<html>" + "This algorithm searches for circular sub-networks<br>"
	...
	public Parameter[] getParameters() {
		...
		return new Parameter[] {
				new IntegerParameter((startNodeCount > nNodes) ? nNodes : startNodeCount, MIN_CIRCLE_SIZE, nNodes,
						"Maximum Node Count", ""),
	...
	public void execute() {
		...
		BackgroundTaskHelper.issueSimpleTask(this.getName(), "Please wait...", new Runnable() {
			public void run() {
				...
				if (!status.wantsToStop()) {
					...
					if (circleGraphs.size() > 0) {
						...
						try {
							...
							NaivePatternFinderAlgorithm.searchPatterns(graph, circleGraphs, layout, true, true,
									startLarge, status);
						} finally {
	...
	public Set<Category> getSetCategory() {
		return new HashSet<Category>(Arrays.asList(Category.GRAPH, Category.LAYOUT, Category.SEARCH));
	}
