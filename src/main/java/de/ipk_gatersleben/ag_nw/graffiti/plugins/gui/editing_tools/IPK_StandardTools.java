...
public class IPK_StandardTools extends IPK_EditorPluginAdapter implements ViewListener, SessionListener {
	...
	public IPK_StandardTools() {
		...
		tools = new Tool[] { megaCreate, megaMove // ,
				// label,
				// megaIPKsubstrat
		};
		...
		/*
		 * labelButton = new ToolButton(label, DefaultEditMode.sid,
		 * iBundle.getImageIcon("tool.label"));
		 * labelButton.setToolTipText("Add/Modify Node- and Edge-Labels"); /*
		 * megaIPKsubstratButton = new ToolButton(megaIPKsubstrat, DefaultEditMode.sid,
		 * iBundle.getImageIcon("tool.megaCreate"));
		 */
		guiComponents = new GraffitiComponent[] { megaCreateButton, megaMoveButton };
					...
					for (GraphElement ge : s.getElements()) {
						if (c == 8) {
							...
							if (currentLabel != null) {
								...
								} else if (currentLabel != null && currentLabel.length() > 0) {
