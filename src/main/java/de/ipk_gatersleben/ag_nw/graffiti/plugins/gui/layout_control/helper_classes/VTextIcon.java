 ...
 * component which supports Icons, such as JLabel or JButton You can provide a
 * hint to indicate whether to rotate the string to the left or right, or not at
 * all, and it checks to make sure that the rotation is legal for the given
 * string (for example, Chinese/Japanese/Korean scripts have special rules when
...
public class VTextIcon implements Icon, PropertyChangeListener {
	 ...
	 * string without creating a VTextIcon from
	 * http://www.unicode.org/unicode/reports/tr9/tr9-3.html When setting text using
	 * the Arabic script in vertical lines, it is more common to employ a horizontal
	 * baseline that is rotated by 90� counterclockwise so that the characters are
	 * ordered from top to bottom. Latin text and numbers may be rotated 90�
	 * clockwise so that the characters are also ordered from top to bottom.
	 ...
	 * (clockwise) from the online edition of _The Unicode Standard, Version 3.0_,
	 * file ch10.pdf page 4 Ideographs are found in three blocks of the Unicode
	 * Standard... U+4E00-U+9FFF, U+3400-U+4DFF, U+F900-U+FAFF Hiragana is
	 * U+3040-U+309F, katakana is U+30A0-U+30FF from
	 * http://www.unicode.org/unicode/faq/writingdirections.html East Asian scripts
	 * are frequently written in vertical lines which run from top-to-bottom and are
	 * arrange columns either from left-to-right (Mongolian) or right-to-left (other
	 * scripts). Most characters use the same shape and orientation when displayed
	 * horizontally or vertically, but many punctuation characters will change their
	 * shape when displayed vertically. Letters and words from other scripts are
	 * generally rotated through ninety degree angles so that they, too, will read
	 * from top to bottom. That is, letters from left-to-right scripts will be
	 * rotated clockwise and letters from right-to-left scripts counterclockwise,
	 * both through ninety degree angles. Unlike the bidirectional case, the choice
	 * of vertical layout is usually treated as a formatting style; therefore, the
	 * Unicode Standard does not define default rendering behavior for vertical text
