...
public class KeggGmlHelper implements HelperClass {
		...
		try {
			...
			for (int i = 0; i < maxi; i++) {
				if (AttributeHelper.hasAttribute(nodeOrEdge, "kegg", "kegg_reaction_type" + i)) {
					String reactionType = (String) AttributeHelper.getAttributeValue(nodeOrEdge, "kegg",
							"kegg_reaction_type" + i, null, "");
	...
	public static String getKeggGraphicsTitle(Node n) {
		// TODO: process special characters, they are allowed in KGML/XML, but not in
		// GML
	...
	// return (String) AttributeHelper.getAttributeValue(n, "kegg", "kegg_map_link",
	// null, "");
	// }
	...
	private static ArrayList<IndexAndString> getIndexAndStringValuesFromHashMapAttribute(Attributable attributeable,
			String path, String prefixOfAttributeName) {
		ArrayList<IndexAndString> result = new ArrayList<IndexAndString>();
		try {
	 ...
	 * @param i
	 *           The index, value needs to correspond to to the method call
	 *           <code>setRelationTypeInformation</code>
	 * @param e1
	...
	public static void setRelationSrcTgtInformation(Edge e, int i, IdRef e1, IdRef e2) {
		...
		} else if (keggIdB.equals(e1.getRef().getName().getId())) {
			sourceKeggId = keggIdB;
			targetKeggId = e2.getRef().getName().getId();
		} else if (keggIdA.equals(e2.getRef().getName().getId())) {
			sourceKeggId = e1.getRef().getName().getId();
			targetKeggId = keggIdA;
		} else if (keggIdB.equals(e2.getRef().getName().getId())) {
			sourceKeggId = e1.getRef().getName().getId();
			targetKeggId = keggIdB;
		} else {
			// error: neither e1 nor e2 are corresponding to one of the end nodes of e
			ErrorMsg.addErrorMessage(
					"Internal Error: Could not find corresponding Source or Target KeggId for end point nodes of edge, representing a relation.");
			sourceKeggId = null;
			targetKeggId = null;
		}
	...
	private static boolean isInside(Node bigNode, Node n, HashMap<Node, Vector2d> node2upperLeftPos,
