...
public class RTTreeLayout extends AbstractAlgorithm {
	 ...
	 */
	private final String BENDS = GraphicAttributeConstants.GRAPHICS + Attribute.SEPARATOR
			+ GraphicAttributeConstants.BENDS;
	/**
	 * Dynamical defined edge shape attribute.
	 */
	private final String SHAPE = GraphicAttributeConstants.GRAPHICS + Attribute.SEPARATOR
	...
	/**
	 * The sum of all modifiers fields of ancestors which adjusts the successors.
	 */
	...
	public void check() throws PreconditionException {
		...
		/*
		 * g.getListenerManager().transactionStarted(this); try { ((BooleanAttribute)
		 * g.getAttribute(".directed")).setBoolean(false); } catch (Exception e) {
		 * e.printStackTrace(); } g.getListenerManager().transactionFinished(this);
		 * g.getListenerManager().transactionStarted(this); try { ((BooleanAttribute)
		 * g.getAttribute(".directed")).setBoolean(true); } catch (Exception e) {
		 * e.printStackTrace(); } g.getListenerManager().transactionFinished(this);
		 */
	...
	public Parameter[] getParameters() {
		if (xDistanceParam == null) {
			xDistanceParam = new DoubleParameter("X distance", "The distance between nodes in horizontal direction.");
			...
			treeDirectionParam = new ObjectListParameter(treeDirectionParameter[0], "Tree Direction (0,90,180,270)",
					"Move all trees in 0, 90, 180 or 270 degree", treeDirectionParameter);
			
			removeBendParam = new BooleanParameter(isRemoveBends, "Remove Bends", "Remove all bends in the forest");
			
			busLayoutParam = new BooleanParameter(isBusLayout, "Bus Layout", "Layout the trees in bus format");
			
			xDistanceParam.setDouble(xNodeDistance);
			yDistanceParam.setDouble(yNodeDistance);
		}
		return new Parameter[] { xDistanceParam, yDistanceParam, xStartParam, yStartParam, horizontalParam,
				removeBendParam, busLayoutParam, treeDirectionParam };
	}
	...
	public void setParameters(Parameter[] params) {
		...
		int i = 0;
		xNodeDistance = ((DoubleParameter) params[i++]).getDouble().doubleValue();
		yNodeDistance = ((DoubleParameter) params[i++]).getDouble().doubleValue();
		xStart = ((DoubleParameter) params[i++]).getDouble().doubleValue();
		yStart = ((DoubleParameter) params[i++]).getDouble().doubleValue();
		horizontalLayout = ((BooleanParameter) params[i++]).getBoolean().booleanValue();
		isRemoveBends = ((BooleanParameter) params[i++]).getBoolean().booleanValue();
		isBusLayout = ((BooleanParameter) params[i++]).getBoolean().booleanValue();
		treeDirection = (Integer) ((ObjectListParameter) params[i++]).getValue();
		// if (!((treeDirection == 0) || (treeDirection == 180) || (treeDirection ==
		// 270) || (treeDirection == 90))) {
		// treeDirection = 0;
		// }
	}
	...
	private Node getLeftBrother(Node node) {
		...
		if (pos != 0) {
			leftBrother = (Node) ((LinkedList) treeMap.get(depth)).get(pos - 1);
		}
	...
	public void execute() {
		...
		for (Node n : selNodes) {
			if (forest.containsKey(n)) {
				...
				computeDepth(n);
				sourceNodes.put(n, new TreeContainer(treeMap, depthOffsets, bfsNum, maxNodeHeight));
		...
		while (forest.keySet().iterator().hasNext()) {
			...
			// System.out.println("nodes left !!! "+((IntegerAttribute)
			// sourceNode.getAttribute("id")).getValue());
			
			/* check circle connection by using the depth of each node */
			computeDepth(sourceNode);
			sourceNodes.put(sourceNode, new TreeContainer(treeMap, depthOffsets, bfsNum, maxNodeHeight));
		...
		try {
			...
			for (Node sourceNode : sourceNodes.keySet()) {
				...
				maxNodeHeight = (sourceNodes.get(sourceNode)).getMaxNodeHeight();
				...
				/*
				 * g.getListenerManager().transactionStarted(this); for (Iterator it =
				 * g.getNodesIterator(); it.hasNext();) { Node node = (Node) it.next();
				 * setLabel( node, "" + ((IntegerAttribute)
	...
	private void formatBusLayout() {
		for (Iterator<Edge> iterator = graph.getEdgesIterator(); iterator.hasNext();) {
			...
			if (bfsNum.get(node1) != null) {
				...
				double yCoord1 = ((depthOffsets.get(bfsNum.get(node1)))).doubleValue()
						+ ((maxNodeHeight.get(bfsNum.get(node1)))).doubleValue() / 2 + yNodeDistance / 2;
				double yCoord2 = ((depthOffsets.get(bfsNum.get(node2)))).doubleValue()
						+ ((maxNodeHeight.get(bfsNum.get(node2)))).doubleValue() / 2 + yNodeDistance / 2;
	...
	private void computeDepth(Node startNode) {
		...
		depthOffsets.put(Integer.valueOf(0), Double.valueOf(getNodeHeight(startNode) / 2.0));
		maxNodeHeight.put(Integer.valueOf(0), Double.valueOf(getNodeHeight(startNode)));
		...
		while (!queue.isEmpty()) {
			...
			for (Node neighbour : v.getOutNeighbors()) {
				...
				if (!bfsNum.containsKey(neighbour)) {
					Integer depth = Integer.valueOf((bfsNum.get(v)).intValue() + 1);
					
					double nodeHeight = getNodeHeight(neighbour);
					
					/* Compute the maximum height of nodes in each level of the tree */
					Double maxNodeHeightValue = maxNodeHeight.get(depth);
					if (maxNodeHeightValue != null) {
						maxNodeHeight.put(depth,
								Double.valueOf(Math.max(maxNodeHeightValue.doubleValue(), nodeHeight)));
					} else {
		...
		for (int depth = 1; depth < maxNodeHeight.size(); depth++) {
			...
			double nodeHeight = (maxNodeHeight.get(Integer.valueOf(depth))).doubleValue();
			double nodeHeightAncestor = (maxNodeHeight.get(Integer.valueOf(depth - 1))).doubleValue();
			double yOffsetAncestor = (depthOffsets.get(Integer.valueOf(depth - 1))).doubleValue();
		...
		for (Iterator<Node> nodesIterator = postorder(sourceNode).iterator(); nodesIterator.hasNext();) {
			...
			if (bfsNum.containsKey(n)) {
				...
				ySpanningMax = Math.max(ySpanningMax,
						(depthOffsets.get(Integer.valueOf(level))).doubleValue() + getNodeHeight(n) / 2.0);
				
				ySpanningMin = Math.min(ySpanningMin,
						(depthOffsets.get(Integer.valueOf(level))).doubleValue() - getNodeHeight(n) / 2.0);
				
				setY(n, yStart + (depthOffsets.get(Integer.valueOf(level))).doubleValue());
			}
		}
	...
	/**
	 * Walk through the tree and computes the relative coordinates und modifiers for
	 * each node
	 */
		...
		while (nodesIterator.hasNext()) {
			...
			if (!nodeSuccessors.hasNext()) {
				...
				if (leftBrother != null) {
					...
					relativeCoord = leftOrientedCoordinates(leftBrother)
							+ (leftBrotherWidth / 2.0 + localXdist + nodeWidth / 2.0);
				}
				...
				double leftMostCoord = ((Double) relativeCoords.get(leftMost)).doubleValue();
				double leftMostModif = ((Double) modifierField.get(leftMost)).doubleValue();
				Node rightMost = leftMost;
				while (nodeSuccessors.hasNext()) {
					rightMost = (Node) nodeSuccessors.next();
				}
				double rightMostCoord = ((Double) relativeCoords.get(rightMost)).doubleValue();
				double rightMostModif = ((Double) modifierField.get(rightMost)).doubleValue();
				relativeCoord = ((leftMostCoord + leftMostModif) + (rightMostCoord + rightMostModif)) / 2;
				Node leftBrother = getLeftBrother(node);
				/* Has left brother ? */
				if (leftBrother != null) {
					/*
					 ...
					 */
					double leftBrothCoord = leftOrientedCoordinates(leftBrother);
					double leftBrotherWidth = getNodeWidth(leftBrother);
					double nodeWidth = getNodeWidth(node);
					
					/*
					 * Looks for an overlap with an already positioned tree on the left. In this
					 * case, compute how much the tree needs to be adjusted (to the right) and
					 * adjust both relative coordinates and modifier fields.
					 */
					...
					modifier = Math.max(0.0,
							(leftBrotherWidth / 2.0 + localXdist + nodeWidth / 2.0) - (relativeCoord - leftBrothCoord));
				}
				/*
				 * no left brother, set relative coordinates to center of children's position,
				 * modifier 0
				 */
			}
			...
			/*
			 * initialization of cumulative modifier field needed for
	...
	/**
	 * Computes the real coordinates for the successors by using the sum of the
	 * modifiers
	 */
	private void adjustRelativeCoordinates(Node sourceNode) {
		...
		while (nodesIterator.hasNext()) {
			Node node = nodesIterator.next();
			double nodeModif = ((Double) modifierField.get(node)).doubleValue();
			double nodeCumulModif = ((Double) cumulModifier.get(node)).doubleValue();
			
			double nodeXCoord = ((Double) relativeCoords.get(node)).doubleValue() + nodeCumulModif + nodeModif;
			
			xSpanningMax = Math.max(xSpanningMax, nodeXCoord + getNodeWidth(node) / 2.0);
			xSpanningMin = Math.min(xSpanningMin, nodeXCoord - getNodeWidth(node) / 2.0);
			...
			while (nodeSuccessors.hasNext()) {
				...
				cumulModifier.put(succ, Double.valueOf(nodeModif + nodeCumulModif));
			}
		}
	...
	/**
	 * Computes the coordinates from the left side of the tree by using the
	 * coordinates of the given leftBrother and the modifiers of the predecessors.
	 * 
	 * @param node
	 * @return
	 */
