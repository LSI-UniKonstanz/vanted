...
public class CreateFuncatGraphAlgorithm extends AbstractAlgorithm {
	...
	public Parameter[] getParameters() {
		...
		return new Parameter[] {
				...
				new ObjectListParameter(currentAlternativeSubstanceIDindex,
						"<html>" + "Index of Alternative ID<br><small>" + "(used only for provider setting 2)",
						"<html>" + "In case the alternative substance IDs should be processed, "
								+ "this setting specifies, which identifer should be used.",
						selvals),
	...
	public void execute() {
		...
		if (currentInformationProvider.equals(settingSpecialR)) {
			try {
				String fileName = OpenFileDialogService
						.getFile(new String[] { ".txt" }, "Special R output file", "Process File").getAbsolutePath();
				...
				try {
					...
					for (Node graphNode : workNodes) {
						...
						status.setCurrentStatusText1(
								"Created " + newNodes.size() + " nodes and " + newEdges.size() + " edges ");
						...
						for (String altId : checkTheseIds) {
							...
							if (currentInformationProcessingSetting.equals(settingKOgeneLookup)) {
								try {
									HashSet<BriteEntry> briteEntries = KeggBriteService.getInstance()
											.getBriteEntryForHierarchyByID("ko00001", altId);
							...
							if (currentInformationProcessingSetting.equals(settingKOenzymeLookup)) {
								...
								if (ecId != null) {
									try {
										HashSet<BriteEntry> briteEntries = KeggBriteService.getInstance()
												.getBriteEntryForHierarchyByEC("ko00001", ecId);
						...
						for (BriteEntry hierarchyInformation : hierarchyInformationsForCurrentNode) {
							...
							// StringManipulationTools.htmlToUnicode(hierarchyInformation);
							// if (hierarchyInformation.indexOf("§") >= 0) {
							// ErrorMsg
							// .addErrorMessage("Internal error: funcat/ko definition should not contain
							// character § (problematic: '"
							// + hierarchyInformation + "')");
							// } else {
							// if (hierarchyInformation.startsWith("KO Classes;")) {
							// hierarchyInformation =
							// StringManipulationTools.stringReplace(hierarchyInformation, ";",
							// "§");
							// } else {
							// hierarchyInformation =
							// StringManipulationTools.stringReplace(hierarchyInformation, ".",
							...
							do {
								...
								if (!hierarchy_tree_itemName2node.containsKey(hierarchyEntityName)) {
									...
									Node n = GraphHelper.addNodeToGraph(fgraph,
					...
					if (!status.wantsToStop()) {
						...
						MainFrame.showMessage(
								"Added " + newNodes.size() + " nodes and " + newEdges.size() + " edges to the graph",
								MessageType.INFO);
