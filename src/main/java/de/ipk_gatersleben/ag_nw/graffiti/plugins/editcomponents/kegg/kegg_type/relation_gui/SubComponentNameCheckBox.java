...
public class SubComponentNameCheckBox extends JCheckBox {
	...
	private String isSupportedText() {
		...
		if (currentRelation == null)
			supported = false;
		else if (currentRelation.getType() == RelationType.maplink) {
			if (subTypeName == SubtypeName.indirect)
				supported = true;
		} else if (currentRelation.getType() == RelationType.ECrel) {
			if (subTypeName == SubtypeName.compound)
				supported = true;
			if (subTypeName == SubtypeName.hiddenCompound)
				supported = true;
		} else if (currentRelation.getType() == RelationType.PPrel) {
			if (subTypeName == SubtypeName.compound)
				supported = true;
			...
			if (subTypeName == SubtypeName.demethylation)
				supported = true;
		} else if (currentRelation.getType() == RelationType.GErel) {
		...
		if (subTypeName != SubtypeName.compound && subTypeName != SubtypeName.hiddenCompound
				&& subTypeName != SubtypeName.activation && subTypeName != SubtypeName.inhibition
				&& subTypeName != SubtypeName.expression && subTypeName != SubtypeName.repression
				&& subTypeName != SubtypeName.indirectEffect && subTypeName != SubtypeName.indirect
				&& subTypeName != SubtypeName.stateChange && subTypeName != SubtypeName.state
				&& subTypeName != SubtypeName.binding_association && subTypeName != SubtypeName.dissociation
				&& subTypeName != SubtypeName.phosphorylation && subTypeName != SubtypeName.dephosphorylation
				&& subTypeName != SubtypeName.glycosylation && subTypeName != SubtypeName.ubiquination
				&& subTypeName != SubtypeName.methylation && subTypeName != SubtypeName.demethylation)
