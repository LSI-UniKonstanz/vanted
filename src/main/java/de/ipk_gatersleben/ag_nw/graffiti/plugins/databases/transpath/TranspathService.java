...
public class TranspathService
	...
	private static synchronized void initService(boolean initInBackground) {
		if (initInBackground) {
			if (!read_transpath_DB_txt) {
				...
				}, tps, "<html>TRANSPATH &reg; Database", "<html>TRANSPATH &reg; Database Service", true, false);
	...
	private static void readLigantTXTforReleaseInfo() {
		...
		if (f.exists()) {
			// koDBrelease = "unknown (downloaded
			// "+DateFormat.getTimeInstance(DateFormat.DEFAULT).format(new
			// Date(f.lastModified())+")");
	...
	private static void readTranspathDB() {
		...
		analyze("reference.xml", "Reference", TranspathReference.class, (Map) tpReferences, i * stepSize,
				(++i) * stepSize);
		...
		try {
			...
			try {
				...
				ErrorMsg.addErrorMessage(
						"Consult Help/Database Status to fix this problem: File not found: " + fileName);
				return null;
			}
		}
	}
	...
	public static InputSource getFileInputSource(String fileName) {
		try {
			InputSource is = new InputSource(
					new FileInputStream(new File(ReleaseInfo.getAppFolderWithFinalSep() + fileName)));
	...
	private JComponent getDownloadButton() {
		return GUIhelper.getWebsiteButton("Download", "http://www.biobase-international.com",
				...
				"<html>" + "The TRANSPATH database is available from the website<br>"
						...
						+ "The specially prepared files need to be placed in following location:<br><br>" + "<code><b>"
