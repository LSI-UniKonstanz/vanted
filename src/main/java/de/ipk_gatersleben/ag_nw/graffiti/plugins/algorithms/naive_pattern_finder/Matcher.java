...
class Matcher {
	...
	/**
	 * Finds a matching between two graph, if it exists, given the initial state of
	 * the matching process. Returns true if a match has been found.
	...
	/**
	 * Visits all the matches between two graphs, given the initial state of the
	 * match. Stops when there are no more matches, or the visitor returns true.
	 * 
	 * @param state
	 *            the state for the matching process
	 * @param visitor
	 ...
	 */
	public void match(State state, PatternVisitor visitor, PatternVisitor optVisitor2, HashSet<Node> resultNodes,
	...
	private boolean match2(State state, PatternVisitor visitor, PatternVisitor optVisitor2, HashSet<Node> resultNodes,
		...
		if (state.isGoal()) {
			...
			if (resultNodes != null)
				for (Node n : matchingNodesOfTarget)
					resultNodes.add(n);
			if (optVisitor2 != null)
				optVisitor2.visitPattern(numberOfMatchingNodes, matchingNodesOfPattern, matchingNodesOfTarget,
						additionalInformation, allowOverlap);
			return visitor.visitPattern(numberOfMatchingNodes, matchingNodesOfPattern, matchingNodesOfTarget,
					additionalInformation, allowOverlap);
		}
		...
		while (state.computeNextPair(nodeOfPattern, nodeOfTarget)) {
			logger.debug("level[" + +((UllmannSubraphIsomAdjMatrixState) state).currentRecursionLevel
					+ "] while.computeNextPair");
