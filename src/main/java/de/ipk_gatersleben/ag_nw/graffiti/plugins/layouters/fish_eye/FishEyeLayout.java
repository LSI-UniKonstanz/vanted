...
public class FishEyeLayout extends AbstractAlgorithm {
	...
	public void check() throws PreconditionException {
		...
		for (Iterator iterator = selection.getNodes().iterator(); iterator.hasNext();) {
			...
			if (forest.containsKey(sourceNode)) {
				...
				sourceNodes.put(sourceNode, new TreeContainer(bfsNum, maxNodeHeight, edges));
	...
	public Parameter[] getParameters() {
		...
		DoubleParameter distortionParam = new DoubleParameter("distortion", "The distortion factor (0-9)");
		...
		BooleanParameter polarParam = new BooleanParameter(isPolar, "polar mapping (cartesian)",
				"Polar or cartesian coordinate system mapping");
		
		BooleanParameter horizontalParam = new BooleanParameter(horizontalLayout, "trees in a row",
		...
		return new Parameter[] { selParam, distortionParam, xStartParam, yStartParam, horizontalParam, removeBendParam,
				polarParam };
	}
	...
	public void setParameters(Parameter[] params) {
		...
		horizontalLayout = ((BooleanParameter) params[4]).getBoolean().booleanValue();
		isRemoveBends = ((BooleanParameter) params[5]).getBoolean().booleanValue();
	...
	protected void computePositions() {
		...
		while (edgesIterator.hasNext()) {
			...
			Map bends = ((LinkedHashMapAttribute) edge.getAttribute(BENDS)).getCollection();
	...
	public void execute() {
		...
		for (Iterator iterator = sourceNodes.keySet().iterator(); iterator.hasNext();) {
			...
			while (nodesIterator.hasNext()) {
				...
				if (rectSize == null) {
					rectSize = new Rectangle((int) (getX(node) - getNodeWidth(node) / 2),
							(int) (getY(node) - getNodeHeight(node) / 2), 0, 0);
				}
				rectSize.add(getX(node) - getNodeWidth(node) / 2, getY(node) - getNodeHeight(node) / 2);
				rectSize.add(getX(node) + getNodeWidth(node) / 2, getY(node) + getNodeHeight(node) / 2);
			}
			...
			while (nodesIterator.hasNext()) {
				...
				if (rectSizeComputed == null) {
					rectSizeComputed = new Rectangle((int) (getX(node) - getNodeWidth(node) / 2),
							(int) (getY(node) - getNodeHeight(node) / 2), 0, 0);
				}
				
				rectSizeComputed.add(getX(node) - getNodeWidth(node) / 2, getY(node) - getNodeHeight(node) / 2);
				rectSizeComputed.add(getX(node) + getNodeWidth(node) / 2, getY(node) + getNodeHeight(node) / 2);
			}
			
			/* compute the x and y scale factors */
			float xs = (float) (rectSize.width) / (float) (rectSizeComputed.width);
			float ys = (float) (rectSize.height) / (float) (rectSizeComputed.height);
			
			/* compute the starting position of the tree */
			double dx = Math.abs(rectSizeComputed.getX() * xs - rectSize.getX());
			double dy = Math.abs(rectSizeComputed.getY() * ys - rectSize.getY());
