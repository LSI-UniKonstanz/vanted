...
public class SBML_InitialAssignment_Reader {
	...
	public void addInitialAssignments(ListOf<InitialAssignment> initialAssignmentList, Graph g) {
		...
		while (itInitialAssignment.hasNext()) {
			 ...
			 * keySBOTerm = SBML_Constants.addToNiceIdList(presentedHeadline, "SBOTerm");
			 * String keyToolTip = SBML_Constants.addToNiceIdList(presentedHeadline,
			...
			if (initialAssignment.isSetAnnotation()) {
				if (initialAssignment.getAnnotation().isSetRDFannotation()) {
					initialAssignmentHelper.setAnnotation(initialAssignment.getAnnotation());
				}
