...
public class UllmannSubraphIsomAdjMatrixState implements State {
		...
		for (int idxPat = 0; idxPat < lenArrayNodesPattern; idxPat++) {
			...
			for (int idxGraph = 0; idxGraph < lenArrayNodesGraph; idxGraph++) {
				...
				if (ignoreEdgeDirection) {
					...
					if (inDegreePatternNode <= inDegreeGraphNode && outDegreePatternNode <= outDegreeGraphNode)
						compatibilityMatrix[idxPat][idxGraph] = true;
					else
						compatibilityMatrix[idxPat][idxGraph] = false;
				}
	...
	public boolean computeNextPair(int prevPatternNodeID, int prevTargetNodeID) {
		...
		if (idxNextGraphNode < getArrayNodesGraph().length) {
			...
			// retval = checkForVincinity(); //check if the valid found nodes are actually
			// mirroring the connectivity given by pattern
			// }
			// else
			retval = true;
		} else
	...
	public boolean isGoal() {
		if (curLengthPattern == lenArrayNodesPattern) {
			...
			/*
			 * check adjacency matrix for both graphs substracting each must be 0, then the
			 * found graph is actually compatible with the pattern We have quadratic matrix
			 * since we check all pattern nodes and #foundgraphnodes == numpatternnodes
			 */
	...
	@Override
	public Object clone() {
		UllmannSubraphIsomAdjMatrixState newState = new UllmannSubraphIsomAdjMatrixState(patternGraph, targetGraph,
				ignoreEdgeDirection);
	...
	public void printCompatibilityMatrix() {
		...
		for (int y = 0; y < arrayNodesPattern.length; y++) {
			System.out.print(
					"\t" + AttributeHelper.getLabel(arrayNodesPattern[y], arrayNodesPattern[y].toString()) + "\t\t");
