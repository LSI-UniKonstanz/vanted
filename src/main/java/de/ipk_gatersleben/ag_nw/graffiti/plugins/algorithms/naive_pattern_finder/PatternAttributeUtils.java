 ...
 * corresponding definition of the strings. Pattern occurences are represented
 * as attributes of nodes fand edges. Every occurence of a pattern is marked as
 * a set of four attributes: the pattern name, the index of the pattern, the
 * index of the node (or edge) within this special occurence of the pattern, and
 * the internal number of the node (or edge) within the pattern graph. These
 * four attributes are group into a number container. This container is called
 * the position container. Every node or edge may have one or more containers,
 * which are encapsulated by another attribute named "PATTERN". The list of
 * patterns is itself encapsulated by the attribute "AGNW" which is the
 * container for the group in Gatersleben. A simple example:
 * 
 * <pre>
 * AGNW [
 *   PATTERN [
...
public class PatternAttributeUtils {
	 ...
	 */
	public static final String PATTERN_PATH = AttributeConstants.AGNW_PATH + Attribute.SEPARATOR + "PATTERN";

	/**
	 * Path to the attribute for the minimal value of additional incoming edges.
	 */
	public static final String MIN_ADD_INC_EDGES_PATH = AttributeConstants.AGNW_PATH + Attribute.SEPARATOR
			+ "MIN_ADD_INC_EDGES";

	/**
	 * Path to the attribute for the maximal value of additional incoming edges.
	 */
	public static final String MAX_ADD_INC_EDGES_PATH = AttributeConstants.AGNW_PATH + Attribute.SEPARATOR
			+ "MAX_ADD_INC_EDGES";

	/**
	 * Path to the attribute for the minimal value of additional outgoing edges.
	 */
	public static final String MIN_ADD_OUT_EDGES_PATH = AttributeConstants.AGNW_PATH + Attribute.SEPARATOR
			+ "MIN_ADD_OUT_EDGES";

	/**
	 * Path to the attribute for the maximal value of additional outgoing edges.
	 */
	public static final String MAX_ADD_OUT_EDGES_PATH = AttributeConstants.AGNW_PATH + Attribute.SEPARATOR
			+ "MAX_ADD_OUT_EDGES";

	/**
	 * The prefix of the attribute name for the pattern record. Every attribute
	 * needs a unique name which has to start with a string. Therefore we generate a
	 * new unique name based on this prefix for every occurence of a pattern at a
	 * node.
	 */
	public static final String PATTERN_RECORD_PREFIX = "PATTERN_";

	/**
	 * The attribute name for the pattern name. The pattern name is of type String
	 * and stores the name of the pattern.
	 */
	public static final String PATTERN_NAME = "PATTERN_NAME";

	/**
	 * The attribute name for the pattern index. Each pattern can be found several
	 * times in a graph. If a pattern of one type is found two times in a graph, all
	 * nodes which belong to the first accourence will get the index 1, the second
	 * found pattern will get the number 2. This number will be the same for
	 * differnt patterns. So the first found pattern of a second type will get also
	 * the number 1 for the pattern_index.
	 */
	public static final String PATTERN_INDEX = "PATTERN_INDEX";

	/**
	 * The attribute name for the node index. Every node within the occurence of a
	 * pattern will be enumerated within this attribute.
	 */
	public static final String NODE_INDEX = "NODE_INDEX";

	/**
	 * The attribute name for the edge index. Every edge within the occurence of a
	 * pattern will be enumerated within this attribute.
	 */
	public static final String EDGE_INDEX = "EDGE_INDEX";

	/**
	 * The attribute name for the internal node index. Additionally to the node
	 * index the internal id of the node within the pattern graph is saved in the
	 * target graph. Beware: The internal id may be different from the node index,
	 * as the id is automatically generated during constrution of the pattern graph!
	 */
	public static final String INTERNAL_NODE_INDEX = "INTERNAL_NODE_INDEX";

	/**
	 * The attribute name for the internal edge index. Additionally to the edge
	 * index the internal id of the edge within the pattern graph is saved in the
	 * target graph. Beware: The internal id may be different from the edge index,
	 * as the id is automatically generated during constrution of the pattern graph!
	 */
	public static final String INTERNAL_EDGE_INDEX = "INTERNAL_EDGE_INDEX";

	/**
	 * Checks, if the top level container attribute "AGNW" exists at the given
	 * graph.
	 * 
	 * @param g
	 *            a graph
	...
	/**
	 * Checks, if the pattern container attribute "AGNW.PATTERN" exists at the given
	 * graph.
	 * 
	 * @param g
	 *            a graph
	...
	/**
	 * Checks, if the pattern container for the pattern position exists at the given
	 * node.
	 * 
	 * @param n
	 *            a node
	...
	public static boolean checkExistencePattern(Node n, int position) {
		try {
			@SuppressWarnings("unused")
			Attribute tempA = n.getAttribute(PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position);
	...
	public static boolean checkExistencePattern(Edge e, int position) {
		try {
			@SuppressWarnings("unused")
			Attribute tempA = e.getAttribute(PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position);
	...
	public static void addPatternContainer(Graph g) {
		g.addAttribute(new HashMapAttribute("PATTERN"), AttributeConstants.AGNW_PATH);
	}
	...
	public static void addPatternContainer(Node n) {
		n.addAttribute(new HashMapAttribute("PATTERN"), AttributeConstants.AGNW_PATH);
	}
	...
	public static void addPatternContainer(Edge e) {
		e.addAttribute(new HashMapAttribute("PATTERN"), AttributeConstants.AGNW_PATH);
	}
	...
	public static void addPattern(Node n, int position) {
		n.addAttribute(new HashMapAttribute(PATTERN_RECORD_PREFIX + position), PATTERN_PATH);
	}
	...
	public static void addPattern(Edge e, int position) {
		e.addAttribute(new HashMapAttribute(PATTERN_RECORD_PREFIX + position), PATTERN_PATH);
	}

	/**
	 * Adds the top level container attribute "AGNW" to the given graph, if it is
	 * not already there.
	 * 
	 * @param g
	 *            a graph
	...
	/**
	 * Adds the top level container attribute "AGNW" to the given node, if it is not
	 * already there.
	 * 
	 * @param n
	 *            a node
	...
	/**
	 * Adds the top level container attribute "AGNW" to the given edge, if it is not
	 * already there.
	 * 
	 * @param e
	 *            an edge
	...
	/**
	 * Adds the pattern container attribute "AGNW.PATTERN" to the given graph, if it
	...
	/**
	 * Adds the pattern container attribute "AGNW.PATTERN" to the given node, if it
	 * is not already there.
	 * 
	 * @param n
	 *            a node
	...
	/**
	 * Adds the pattern container attribute "AGNW.PATTERN" to the given edge, if it
	 * is not already there.
	 * 
	 * @param e
	 *            an edge
	...
	/**
	 * Adds the pattern container for the pattern position to the given node, if it
	 * is not already there.
	 * 
	 * @param n
	 *            a node
	...
	/**
	 * Adds the pattern container for the pattern position to the given edge, if it
	 * is not already there.
	 * 
	 * @param e
	 *            an edge
	...
	public static String getPatternName(Node n, int position) {
		String patternNameString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + PATTERN_NAME;
	...
	public static Integer getPatternIndex(Node n, int position) {
		String patternIndexString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + PATTERN_INDEX;
	...
	public static Integer getNodeIndex(Node n, int position) {
		String nodeIndexString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + NODE_INDEX;
	...
	public static Integer getEdgeIndex(Edge e, int position) {
		String edgeIndexString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + EDGE_INDEX;
	...
	public static Integer getInternalNodeIndex(Node n, int position) {
		String nodeIndexString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + INTERNAL_NODE_INDEX;
	...
	public static Integer getInternalEdgeIndex(Edge e, int position) {
		String edgeIndexString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position
				+ Attribute.SEPARATOR + INTERNAL_EDGE_INDEX;
	 ...
	 */
	public static void addPatternInformation(Node n, int position, String patternName, Integer patternIndex,
			Integer nodeIndex, Node patternNode) {
		String patternPositionString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position;

		Attribute patternNameAttribute = StringAttribute.getTypedStringAttribute(PATTERN_NAME, patternName);
		Attribute patternIndexAttribute = new IntegerAttribute(PATTERN_INDEX, patternIndex);
	 ...
	 */
	public static void addPatternInformation(Edge e, int position, String patternName, Integer patternIndex,
			Integer edgeIndex) {
		if (e == null)
			return;
		String patternPositionString = PATTERN_PATH + Attribute.SEPARATOR + PATTERN_RECORD_PREFIX + position;
		...
		Attribute patternNameAttribute = StringAttribute.getTypedStringAttribute(PATTERN_NAME, patternName);
		Attribute patternIndexAttribute = new IntegerAttribute(PATTERN_INDEX, patternIndex);
		Attribute edgeIndexAttribute = new IntegerAttribute(EDGE_INDEX, edgeIndex);
	...
	public static int getMaximumPatternPosition(Node n) {
		if (!checkExistenceAGNWContainer(n) || !checkExistencePatternContainer(n)) {
			return 0;
		}

		CollectionAttribute allPatternContainer = (CollectionAttribute) n.getAttribute(PATTERN_PATH);
		...
		while (i.hasNext()) {
			...
			try {
				int startingPosition = attributeId.indexOf(PATTERN_RECORD_PREFIX) + PATTERN_RECORD_PREFIX.length();

				int value = Integer.parseInt(attributeId.substring(startingPosition, attributeId.length()));
	...
	public static int getMaximumPatternPosition(Edge e) {
		if (!checkExistenceAGNWContainer(e) || !checkExistencePatternContainer(e)) {
			return 0;
		}

		CollectionAttribute allPatternContainer = (CollectionAttribute) e.getAttribute(PATTERN_PATH);
	...
	public static boolean checkExistenceOfAnyPattern(Node n) {
		return getMaximumPatternPosition(n) == 0 ? false : true;
	}
	...
	public static boolean checkExistenceOfAnyPattern(Edge e) {
		return getMaximumPatternPosition(e) == 0 ? false : true;
	}

	/**
	 * Returns the position of the given pattern name and pattern index for the
	 * given node.
	 * 
	 * @param n
	 *            a node
	 ...
	 * @param patternIndex
	 *            the pattern index
	 * @return the position or zero, if the pattern name and index combination does
	 *         not exists
	 */
	public static int findPatternPosition(Node n, String patternName, Integer patternIndex) {
		...
		for (int i = 1; i <= maximumPosition; i++) {
			if (patternName.equals(PatternAttributeUtils.getPatternName(n, i))
					&& patternIndex.equals(PatternAttributeUtils.getPatternIndex(n, i))) {
				return i;
			}
		}
	...
	public static int findPatternPosition(Edge e, String patternName, Integer patternIndex) {
		...
		for (int i = 1; i <= maximumPosition; i++) {
			if (patternName.equals(PatternAttributeUtils.getPatternName(e, i))
					&& patternIndex.equals(PatternAttributeUtils.getPatternIndex(e, i))) {
				return i;
			}
		}
	...
	/**
	 * Helper methods for finding the integer value for additional edges within the
	 * node attributes
	 * 
	 * @param n
