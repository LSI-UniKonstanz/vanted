...
public class KEGG_XML_Reader_g extends AbstractInputSerializer {
	...
	private void readEntryNodes(Graph myGraph, Element kegg, HashMap<Integer, Node> mapID2GraphNode,
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			if (entry.getAttributeValue("type").equalsIgnoreCase("map")) {
				...
				StringAttribute ua = new StringAttribute("kegg_map_link", entry.getAttributeValue("name"));
				newNode.addAttribute(ua, "kegg");
				String title = getLabel(newNode, "");
				if (title.startsWith("TITLE:")) {
					setLabel(newNode, title.substring("TITLE:".length()).toUpperCase());
					setFillColor(newNode, Color.WHITE);
	...
	private void setNodeGraphicsAttribute(double posx, double posy, double frameThickness_3, double width_25,
		...
		graphics.getDimension()
				.setDimension(new Dimension((int) java.lang.Math.round(height), (int) java.lang.Math.round(width)));
	...
	private void processGraphicsEntry(Node newNode, Element xmlGraphicsEntry) {
		...
		NodeGraphicAttribute na = (NodeGraphicAttribute) newNode.getAttribute(GraphicAttributeConstants.GRAPHICS);
	...
	private Node getNearestNode(Node reactionNode, ArrayList<Node> nodeList) {
		...
		for (Node testNode : nodeList) {
			...
			double currDist = Math.sqrt((posA.x - posB.x) * (posA.x - posB.x) + (posA.y - posB.y) * (posA.y - posB.y));
	...
	private void readRelationNodes(Graph myGraph, Element kegg, HashMap<Integer, Node> mapID2GraphNode,
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			for (Object subTypeObj : entry.getChildren("subtype")) {
				...
				if (subType.getAttributeValue("name").equalsIgnoreCase("hidden compound")) {
			...
			for (NodePair np : edgesToBeAdded) {
				Edge newEdge = null;
				if (!((entry.getAttribute("type") != null)
						&& (entry.getAttributeValue("type").equalsIgnoreCase("maplink"))))
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			for (Node reactionNode : reactionNodeList) {
				...
				for (int si = 0; si < substrates.size(); si++) {
					...
					Node substrateNode = getNearestNode(reactionNode, entryName2GraphNode.get(substrateName));
					if (substrateNode == null) {
					} else {
						Edge e;
						if (reversible)
							e = addEdgeIfNotExistant(myGraph, reactionNode, substrateNode, true,
									getDefaultGraphicsAttributeForEdge(arrowColor, arrowColor, true));
						else
							e = addEdgeIfNotExistant(myGraph, substrateNode, reactionNode, false,
									getDefaultGraphicsAttributeForEdge(arrowColor, arrowColor, false));
						reactionEdges.add(e);
						setAttribute(e, "kegg", "kegg_link_reaction",
				...
				for (int pi = 0; pi < products.size(); pi++) {
					...
					Node productNode = getNearestNode(reactionNode, entryName2GraphNode.get(productName));
					if (productNode == null) {
					} else {
						Edge e;
						if (reversible)
							e = addEdgeIfNotExistant(myGraph, reactionNode, productNode, true,
									getDefaultGraphicsAttributeForEdge(arrowColor, arrowColor, true));
						else
							e = addEdgeIfNotExistant(myGraph, reactionNode, productNode, true,
									getDefaultGraphicsAttributeForEdge(arrowColor, arrowColor, true));
