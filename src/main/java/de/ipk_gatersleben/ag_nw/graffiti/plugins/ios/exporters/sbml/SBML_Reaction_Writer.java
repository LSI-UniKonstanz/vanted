		...
		while (itNodes.hasNext()) {
			...
			if (AttributeHelper.getSBMLrole(node).equals("reaction")) {
				...
				if (reactionHelper.isSetReversible(node)) {
					logger.debug("Reaction: " + reactionHelper.getID(node));
				...
				while (itEdges.hasNext()) {
					...
					if (AttributeHelper.getSBMLrole(edge).equals("reactant")) {
						...
						if (reactionHelper.isSetStoichiometry(edge)) {
							reactant.setStoichiometry(Double.parseDouble(reactionHelper.getStoichiometry(edge)));
						}
					...
					if (AttributeHelper.getSBMLrole(edge).equals("product")) {
						...
						if (reactionHelper.isSetStoichiometry(edge)) {
							product.setStoichiometry(Double.parseDouble(reactionHelper.getStoichiometry(edge)));
						}
						if (reactionHelper.isSetID(edge)) {
				...
				if (kineticLawList.size() > 0) {
					...
					while (AttributeHelper.hasAttribute(node, SBML_Constants.SBML_KINETIC_LAW,
						...
						if (localParameterHelper.isSetID(node)) {
							localParameter = new LocalParameter(localParameterHelper.getID(node), model.getLevel(),
									model.getVersion());
						} else {
						...
						if (localParameterHelper.isSetValue(node)) {
							localParameter.setValue(localParameterHelper.getValue(node));
						}
						if (localParameterHelper.isSetUnits(node)) {
							localParameter.setUnits(localParameterHelper.getUnits(node));
						}
