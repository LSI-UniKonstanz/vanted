...
public class SBGNCircleShape extends CircleNodeShape implements ProvidesAdditonalDrawingShapes {
	...
	public void buildShape(NodeGraphicAttribute nodeGraphicAttribute) {
		...
		if (width > height) // horizontal association or logical operator
			...
			if (height > width) // vertical association or logical operator
	...
	public Collection<Shape> getPostBorderShapes() {
		...
		if (width > height) { // horizontal sticks
			...
			stickShapes.add(new Line2D.Double(bounds2D.getMinX(), bounds2D.getCenterY(), bounds2D.getMinX() + length, bounds2D.getCenterY()));
			stickShapes.add(new Line2D.Double(bounds2D.getMaxX() - length, bounds2D.getCenterY(), bounds2D.getMaxX() - 1, bounds2D.getCenterY()));
		} else
			if (height > width) { // vertical sticks
				double length = (height - width) / 2d;
				stickShapes.add(new Line2D.Double(bounds2D.getCenterX(), bounds2D.getMinY(), bounds2D.getCenterX(), bounds2D.getMinY() + length));
				stickShapes.add(new Line2D.Double(bounds2D.getCenterX(), bounds2D.getMaxY() - length, bounds2D.getCenterX(), bounds2D.getMaxY() - 1));
			} else
