...
public class WorkflowHelper extends InspectorTab implements ScenarioGui, ContainsTabbedPane, HelperClass {
	...
	private JComponent getEnableDisableOption(final PluginEntry pe) {
		...
		if (pe.getDescription().isOptionalDefaultTrue())
			setting = new SettingsHelperDefaultIsTrue().getBooleanSettingsEditor(pe.getDescription().getName(),
					pe.getDescription().getName(), null, null);
		else
			setting = new SettingsHelperDefaultIsFalse().getBooleanSettingsEditor(pe.getDescription().getName(),
					pe.getDescription().getName(), null, null);
		try {
	...
	public static JPanel getWorkFlowHelp() {
		...
		step2.addGuiComponentRow(getCustomizedLabel(new JLabel("<html>Either create a graph from scratch"
				...
				5);
		step2.addGuiComponentRow(getCustomizedLabel(new JLabel("<html>Or use a existing network"
				...
				FolderPanel.getHelpButton(JLabelJavaHelpLink.getHelpActionListener("networkloading"), Color.WHITE),
				false, 5);
		step2.addGuiComponentRow(getCustomizedLabel(new JLabel("<html>Then modify the network"
				+ "<small><ul style=\"list-style-type:none\">"
		...
		step3.addGuiComponentRow(getCustomizedLabel(new JLabel(
		...
		step4.addGuiComponentRow(getCustomizedLabel(new JLabel(
				...
				FolderPanel.getHelpButton(JLabelJavaHelpLink.getHelpActionListener("sidepanels"), Color.WHITE), false,
				6);
		step4.layoutRows();
		...
		step6.addGuiComponentRow(getCustomizedLabel(new JLabel(
	...
	public void postWorkflowStep(final Algorithm algorithm, final Parameter[] params) {
		if (workflowFolderPanel != null) {
			...
			// "+ErrorMsg.removeHTMLtags(algorithm.getDescription())+"<br><br>");
			String paramList = getObjectList(params, ", ");
			// sb.append(algorithm.getClass().getCanonicalName()+".execute(new Object[]
			// {"+paramList+"});");
			//
	...
	private JPanel getScenarioControl() {
		...
		control.addGuiComponentRow(getCustomizedLabel(new JLabel("<html><small>"
				...
				// Color.WHITE
				null, false, 5);
		control.addGuiComponentRow(
				TableLayout.get4Split(getRecordButton(), getWorkflowPauseButton(), getSaveButton(), recordStatus,
			...
			public void actionPerformed(ActionEvent e) {
				if (ScenarioService.getCurrentScenario() == null)
					...
					Object[] res = MyInputHelper.getInput(
							"<html>" + "Please specify menu group and<br>" + "menu command title:", "Save Macro",
