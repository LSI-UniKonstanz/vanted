...
public class ClusterAnalysisMain {
		...
		for (String pluginLocation : pluginLocations) {
			if (pluginLocation.length() > 0) {
				...
				try {
					...
					pluginEntries[cnt++] = new DefaultPluginEntry(pluginUrl.toString(), desc);
					...
					ErrorMsg.addErrorMessage(
							"Exception for " + pluginLocation + ". Exception: " + err.getLocalizedMessage());
					messages.add(err.getLocalizedMessage());
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		// if (r2 == null)
		// ErrorMsg
		// .addErrorMessage("Error: Default Graph file could not be loaded from
		// resource.");
