...
public class EdgeComponentHelper implements HelperClass {
	public static void updatePositionForEdgeMapping(Attribute aaa, GraphElementShape geShape, String attributeName,
			String attributeFolder, double flatness, int width, int height, Point loc) {
		...
		try {
			CollectionAttribute ca = (CollectionAttribute) aaa.getAttributable()
					.getAttribute(positionAttributePathAndName);
			if (ca != null) {
		...
		loc.setLocation(labelLoc.getX() - (width / 2.0d) + posAttr.getAbsHor(),
				labelLoc.getY() - (height / 2.0d) + posAttr.getAbsVert());
	}
	 ...
	 * @return DOCUMENT ME!
	 */
	protected static Pair calculateDists(PathIterator pi, Point2D segStartPos, Point2D segEndPos) {
