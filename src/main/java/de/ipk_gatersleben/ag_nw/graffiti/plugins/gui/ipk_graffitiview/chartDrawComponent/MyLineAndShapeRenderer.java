		...
		if (showStdDevAsVerticalT || showStdDevAsFilLRange) {
			...
			if (showStdDevAsVerticalT && stdDevAvail) {
				...
				if (orientation == PlotOrientation.HORIZONTAL) {
					...
					b = new Line2D.Double(x1 - stdDevTopWidth, y1 - yStdDev1, x1 + stdDevTopWidth, y1 - yStdDev1);
					c = new Line2D.Double(x1 - stdDevTopWidth, y1 + yStdDev1, x1 + stdDevTopWidth, y1 + yStdDev1);
				}
				g2.draw(a);
				g2.draw(b);
				g2.draw(c);
			...
			if (showStdDevAsFilLRange && column != 0) {
				...
				do {
					...
					previousStdDevValue = bioDataset.getStdDevValue(row, column - 1 - offset);
					...
					tColor = new Color(tColor.getRed(), tColor.getGreen(), tColor.getBlue(), 64);
					offset++;
				...
				if (previousValue != null && previousStdDevValue != null
						&& (previousStdDevValue.doubleValue() > 0 || stdDevValue > 0)) {
					// previous data point...
					...
					Line2D lineB = null;
					if (orientation == PlotOrientation.HORIZONTAL) {
						lineA = new Line2D.Double(y0 + yStdDev0, x0, y1 + yStdDev1, x1);
						lineB = new Line2D.Double(y0 - yStdDev0, x0, y1 - yStdDev1, x1);
					} else if (orientation == PlotOrientation.VERTICAL) {
						lineA = new Line2D.Double(x0, y0 + yStdDev0, x1, y1 + yStdDev1);
						lineB = new Line2D.Double(x0, y0 - yStdDev0, x1, y1 - yStdDev1);
					}
					...
					if (column + 1 < dataset.getColumnCount()) {
						...
						nextValue = null;
					if (nextValue == null || column + 1 + nextOffset == dataset.getColumnCount()) {
						Line2D lineA = null;
						if (orientation == PlotOrientation.HORIZONTAL) {
							lineA = new Line2D.Double(y1 - yStdDev1, x1, y1 + yStdDev1, x1);
						} else if (orientation == PlotOrientation.VERTICAL) {
							lineA = new Line2D.Double(x1, y1 - yStdDev1, x1, y1 + yStdDev1);
						}
						g2.draw(lineA);
					}
				}
