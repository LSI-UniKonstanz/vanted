 ...
 * labels.
 */
public class RemoveSelectedNodesPreserveEdgesAlgorithm extends AbstractAlgorithm
	...
	public Parameter[] getParameters() {
		return new Parameter[] {
				new BooleanParameter(ignoreDirection, "Ignore Edge Direction",
	...
	public static int removeNodesPreserveEdges(ArrayList<Node> workNodes, Graph graph, boolean ignoreDirection,
			boolean layoutParallelEdges, BackgroundTaskStatusProviderSupportingExternalCall optStatus) {
		...
		for (Node n : workNodes) {
			i++;
			/**
			 * Process undirected edges as follows: all nodes that have an undirected
			 * connection to the node, which are to be deleted, will be connected by a new
			 * undirected edge to all neighbours of the node, which is to be deleted.
			 */
			...
			if (optStatus != null) {
				if (selfLoops <= 0)
					...
					optStatus.setCurrentStatusText2(
							"Created " + edgeCopies + " edge copies (" + selfLoops + " self-loops)");
		...
		if (selfLoops <= 0)
			...
			MainFrame.showMessage(
					"Removed " + workCount + "/" + workCount + " nodes, created " + selfLoops + " self-loop edge(s)!",
					MessageType.INFO);
		if (optStatus != null)
	...
	/**
	 * Deletes selected nodes (workNodes) and connects their adjacent nodes. It will
	 * also keep the overall edge connection as well as the head/tail arrow setting.
	 * 
	 * @param workNodes
	 */
				...
				for (int second = first + 1; second < edges.length; second++) {
					...
					if (sourceNode == null || targetNode == null) {
						...
						if (!source.getNeighbors().contains(target)) {
							Edge foldedEdge = g.addEdgeCopy(edge1, source, target); // both undirected (or direction
							// ignored)
					...
					if (!sourceNode.getNeighbors().contains(targetNode)) {
						...
						Edge foldedEdge = g.addEdge(sourceNode, targetNode, true,
	 ...
	 * 
	 * @param curWorkNode
	 *           the node, whose neighbours are gotten
	 * @param edge1
	 *           first incident edge
	 * @param edge2
	 *           second incident edge
	...
	private static Node[] getSourceAndTarget(Node curWorkNode, Edge edge1, Edge edge2) {
		boolean isEdge_1_Directed =
				...
				edge1.isDirected() &&
				// and visually
						!(AttributeHelper.getArrowhead(edge1).equals("")
								&& AttributeHelper.getArrowtail(edge1).equals(""));
		boolean isEdge_2_Directed =
				// semantically
				edge2.isDirected() &&
				// and visually
						!(AttributeHelper.getArrowhead(edge2).equals("")
								&& AttributeHelper.getArrowtail(edge2).equals(""));
				
		if (isEdge_1_Directed && isEdge_2_Directed) {
			/*
			 * If in Preserving scenario (ignoreDirection is true) treat both as undirected
			 * (s. method's last else), otherwise set for deletion.
			 */
		...
		/*
		 * When both are directed and there is no change in direction, otherwise take
		 * only the directed one as direction basis.
		 */
	...
	/**
	 * Below we consider only the non-preserving scenario (ignoreDirection is
	 * false).
	...
	private static Node[] getNeightbouringNodesOf(Node curWorkNode, Edge edge1, Edge edge2) {
		...
		// assure we didn't grab the curWorkNode (directed & undirected edges mixed
		// case)
	...
	private enum EdgeType {
		...
		OUT_EDGE, OUT_EDGE_IN, OUT_EDGE_BI
	}
	
	/**
	 * returns the edge type depending on the given node (the node, that will be
	 * deleted (see above)) and the edge to test
	 * 
	 * @param n
	 * @param e
	 * @return
	...
	/**
	 * returns the EdgeType for the folded edge the direction of the edge is with
	 * regard to the new target node that is the node on edge2 opposite to the node
	...
	private static EdgeType getResultEdge(EdgeType edge1, EdgeType edge2) {
		...
		if (edge1 == EdgeType.IN_EDGE || edge1 == EdgeType.OUT_EDGE_IN) {
			if (edge2 == EdgeType.OUT_EDGE || edge2 == EdgeType.OUT_EDGE_BI || edge2 == EdgeType.IN_EDGE_BI
		...
		// check combinations for an OUT_EDGE
			...
			if (edge2 == EdgeType.IN_EDGE || edge2 == EdgeType.OUT_EDGE_IN || edge2 == EdgeType.IN_EDGE_BI
	...
	public Set<Category> getSetCategory() {
		return new HashSet<Category>(Arrays.asList(Category.NODE, Category.COMPUTATION));
	}
	
	@Override
	public String getDescription() {
		return "<html><br />" + "This command helps you delete intermediate nodes,<br />"
