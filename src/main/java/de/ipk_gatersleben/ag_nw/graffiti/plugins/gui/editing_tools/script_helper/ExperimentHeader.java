...
public class ExperimentHeader implements ExperimentHeaderInterface {
	...
	public void toString(StringBuilder r, int measurementcount) {
		...
		Substance.getAttributeString(r,
				new String[] { "experimentname", "database", "remark", "coordinator", "experimenttype", "sequence",
						"excelfileid", "importusername", "importusergroup", "importdate", "startdate", "measurements",
						"imagefiles", "sizekb" },
				new Object[] { experimentName, database, remark, coordinator, experimentType, sequence, databaseId,
						importUserName, importUserGroup, AttributeHelper.getDateString(importDate),
						AttributeHelper.getDateString(startDate), measurementcount,
						(imageFiles == null ? 0 : imageFiles), sizekb },
				true);
		r.append("</experiment>");
	}
	...
	public boolean equals(Object obj) {
		...
		if (oldStyle)
			...
			String s2 = e.experimentName + ";" + e.remark + ";" + e.coordinator + ";" + e.databaseId + ";"
					...
					+ (e.importDate != null ? e.importDate.getTime() : "") + ";"
					+ (e.startDate != null ? e.startDate.getTime() : "");
			return s1.equals(s2);
		}
	}
