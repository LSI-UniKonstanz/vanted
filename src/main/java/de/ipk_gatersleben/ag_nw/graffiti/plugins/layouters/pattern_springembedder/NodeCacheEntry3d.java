...
public class NodeCacheEntry3d {
	 ...
	 * counter is increased by 1. As soon as a node has been updated, it lastTouch
	 * value is set to the current counter number. Patterns are calculated as one
	 * group. Because of the lastTouch value it can be checked, if a node has been
	 * already moved in the current springembedder run. This way the patterns are
	 * threated more equalily to the normal nodes, as they are not updated more
	 ...
	 * connected to this pattern might have the number patternType 1 and
	 ...
	 * the specific cluster nubmer. All nodes within a cluster get the same cluster
	 * index number (id). The node attribute is called cluster/cluster:xyz. This
