...
public class GridLayoutAlgorithm extends AbstractAlgorithm {
	...
	public void executeWithResize() {
		...
		MainFrame.showMessage("Grid-Layout: columns: " + nodesOnLine + ", rows: " + rows
				+ ", inital avergage node position (x/y): " + center.x + " / " + center.y, MessageType.INFO);
		...
		sel.addAll(nodes);
		if (moveToTop)
			GravistoService.getInstance().runAlgorithm(new CenterLayouterAlgorithm(), graph, sel, getActionEvent());
	}
	...
	public void layoutOnGrid(Collection<Node> workNodes, double widthHeightRatio, double xDistance, double yDistance) {
		...
		/*
		 * Computes the number of nodes on each grid line under consideration of the
		 * given width/heigth ratio
		 */
		...
		Vector2d ctr = NodeTools.getMinimumXY(workNodes, 1, 0, 0, false);
		;// NodeTools.getCenter(workNodes);
	...
	public Parameter[] getParameters() {
		...
		if (widthHeightRatioParam == null) {
			...
			yDistanceParam = new DoubleParameter("Vertical space", "The distance between nodes in vertical direction.");

			widthParam = new DoubleParameter("Node width", "The new width of the selected (or all) nodes.");

			heightParam = new DoubleParameter("Node height", "The new height.");

			limitXparameter = new BooleanParameter(setWidth, "Consider Max Nodes Horizontally",
					"If selected, the 'Max Nodes horizontally' parameter will be used");

			widthParameter = new IntegerParameter(maxX, "Max Nodes horizontally",
					"The height of the grid depends on the number of nodes and the grid width");

			// BooleanParameter moveToTopParameter =
			// new BooleanParameter(moveToTop, "Finish: Move to Upper-Left", "Move all
			// network elements to the upper left");
		...
		return new Parameter[] {
				...
				xDistanceParam, yDistanceParam, widthParam, heightParam, limitXparameter, widthParameter,
				// moveToTopParameter,
