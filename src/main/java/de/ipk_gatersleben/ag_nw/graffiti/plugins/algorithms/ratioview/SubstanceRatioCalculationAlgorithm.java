...
public class SubstanceRatioCalculationAlgorithm extends AbstractAlgorithm implements AlgorithmWithComponentDescription {
	...
	public void execute() {
		...
		BackgroundTaskHelper.issueSimpleTask("Substance-Ratio View", "Calculate substance ratios", new Runnable() {
			public void run() {
			...
			for (NodeHelper nh2 : workNodes) {
				...
				for (DataMappingId id : allIds) {
					...
					if (vl1 != null && vl1.size() > 0 && vl2 != null && vl2.size() > 0) {
						...
						double v = avgA / avgB;
						int plantID = ratioNodeHelper.memGetPlantID(id.getSpecies(), id.getGenoType(), null, null,
								null);
						added++;
						ratioNodeHelper.memSample(v, id.getReplicateId(), plantID, "ratio", id.getTimeUnit(),
								id.getTimePoint());
					}
				}
