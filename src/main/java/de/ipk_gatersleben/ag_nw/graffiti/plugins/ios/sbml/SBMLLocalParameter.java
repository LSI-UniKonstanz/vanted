...
public class SBMLLocalParameter {
	...
	public void setMetaID(Node reactionNode, String metaID) {
		if (!metaID.equals(SBML_Constants.EMPTY)) {
			AttributeHelper.setAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.META_ID).toString(), metaID);
		}
	}

	public String getMetaID(Node node) {
		if (isSetMetaID(node)) {
			return (String) attWriter.getAttribute(node, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.META_ID).toString());
		} else {
	...
	public void setSBOTerm(Node reactionNode, String sboTerm) {
		if (!sboTerm.equals(SBML_Constants.EMPTY)) {
			AttributeHelper.setAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.SBOTERM).toString(), sboTerm);
		}
	}

	public Boolean isSetSBOTerm(Node reactionNode) {
		if (AttributeHelper.hasAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
				new StringBuffer(internAttributeName).append(SBML_Constants.SBOTERM).toString())) {
			return true;
		} else {
			return false;
		}
	...
	public void setAnnotation(Node reactionNode, Annotation annotation) {
		AttributeHelper.setAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
				new StringBuffer(internAttributeName).append(SBML_Constants.ANNOTATION).toString(), annotation);
		discardedRowIDs.add(new StringBuffer(internAttributeName).append(SBML_Constants.ANNOTATION).toString());
	}

	public Boolean isSetAnnotation(Node reactionNode) {
		if (AttributeHelper.hasAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
				new StringBuffer(internAttributeName).append(SBML_Constants.ANNOTATION).toString())) {
			return true;
		} else {
			return false;
		}
	...
	public Annotation getAnnotation(Node node) {
		if (isSetAnnotation(node)) {
			return (Annotation) attWriter.getAttribute(node, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.ANNOTATION).toString());
		} else {
			return null;
		}
	...
	public void setNonRDFAnnotation(Node reactionNode, XMLNode nonRDFAnnotation) {
		AttributeHelper.setAttribute(reactionNode, SBML_Constants.SBML_KINETIC_LAW,
				new StringBuffer(internAttributeName).append(SBML_Constants.NON_RDF_ANNOTATION).toString(),
				nonRDFAnnotation);
	...
	public XMLNode getNonRDFAnnotation(Node node) {
		if (isSetNonRDFAnnotation(node)) {
			return (XMLNode) attWriter.getAttribute(node, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.NON_RDF_ANNOTATION).toString());
		} else {
			return null;
		}
	...
	public void setNotes(Node reactionNode, String notes, XMLNode notesObj) {
		if (!notes.equals(SBML_Constants.EMPTY)) {
			attReader.addNotes(notesObj, notes, reactionNode, SBML_Constants.SBML_KINETIC_LAW,
					new StringBuffer(internAttributeName).append(SBML_Constants.NOTES).toString());
		}
		// discardedRowIDs.add(new StringBuffer(internAttributeName).append(
	...
	private void initLocalParameterNideIDs(String presentedAttributeName, String internAttributeName,
