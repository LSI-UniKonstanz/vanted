...
public class KGMLerrorWindow extends JDialog {
	...
	public static void updatePanels(FolderPanel panelWarning, FolderPanel panelError,
		...
		if (warnings == null || warnings.size() <= 0) {
			...
			panelWarning.addGuiComponentRow(new JLabel("<html>Description<hr>"),
					new JLabel("<html>Related Graphelement<hr>"), false);
			panelWarning.setTitle("Warnings (" + warnings.size() + ")");
			for (Gml2PathwayWarningInformation wi : warnings) {
				panelWarning.addGuiComponentRow(
						FolderPanel.getBorderedComponent(new JLabel(wi.getWarning().toString()), 2, 0, 0, 5),
						new GraphElementSelectionLabel(wi.getCausingGraphElement()), false);
			}
		}
		...
		if (errors == null || errors.size() <= 0) {
			...
			panelError.addGuiComponentRow(new JLabel("<html>Description<hr>"),
					new JLabel("<html>Related Graphelement(s)<hr>"), false);
			panelError.setTitle("Errors (" + errors.size() + ")");
			for (Gml2PathwayErrorInformation er : errors) {
				panelError.addGuiComponentRow(
						FolderPanel.getBorderedComponent(new JLabel(er.getError().toString()), 2, 0, 0, 5),
						new GraphElementSelectionLabel(er.getCausingGraphElements()), false);
			}
		}
