 ...
 * target graph. As a special extension a match is skipped, if at least one node
 * is a member of a previous match. Therefore no node will be a member of more
 * than one match.
 * 
...
class MarkingPatternVisitor implements PatternVisitor {
	...
	/**
	 * Mapping from pattern names to a counter of occurances of the pattern in the
	 * target graph.
	 */
	...
	/**
	 * This method is invoked for every match that has been found by the matcher.
	 * The matched nodes are marked with an pattern attribute.
	 * 
	...
	public boolean visitPattern(int numberOfNodesInMatch, Node[] matchInPattern, Node[] matchInTarget,
		...
		/*
		 * if option to NOT allow overlap: if we find an already marked nodes we ignore
		 * this match completely.
		 */
	...
	public static boolean checkForDuplicateMatch(Node[] matchInTarget) {
		for (int i = 0; i < matchInTarget.length; i++) {
			...
			int patternPosition = PatternAttributeUtils.getMaximumPatternPosition(nodeInTarget);
	...
	/**
	 * Markes the given nodes in the target graph with the given pattern name and
	 * the necessary additional occurance information.
	 * 
	 * @param matchInPattern
	 ...
	 *            Name of the pattern
	 */
	private void markNodes(Node[] matchInPattern, Node[] matchInTarget, String patternName) {
		...
		for (int i = 0; i < matchInTarget.length; i++) {
			...
			PatternAttributeUtils.addPatternInformation(nodeInTarget, patternPosition, patternName, patternOccurance,
					Integer.valueOf(i + 1), matchInPattern[i]);
		}
	}
	 ...
	 *            DOCUMENT ME!
	 */
	private void markEdges(Node[] matchInPattern, Node[] matchInTarget, String patternName) {
		Integer patternOccurance = getMaxOccurance(patternName);
		...
		for (int i = 0; i < matchInPattern.length; i++) {
			...
			for (int j = 0; j < matchInPattern.length; j++) {
				...
				if (NaivePatternFinderUtils.checkIfEdgeExists(sourceInPatternGraph, targetInPatternGraph,
						ignoreEdgeDirection)) {
					Edge edgeInTargetGraph = NaivePatternFinderUtils.getUniqueDirectedEdge(matchInTarget[i],
							matchInTarget[j], ignoreEdgeDirection);

					int patternPosition = PatternAttributeUtils.getMaximumPatternPosition(edgeInTargetGraph);

					patternPosition = patternPosition + 1;

					assert (patternPosition == 1);

					PatternAttributeUtils.addPatternInformation(edgeInTargetGraph, patternPosition, patternName,
							patternOccurance, Integer.valueOf(i + 1));
	...
	private void incrMaxOccurance(String patternName) {
		Integer currentValue = (Integer) maxOccurancePatternName.get(patternName);
