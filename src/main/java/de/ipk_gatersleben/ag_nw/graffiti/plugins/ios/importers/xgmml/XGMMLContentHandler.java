...
public class XGMMLContentHandler extends XGMMLHandler {
	...
	/**
	 * Instantiates a new graph. <strong>Currently only one graph per file is
	 * supported</strong>
	 * 
	 * @throws Exception
	...
	public void instantiateNode() throws Exception {
		...
		NodeGraphicAttribute graphicAttr = new NodeGraphicAttribute(GraphicAttributeConstants.GRAPHICS);
		...
		// save node in id map
		nodeIDMap.put(attribs.getValue(XGMMLConstants.ID_ATTRIBUTE_LITERAL), currentNode);
		
		// Label
		AttributeHelper.setLabel(currentNode, attribs.getValue(XGMMLConstants.LABEL_ATTRIBUTE_LITERAL),
				XGMMLConstants.FONT_ATTRIBUTE_LITERAL, null);
		
		// Position
		AttributeHelper.setPosition(currentNode, new Point2D.Double(centerX, centerY));
	...
	public void startGraphElement(Attributes attribs) {
		...
		this.directed = "1".equals(attribs.getValue(XGMMLConstants.DIRECTED_ATTRIBUTE_LITERAL));
	}
	...
	public void startAttElement(Attributes attribs) {
		...
		if (entry.getElementName().equals(XGMMLConstants.GRAPH_ELEMENT_LITERAL)) {
			if (attribs.getValue(XGMMLConstants.NAME_ATTRIBUTE_LITERAL)
					.equals(XGMMLConstants.ATT_ELEMENT_NAME_ATTRIBUTE_VALUE_WEIGHTED)
					&& attribs.getValue(XGMMLConstants.VALUE_ATTRIBUTE_LITERAL).equals("1")) {
				this.weighted = true;
			}
		}
		...
		if (entry.getElementName().equals(XGMMLConstants.GRAPHICS_ELEMENT_LITERAL)) {
	...
	public void startGraphicsElement(Attributes attribs) throws Exception {
		try {
			if (attribs.getValue(XGMMLConstants.WIDTH_ATTRIBUTE_LITERAL) != null)
				this.width = Double.parseDouble(attribs.getValue(XGMMLConstants.WIDTH_ATTRIBUTE_LITERAL));
		} catch (NumberFormatException e) {
		}
		
		try {
			if (attribs.getValue(XGMMLConstants.HEIGHT_ATTRIBUTE_LITERAL) != null)
				this.height = Double.parseDouble(attribs.getValue(XGMMLConstants.HEIGHT_ATTRIBUTE_LITERAL));
		} catch (NumberFormatException e) {
		}
		...
		try {
			if (attribs.getValue(XGMMLConstants.FILL_ATTRIBUTE_LITERAL) != null)
				this.fillColor = new Color(
						Long.valueOf(attribs.getValue(XGMMLConstants.FILL_ATTRIBUTE_LITERAL), 16).intValue(), true);
		} catch (NumberFormatException e) {
		}
		...
		try {
			if (attribs.getValue(XGMMLConstants.OUTLINE_ATTRIBUTE_LITERAL) != null)
				this.outlineColor = new Color(
						Long.valueOf(attribs.getValue(XGMMLConstants.OUTLINE_ATTRIBUTE_LITERAL), 16).intValue(), true);
		} catch (NumberFormatException e) {
		}
	}
	...
	public void startCenterElement(Attributes attribs) throws Exception {
		this.centerX = Double.parseDouble(attribs.getValue(XGMMLConstants.X_ATTRIBUTE_LITERAL));
		
		this.centerY = Double.parseDouble(attribs.getValue(XGMMLConstants.Y_ATTRIBUTE_LITERAL));
	}
	...
	public void startPointElement(Attributes attribs) throws Exception {
		if (this.firstPoint) {
			this.path.moveTo(Float.parseFloat(attribs.getValue(XGMMLConstants.X_ATTRIBUTE_LITERAL)),
					Float.parseFloat(attribs.getValue(XGMMLConstants.Y_ATTRIBUTE_LITERAL)));
			this.firstPoint = false;
		} else
			this.path.lineTo(Float.parseFloat(attribs.getValue(XGMMLConstants.X_ATTRIBUTE_LITERAL)),
					Float.parseFloat(attribs.getValue(XGMMLConstants.Y_ATTRIBUTE_LITERAL)));
	}
	...
	public void instantiateEdge() throws Exception {
		...
		Edge edge = this.graph.addEdge(this.edgeSourceNode, this.edgeTargetNode, this.directed);
		
		AttributeHelper.setLabel(edge, attribs.getValue(XGMMLConstants.LABEL_ATTRIBUTE_LITERAL),
