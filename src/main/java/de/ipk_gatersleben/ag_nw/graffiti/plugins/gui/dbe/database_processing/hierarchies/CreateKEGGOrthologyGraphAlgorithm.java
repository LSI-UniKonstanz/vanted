...
public class CreateKEGGOrthologyGraphAlgorithm extends AbstractAlgorithm {
	...
	public CreateKEGGOrthologyGraphAlgorithm() {
		 ...
		 * is called without setting parameters
		 */
	...
	public Parameter[] getParameters() {
		/*
		 * create context sensitive menu Check, if network has alternative identifiers
		 * and only show the option to search alternative labels, if there are some
		 */
				...
				try {
						...
						if (selectedOntology.equals(settingCreateKO)) {
							...
							if (selectedIdentifierType.equals(settingEnzymeIdGiven)) {
								...
								if (selectedSearchMainLabel) {
									...
									if (curLabelToCheck != null) {
										HashSet<BriteEntry> hashSet = KeggBriteService.getInstance()
												.getBriteHierarchy("ko00001").getECEntriesMap().get(curLabelToCheck);
										if (hashSet != null)
								...
								if (selectedSearchAlternativeLabel) {
									for (int i = 1; i < labels.size(); i++) {
										...
										if (curLabelToCheck != null) {
											HashSet<BriteEntry> hashSet = KeggBriteService.getInstance()
													.getBriteHierarchy("ko00001").getECEntriesMap()
													.get(curLabelToCheck);
											if (hashSet != null)
												briteEntries.addAll(hashSet);
											// else
					...
					if (!status.wantsToStop()) {
						...
						MainFrame.showMessage(
								"Added " + newNodes.size() + " nodes and " + newEdges.size() + " edges to the network",
								MessageType.INFO);
						...
						if (newNodes.size() > 0) {
							...
							GridLayouterAlgorithm.layoutOnGrid(geneNodes, 1, 20, 20, 30,
									new Point((int) minX, (int) maxY + 50));
			...
			do {
				...
				if (lastNode != null) {
					...
					if (!lastNode.getNeighbors().contains(hierarchy_tree_node)) {
						Edge edge = graph.addEdge(hierarchy_tree_node, lastNode, true,
