...
public class SBML_UnitDefinition_Writer extends SBML_SBase_Writer {
	...
	public void addUnitDefinition(Model model, Graph g, String niceID) {
		...
		if (AttributeHelper.hasAttribute(g, niceID,
				new StringBuffer(niceID).append(SBML_Constants.UNIT_DEFINITION_ID).toString())) {
			String id = (String) getAttribute(g, niceID,
					new StringBuffer(niceID).append(SBML_Constants.UNIT_DEFINITION_ID).toString());
			if (!SBML_Constants.EMPTY.equals(id)
					&& UnitDefinition.isValidId(id, unitDefinition.getLevel(), unitDefinition.getVersion())) {
				unitDefinition.setId(id);
			}
		}
		if (AttributeHelper.hasAttribute(g, niceID,
				new StringBuffer(niceID).append(SBML_Constants.UNIT_DEFINITION_NAME).toString())) {
			String name = (String) getAttribute(g, niceID,
					new StringBuffer(niceID).append(SBML_Constants.UNIT_DEFINITION_NAME).toString());
