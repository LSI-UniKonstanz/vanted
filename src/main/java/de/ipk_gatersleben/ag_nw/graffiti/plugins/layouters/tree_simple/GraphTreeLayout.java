...
public class GraphTreeLayout extends AbstractAlgorithm {
	...
	public void check() throws PreconditionException {
		...
		for (Iterator iterator = selection.getNodes().iterator(); iterator.hasNext();) {
			...
			if (forest.containsKey(sourceNode)) {
				...
				sourceNodes.put(sourceNode, new TreeContainer(depthOffsets, bfsNum, maxNodeHeight, bfsNumUpsideDown));
		...
		while (forest.keySet().iterator().hasNext()) {
			...
			for (Iterator iterator = preorder(sourceNode).iterator(); iterator.hasNext();) {
				...
				// sourceNode.getAttribute("id")).getValue()+*/" in "+node.getInDegree()+" out
				// "+node.getOutDegree());
				...
				if ((node.getInDegree() == 0) && (node.getOutDegree() > 0)) {
					...
					computeAppropriateDepth(sourceNode);
					sourceNodes.put(sourceNode,
							new TreeContainer(depthOffsets, bfsNum, maxNodeHeight, bfsNumUpsideDown));
	...
	private void computeDepthUpsideDown(Node startNode) {
		...
		depthOffsets.put(Integer.valueOf(maxNodeHeight.size() - 1), Double.valueOf(getNodeHeight(sourceNode) / 2.0));
		maxNodeHeight.put(Integer.valueOf(maxNodeHeight.size() - 1), Double.valueOf(getNodeHeight(sourceNode)));
	...
	private void computeDepth(Node startNode) {
		...
		/* Compute the maximum height of the root node */
		depthOffsets.put(Integer.valueOf(0), Double.valueOf(getNodeHeight(sourceNode) / 2.0));
		maxNodeHeight.put(Integer.valueOf(0), Double.valueOf(getNodeHeight(sourceNode)));
	...
	public Parameter[] getParameters() {
		...
		DoubleParameter xDistanceParam = new DoubleParameter("X distance",
				"The distance between nodes in horizontal direction.");
		
		DoubleParameter yDistanceParam = new DoubleParameter("Y distance",
				"The distance between nodes in vertical direction.");
		
		DoubleParameter xStartParam = new DoubleParameter("X base",
				"The x coordinate of the starting point of the grid horizontal direction.");
		
		DoubleParameter yStartParam = new DoubleParameter("Y base",
				"The y coordinate of the starting point of the grid horizontal direction.");
		
		BooleanParameter horizontalParam = new BooleanParameter(horizontalLayout, "Place Trees in a Row",
				"Place all trees in a row");
		
		IntegerParameter treeDirectionParam = new IntegerParameter(treeDirection, "Tree Direction (0,90,180,270)",
				"Move all trees in 0, 90, 180 or 270 degree");
		
		BooleanParameter zeroLeavesParam = new BooleanParameter(zeroLeaves, "Bottom Weighted",
				"Place all leaves on the same level");
		
		BooleanParameter busLayoutParam = new BooleanParameter(isBusLayout, "Bus Layout",
				"Layout the trees in bus format");
		
		BooleanParameter removeBendParam = new BooleanParameter(isRemoveBends, "Remove Bends",
		...
		return new Parameter[] { nodeParam, xDistanceParam, yDistanceParam, xStartParam, yStartParam, horizontalParam,
				zeroLeavesParam, removeBendParam, busLayoutParam, treeDirectionParam };
	}
	...
	public void setParameters(Parameter[] params) {
		...
		xNodeDistance = ((DoubleParameter) params[1]).getDouble().doubleValue();
		yNodeDistance = ((DoubleParameter) params[2]).getDouble().doubleValue();
		...
		horizontalLayout = ((BooleanParameter) params[5]).getBoolean().booleanValue();
		zeroLeaves = ((BooleanParameter) params[6]).getBoolean().booleanValue();
		isRemoveBends = ((BooleanParameter) params[7]).getBoolean().booleanValue();
		isBusLayout = ((BooleanParameter) params[8]).getBoolean().booleanValue();
	...
	public void execute() {
		...
		for (Iterator iterator = sourceNodes.keySet().iterator(); iterator.hasNext();) {
			...
			depthOffsets = ((TreeContainer) sourceNodes.get(sourceNode)).getDepthOffset();
			bfsNum = ((TreeContainer) sourceNodes.get(sourceNode)).getBfsNum();
			bfsNumUpsideDown = ((TreeContainer) sourceNodes.get(sourceNode)).getBfsNumUpsideDown();
			maxNodeHeight = ((TreeContainer) sourceNodes.get(sourceNode)).getMaxNodeHeight();
			...
			/*
			 * calulated contains a mapping from a node so that we know that the node has a
			 * x position
			 */
			...
			/*
			 * The loop traverses the tree starting from the selected node (root) and
			 * seperates the nodes from the leaves. The tree is traversed from left to
			 * right.
			 */
			while (!q.isEmpty()) {
			...
			for (int i = 0; i < leaves.size(); i++) {
				...
				xSpanningMax = Math.max(xSpanningMax, leftBrotherCoord + nodeWidth + leftBrotherWidth / 2.0
						+ xNodeDistance - xStart + (maxNodeSizeWidth - minNodeSizeWidth) / 2);
				
				setX(node, leftBrotherCoord + nodeWidth / 2.0 + leftBrotherWidth / 2.0 + xNodeDistance
			...
			for (Iterator nodesIterator = nodes.iterator(); nodesIterator.hasNext();) {
				...
				for (Iterator neighbours = node.getNeighborsIterator(); neighbours.hasNext();) {
					...
					if (calculated.containsKey(neighbour)) {
						if (rect == null) {
							rect = new Rectangle((int) (getX(neighbour) - getNodeWidth(neighbour) / 2), 0, 0, 0);
						}
				...
				xSpanningMax = Math.max(xSpanningMax, (rect.x + (rect.width >> 1)) + getNodeWidth(node) / 2.0 - xStart);
				
				xSpanningMin = Math.min(xSpanningMin,
						
						((rect.x + (rect.width >> 1)) - getNodeWidth(node) / 2.0) - xStart);
				
				/*
				 * Rectangle subTree = null; // System.out.println(rect.width); for (Iterator it
				 * = preorder(node).iterator(); it.hasNext();) { Node n = (Node)it.next(); if
				 * (subTree == null) { subTree = new Rectangle( (int)
				 * (getX(n)-getNodeWidth(n)/2), 0, 0, 0); }
				 * System.out.print("  "+((IntegerAttribute) n.getAttribute("id")).getValue());
				 * subTree.add(getX(n)+getNodeWidth(n)/2, 0); }
				 * System.out.println(((IntegerAttribute)
				 * node.getAttribute("id")).getValue()+"   "+subTree.getWidth()+"   "
				 * +subTree+"      "+getNodeWidth(node)); if (getNodeWidth(node)>subTree.width)
				 * { System.out.println(((IntegerAttribute)
				 * node.getAttribute("id")).getValue()); for (Iterator it =
				 * calculated.keySet().iterator(); it.hasNext();) { Node correctionNode=(Node)
				 * it.next(); if (!usedNodes.containsKey(correctionNode) /*&&
				 * getX(correctionNode)>getX(node)*) { setX(correctionNode,
				 * getX(correctionNode)+(getNodeWidth(node)));
				 * System.out.println("correcting"+((IntegerAttribute)
				 * correctionNode.getAttribute("id")).getValue()); } } setX(node, (int) (rect.x
				 * + (rect.width >> 1))+(getNodeWidth(node)-subTree.width)/2); } else {
		...
		/*
		 * graph.getListenerManager().transactionStarted(this); for (Iterator it =
		 * graph.getNodesIterator(); it.hasNext();) { Node node = (Node) it.next();
	...
	private void formatBusLayout() {
		for (Iterator iterator = graph.getEdgesIterator(); iterator.hasNext();) {
			...
			if (bfs.get(node1) != null) {
				...
				double yCoord1 = ((Double) (depthOffsets.get(bfs.get(node1)))).doubleValue()
						+ ((Double) (maxNodeHeight.get(bfs.get(node1)))).doubleValue() / 2 + yNodeDistance / 2;
				double yCoord2 = ((Double) (depthOffsets.get(bfs.get(node2)))).doubleValue()
						+ ((Double) (maxNodeHeight.get(bfs.get(node2)))).doubleValue() / 2 + yNodeDistance / 2;
	...
	private void removeAllBends() {
		for (Iterator iterator = graph.getEdgesIterator(); iterator.hasNext();) {
			try {
				((LinkedHashMapAttribute) ((Edge) iterator.next()).getAttribute(BENDS)).setCollection(new HashMap());
