...
public class UtilitySuperClassFromGraph extends HelperClass {
		...
		if (RDFId.length() > 0) {
			...
			if (!model.containsID(RDFId)) {
				...
				for (int i = 1; i <= getAttributeOfSetOfString(Messages.getString("UtilitySuperClassToGraph.4"), elem)
					...
					// DB
					Attribute XrefDB = getAttributeWithOneSpecificInnerReplacement(
							Messages.getString("UtilitySuperClassToGraph.6"), elem, i);
					if (XrefDB != null)
						xref.setDb(XrefDB.getValue().toString());
					// DBVersion
					Attribute XrefDBVersion = getAttributeWithOneSpecificInnerReplacement(
							Messages.getString("UtilitySuperClassToGraph.7"), elem, i);
					if (XrefDBVersion != null)
						xref.setDbVersion(XrefDBVersion.getValue().toString());
					// Id
					Attribute XrefId = getAttributeWithOneSpecificInnerReplacement(
							Messages.getString("UtilitySuperClassToGraph.8"), elem, i);
					if (XrefId != null)
						xref.setId(XrefId.getValue().toString());
					// IdVersion
					Attribute XrefIdVersion = getAttributeWithOneSpecificInnerReplacement(
							Messages.getString("UtilitySuperClassToGraph.9"), elem, i);
	...
	protected static void getCofactor(GraphElement elem, Catalysis interaction, Model model) {
		...
		for (Attribute a : getAttributeOfSetOfString(Messages.getString("UtilitySuperClassToGraph.11"), elem)) {
			...
			if (!model.containsID(cofactorRDFId)) {
				 ...
				 * out the kind of node the component was
				 */
				...
				for (int i = 0; i < nodes.size(); i++) {
					...
					if (iNodeRDFId.equals(cofactorRDFId)) {
						...
						UtilityClassSelectorFromGraph.chooseClassToPutAttributesToModell(cofactor, cofactor.getGraph(),
								model);
					}
				}
			}
	...
	protected static void getComponent(GraphElement elem, Complex interaction, Model model) {
		...
		for (Attribute a : getAttributeOfSetOfString(Messages.getString("UtilitySuperClassToGraph.14"), elem)) {
			...
			if (!model.containsID(componentRDFId)) {
				...
				/*
				 * iterate over the nodes of the Graph to find out the kind of node the
				 * component was
				 */
				List<Node> nodes = elem.getGraph().getNodes();
				Node component = null;
				...
				for (int i = 0; i < nodes.size(); i++) {
					...
					if (iNodeRDFId.equals(componentRDFId)) {
						...
						UtilityClassSelectorFromGraph.chooseClassToPutAttributesToModell(component,
								component.getGraph(), model);
	...
	protected static void getComponentStoichiometry(GraphElement elem, Complex interaction, Model model) {
		 ...
		 * ComponentStoichiometry tab and at it this as Stoichiometry to the model
		 */
		...
		for (int i = 1; i <= set.size(); i++) {
			...
			if (!model.containsID(RDFId)) {
				...
				ArrayList<Attribute> secondset = getAttributeOfSetWithTwoInnerReplacements(
						Messages.getString("UtilitySuperClassToGraph.21"), elem, i);
	...
	protected static void getDeltaG(GraphElement elem, BiochemicalReaction interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			...
			Attribute IonicStrength = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.25"), elem, i);
			Attribute getPh = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.26"), elem, i);
			Attribute getPMg = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.27"), elem, i);
			Attribute getTemperature = getAttributeWithOneSpecificInnerReplacement(
	...
	protected static void getEntityReference(GraphElement elem, SimplePhysicalEntity interaction, Model model) {
		...
		} else if (hasAttribute(elem, Messages.getString("UtilitySuperClassToGraph.35"))) {
		...
		} else if (hasAttribute(elem, Messages.getString("UtilitySuperClassToGraph.39"))) {
		...
		} else if (hasAttribute(elem, Messages.getString("UtilitySuperClassToGraph.43"))) {
	...
	protected static void getFeature(GraphElement elem, PhysicalEntity interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			Attribute RDFIdAttribute = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.49"), elem, i);
				...
				if (!model.containsID(RDFId)) {
					...
					if (!model.containsID(ModificationTypeRDFId)) {
						...
						for (Attribute comment : getAttributeOfSetWithTwoInnerReplacements(
								Messages.getString("UtilitySuperClassToGraph.52"), elem, i)) {
					...
					if (aa != null) {
						...
						if (!model.containsID(FeatureLocationTypeRDFId)) {
							...
							for (Attribute comment : getAttributeOfSetWithTwoInnerReplacements(
									Messages.getString("UtilitySuperClassToGraph.57"), elem, i)) {
	...
	protected static void getInteractionType(GraphElement elem, Interaction interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			Attribute RDFIdAttr = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.63"), elem, i);
			String RDFId = RDFIdAttr.getValue().toString();
			InteractionVocabulary iV;
			if (!model.containsID(RDFId)) {
				iV = model.addNew(InteractionVocabulary.class, RDFId);
				for (Attribute term : getAttributeOfSetWithTwoInnerReplacements(
						Messages.getString("UtilitySuperClassToGraph.62"), elem, i)) {
	...
	protected static void getKPrime(GraphElement elem, BiochemicalReaction interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			...
			Attribute IonicStrength = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.65"), elem, i);
			Attribute getPh = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.67"), elem, i);
			Attribute getPMg = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.68"), elem, i);
			Attribute getTemperature = getAttributeWithOneSpecificInnerReplacement(
	...
	protected static void getMemberPhysicalEntity(GraphElement elem, PhysicalEntity interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			String MemberPhysicalEntityRDFId = getAttributeWithOneSpecificInnerReplacement(
					Messages.getString("UtilitySuperClassToGraph.72"), elem, i).getValue().toString();
	...
	protected static void getXRef(GraphElement elem, Entity interaction, Model model) {
		...
		for (int i = 1; i <= set.size(); i++) {
			...
			if (!model.containsID(RDFId)) {
				...
				Attribute dbversion = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.88"), elem, i);
				if (dbversion != null) {
					x.setDbVersion(dbversion.getValue().toString());
				}
				Attribute id = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.89"), elem, i);
				if (id != null) {
					x.setId(id.getValue().toString());
				}
				Attribute idversion = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.90"), elem, i);
				if (idversion != null) {
		...
		for (int i = 1; i <= set.size(); i++) {
			...
			if (!model.containsID(RDFId)) {
				...
				Attribute db = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.96"), elem, i);
				if (db != null) {
					x.setDb(db.getValue().toString());
				}
				Attribute dbversion = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.97"), elem, i);
				if (dbversion != null) {
					x.setDbVersion(dbversion.getValue().toString());
				}
				Attribute id = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.98"), elem, i);
				if (id != null) {
					x.setId(id.getValue().toString());
				}
				Attribute idversion = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.99"), elem, i);
				if (idversion != null) {
					x.setIdVersion(idversion.getValue().toString());
				}
				Attribute title = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.100"), elem, i);
				if (title != null) {
					x.setTitle(title.getValue().toString());
				}
				Attribute year = getAttributeWithOneSpecificInnerReplacement(
						Messages.getString("UtilitySuperClassToGraph.101"), elem, i);
				if (year != null) {
				...
				secondSet = getAttributeOfSetWithTwoInnerReplacements(
						Messages.getString("UtilitySuperClassToGraph.103"), elem, i);
				for (Attribute a : secondSet) {
					x.addSource(a.getValue().toString());
				}
				secondSet = getAttributeOfSetWithTwoInnerReplacements(
						Messages.getString("UtilitySuperClassToGraph.104"), elem, i);
				for (Attribute a : secondSet) {
			...
			if (!model.containsID(RDFId)) {
				...
				if (RelationshipVocRDFIdAttr != null) {
					...
					if (!model.containsID(RelationshipVocRDFId)) {
						...
						for (Attribute term : getAttributeOfSetWithTwoInnerReplacements(
								Messages.getString("UtilitySuperClassToGraph.94"), elem, i)) {
