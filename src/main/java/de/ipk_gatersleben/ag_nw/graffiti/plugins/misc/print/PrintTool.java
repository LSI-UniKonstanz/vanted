...
public class PrintTool {
	 ...
	 * <p>
	 * If the <code>View</code> supports printing its <code>print()</code> method
	 * will be invoked otherwise a {@link DefaultPrintable DefaultPrintable}
	 * instance will be wrapped around.
	 * </p>
	 * 
	 * @param view
	 *           - the view to print
	...
	public static void print(Collection<View> views) {
		...
		if (printJob.printDialog()) {
			try {
				for (View view : views) {
					printJob.setJobName(ReleaseInfo.getRunningReleaseStatus().toString() + " print "
							+ view.getGraph().getName(true));
					Printable printObject = (view instanceof Printable) ? (Printable) view : null;
					if (printObject == null)
						continue;
					printJob.setPrintable(printObject);
