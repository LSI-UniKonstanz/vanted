...
public class Entry {
	...
	public Entry(Id id, KeggId name, EntryType type, Url link, IdRef map, Collection<KeggId> reactions,
		 ...
		 * name.isGroupComplexOfKOs() || name.isGeneProductOfGivenOrganism(); if
		 * (reactions!=null) { for (KeggId reaction : reactions) { assert
		 * reaction.isReaction(); } }
		 */
	}
	...
	public Node addGraphNode(Graph graph) {
		...
		if (type == EntryType.genes || type == EntryType.group)
			n.setViewID(-1);
		// private Id id; // it is not clear if this should be saved and restored or
		...
		if (reactions != null) {
			...
			for (KeggId reaction : reactions) {
				...
				} else if (reaction.getReferenceReaction().getType() == null) {
	...
	public static Collection<Entry> getEntryElementsFromGraphNodes(KgmlIdGenerator idGenerator, List<Node> nodes,
		...
		for (Node graphNode : nodes) {
			Collection<Entry> entriesOfGraphNode = getEntryElementsFromGraphNode(idGenerator, graphNode, warnings,
					errors);
		...
		// all nodes/entries, which lay graphically inside a group/genes
		// entry/graphnode,
		// are regarded to be components of the group/genes entry
		// so the component information is retrieved from the graphical representation
		// of the
		// view
		...
		for (Entry e : result) {
			if (e.getType() == EntryType.genes || e.getType() == EntryType.group) {
				...
				if (n != null) {
					...
					Set<Node> insideNodes = KeggGmlHelper.getNodesInsideThisNode(n, nodes, node2upperLeftPos,
							node2lowerRightPos);
		...
		for (Node n : nodes) {
			if (KeggGmlHelper.getIsPartOfGroup(n)) {
				if (!assigned.contains(n)) {
					warnings.add(new Gml2PathwayWarningInformation(
							Gml2PathwayWarning.CORRESPONDING_GROUP_ENTRY_NOT_FOUND_FOR_COMPONENT, n));
				}
			}
		}
		return result;
	...
	private static Collection<Entry> getEntryElementsFromGraphNode(KgmlIdGenerator idGenerator, Node graphNode,
		...
		// these nodes contain information about elements in the linked pathway in the
		// form of attributes
		...
		if (KeggGmlHelper.getKeggType(graphNode) != null && KeggGmlHelper.getKeggType(graphNode).equals("map")) {
			...
			result.addAll(createEntryElementsFromMapNodeAttributes(idGenerator, graphNode, errors, mapRef, clusterId));
		}
		return result;
	}
	...
	private static ArrayList<Entry> createEntryElementsFromMapNodeAttributes(KgmlIdGenerator idGenerator,
		...
		for (IndexAndString ias : referencedIds) {
			...
			if (typeValue == null) {
				...
				if (refMapType == null) {
					errors.add(
							new Gml2PathwayErrorInformation(Gml2PathwayError.KEGG_REFERENCED_TYPE_INVALID, graphNode));
				}
			}
