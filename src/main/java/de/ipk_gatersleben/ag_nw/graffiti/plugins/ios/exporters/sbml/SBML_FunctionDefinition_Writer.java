...
public class SBML_FunctionDefinition_Writer extends SBML_SBase_Writer {
	...
	public void addFunctionDefinition(Model model, Graph g, String internHeadline,
		...
		SBMLFunctionDefinition functionDefinitionHelper = functionDefinitionHelperObject.addFunctionDefinition(g,
				internHeadline);
		...
		/*
		 * if(functionDefinitionHelper.isSetFunction()){ String formel =
		 * (String)getAttribute(g, internHeadline,
		 * SBML_Constants.FUNCTION_DEFINITION_FUNCTION); try{ ASTNode h =
		 * ASTNode.parseFormula(formel);//"lambda("+formel+")"); //ASTNode math =
		 * (ASTNode)getAttribute(g, niceID, headline+ATT+"ASTNode Math");
		 * //functionDefinition.setMath(math); functionDefinition.setMath(h);
