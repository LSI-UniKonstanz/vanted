...
public class NewsHelper implements HelperClass {
	...
	public JPanel getNews(final JTabbedPane tabbedPane) {
		...
		double[][] size = { { border, TableLayoutConstants.FILL, border }, // Columns
				{ border, TableLayoutConstants.PREFERRED, 2 * border, TableLayoutConstants.PREFERRED, border } }; // Rows
		...
		// subscribeButton = createNewsButton("Subscribe","Subscribe to a RSS
		// News-Feed", getSubscribeAction());
			...
			public void run() {
				...
				if (warning.length() > 0) {
					MainFrame.showMessage(
							" Active Add-on" + (incomp > 1 ? "s are " : " is ") + "not tested with "
									+ DBEgravistoHelper.DBE_GRAVISTO_VERSION + ": " + warning.toString(),
		...
		return new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				MainFrame.showMessageDialog("<html>RSS-Feeds are saved in a text-file, which will be opened<br>"
						+ "after pressing \"OK\".<br>" + "Please use this format for editing:<br><br><code>"
						+ "name of new feed<br>" + "URL1" + RSSFeedManager.urlSeparator + "URL2"
						+ RSSFeedManager.urlSeparator + "URL3" + RSSFeedManager.urlSeparator + "...</code><br><br>"
		...
		return new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Object[] res = MyInputHelper.getInput(
						"<html>Please specify the feeds name and its URLs.<br><br><small>Separate URLs by using the \"|\" symbol.",
						"Subscribe to a News Feed", new Object[] { "Feed Name", "News Feed", "URL(s)", "" });
				if (res != null && res.length == 2 && res[0] != null && !(((String) res[0]).trim().equalsIgnoreCase(""))
	...
	private void refreshFolderPanels() {
		...
		res.add(TableLayout.get3Split(newsButton, autoNews,
				TableLayout.get3Split(null/* subscribeButton */, null, editFeeds, TableLayout.PREFERRED, 4,
						TableLayout.PREFERRED, 0, 0),
				TableLayoutConstants.PREFERRED, TableLayoutConstants.FILL, TableLayoutConstants.PREFERRED), "1,1");
		// res.add(TableLayout.get3Split(newsButton, autoNews,
		// TableLayout.get3Split(editFeeds, null, closeButton, TableLayout.PREFERRED, 4,
		// TableLayout.PREFERRED, 0, 0),
		// TableLayoutConstants.PREFERRED, TableLayoutConstants.FILL,
		// TableLayoutConstants.PREFERRED), "1,1");
	...
	private ActionListener getNewsDownloadAction(final JTabbedPane tabbedPane, final JButton newsButton) {
	...
	public static void refreshNews(final RSSFeedManager rfm, final Runnable finishTask,
		...
		Runnable workTask = new Runnable() {
				...
				try {
					...
					for (int a = 0; a < fpList.size(); a++) {
						if (latestDates.get(a) != null && rfm.getDate(a) != null)
							fpList.get(a)
									.setCondensedState(latestDates.get(a).toString().equalsIgnoreCase(rfm.getDate(a)));
						else {
							boolean auto = new SettingsHelperDefaultIsTrue().isEnabled("download_release_infos");
			 ...
			 * [[url:License:url1|url2|...]] -> show url(s) [[feed:Vanted
			 * Add-ons:url1|url2|...]] -> subscribe to feed(s)
			 * [[preference:Download:url1|url2|...]] -> download file(s) to
			 * preference-folder
			 **/
			private String constructButtons(String text, ArrayList<JComponent> buttons, final JButton newsButton) {
				int a = 0;
							...
							switch (type) {
								case ADDON: {
									if (AddonManagerPlugin.getInstance() == null) {
										...
										b = GUIhelper.getWebsiteDownloadButton(title, null,
												System.getProperty("java.io.tmpdir") + ReleaseInfo.getFileSeparator(),
												"<html>" + "Please manually download the following file(s):<br><br>"
														+ "<code>" + AttributeHelper.getStringList(urls, "<br>")
														+ "</code><br><br>"
												...
												urls, "Manual Download", null, new Runnable() {
													public void run() {
														for (String u : urls) {
															if (u.toLowerCase().endsWith(".jar")) {
																...
																try {
																	AddonManagerPlugin.getInstance().installAddon(
																			System.getProperty("java.io.tmpdir"), u);
																} catch (Exception e) {
																	ErrorMsg.addErrorMessage(e);
																} catch (Error err) {
								...
								case PREFERENCE: {
									b = GUIhelper.getWebsiteDownloadButton(title, null,
											...
											"<html>" + "Please manually download the following file(s):<br><br>" + "<code>"
													+ AttributeHelper.getStringList(urls, "<br>") + "</code><br><br>"
													+ "Then move the download file into the preferences folder of the program!<br>"
													+ "(the preferences folder will be automatically opened in a few seconds)<br><br>",
											urls, "Manual Download", null);
									...
									((JButton) b).addActionListener(new ActionListener() {
										public void actionPerformed(ActionEvent e) {
											if (rfm.registerNewFeed(title, urls)) {
												...
												MainFrame.showMessage("Subscription to Feed(-URLs) exists already!",
								...
								case DOWNLOAD: {
									b = GUIhelper.getWebsiteDownloadButton(title, null, null, "<html>"
											+ "Please manually download the following file(s):<br><br>" + "<code>"
											+ AttributeHelper.getStringList(urls, "<br>") + "</code><br><br>"
			...
			public void run() {
				for (int i = 0; i < urls.size(); i++) {
					...
					if (newsItems == null || newsItems.size() <= 0) {
						JLabel jl = new JLabelHTMLlink("<html><small><b>No news, click here to visit website</b>",
								urls.get(i), "Open feed URL in browser");
