...
public class SampleAverage implements SampleAverageInterface {
	...
	public void getString(StringBuilder r) {
		r.append("<average");
		Substance.getAttributeString(r, new String[] { "max", "min", "replicates", "stddev", "unit" },
				new Object[] { max, min, replicates, stddev, getUnit() });
		r.append(">" + getValue());
		r.append("</average>");
	}
	...
	private void setValueFromAttribute(Attribute a) {
		...
		} else if (a.getName().equals("replicates")) {
			try {
				setReplicateId(Integer.parseInt(a.getValue()));
			} catch (Exception e) {
				ErrorMsg.addErrorMessage(e);
			}
		} else if (a.getName().equals("stddev")) {
			try {
				setStddev(Double.parseDouble(a.getValue()));
			} catch (Exception e) {
				ErrorMsg.addErrorMessage(e);
			}
		} else if (a.getName().equals("unit")) {
			try {
				setUnit(a.getValue());
			} catch (Exception e) {
				ErrorMsg.addErrorMessage(e);
