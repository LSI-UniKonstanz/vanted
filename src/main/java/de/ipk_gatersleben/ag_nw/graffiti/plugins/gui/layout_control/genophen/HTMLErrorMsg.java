...
public class HTMLErrorMsg {
	public static void printErrorMessageAndQuit(Connection conn, String heading, String text, String errmsg) {
		...
		try {
			...
			System.out.println(
					"<table class=\"warning\"><tr><th class=\"warning\">Could not revert to a save database condition</th</tr>");
			System.out.println(
					"<tr><td>The rollback command was not successfull. Please do not use this Webservice for now and inform the database adminstrators.<br>The last import could not succed successfully.</td></tr>");
		...
		if (GravistoService.getInstance().getMainFrame() != null) {
			int res = JOptionPane.showConfirmDialog(GravistoService.getInstance().getMainFrame(), "<html><h1>" + heading
