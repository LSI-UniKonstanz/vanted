...
public class RSSFeedManager implements HelperClass {
	...
	private String getStandardFeedDefinition(String text) {
		if (ReleaseInfo.getRunningReleaseStatus() == Release.KGML_EDITOR) {
			...
			if (text.indexOf(url1) <= 0)
				text = "KGML-ED Notes" + System.getProperty("line.separator") + url1
			...
			if (text.indexOf(url2) <= 0)
				text = "Development Notes" + System.getProperty("line.separator") + url2
						+ System.getProperty("line.separator");
		}
		/*
		 ...
		 */
		// String url3 =
		// "https://immersive-analytics.infotech.monash.edu/vanted/literature.xml";
	...
	public void saveRegisteredFeeds() {
		...
		for (int i = 0; i < desc.size(); i++) {
			...
			text += desc.get(i) + ((date.get(i) == null) ? "" : urlSeparator + date.get(i))
					+ System.getProperty("line.separator");
	...
	public void addFolderPanel(String title, double border) {
		...
		items.addGuiComponentRow(getCustomizedLabel(new JLabel("<html><small>" + "News have not been downloaded.")),
				null, false, 5);
