...
public class Graphics {
	...
	public static Graphics getGraphicsFromKgmlElement(Element graphicsElement) {
		if (graphicsElement == null)
			...
			if (type == kgmlGraphicsType.line) {
				...
				// can't currently specify graph edges, but they specify nodes, therefore, it
				// would be
				...
				if (coordsForLine != null && coordsForLine.length() > 0 && x == null && y == null) {
					...
					if (coo.length == 4) {
						...
						// x1 and x2 (y1 and y2) might be swapped
						// make sure width and height are greater zero
						// width = kgmlNumber.getNumber((width.getValue() - x.getValue()) + "");
						// height = kgmlNumber.getNumber((height.getValue() - y.getValue()) + "");
						width = kgmlNumber.getNumber(Math.abs(width.getValue() - x.getValue()) + "");
	...
	public static void processDefaultNodeDesign(Node n, Entry entry) {
		...
		// so far the node label was set to "id (hidden compound)"
		// since there is no hidden compound in KEGG the label should just be the id
