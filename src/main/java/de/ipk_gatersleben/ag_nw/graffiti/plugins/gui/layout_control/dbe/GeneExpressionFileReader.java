...
public class GeneExpressionFileReader extends ExperimentDataFileReader {
	...
	public ExperimentInterface getXMLDataFromExcelTable(File excelFile, TableData myData,
		 ...
		 * experimentData, ec_number, data_start_col, anno)); return
	 ...
	 * annotation is not added to the list.
	 * 
	 * @param anno
	 * @param myData
	 * @param attributeName
	...
	private static void checkHeaderInfo(ArrayList<Annotation> anno, TableData myData, String attributeName,
		...
		if (getHeaderColumn(myData, excelColumnHeader) >= 0) {
			...
			// ErrorMsg.addErrorMessage("Information: Could not find Excel Column Header
			// \""+excelColumnHeader+"\".");
		}
	}
		...
		for (int row = 2; row <= myData.getMaximumRow(); row++) {
			...
			if (val != null) {
				if (knownSubstanceIds.contains(val)) {
					ErrorMsg.addErrorMessage("<b>Duplicate substance row (row " + row + "). Substance ID: " + val
							+ "</b><br>"
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					MainFrame.showMessageDialog(
							"<html>For this input format each measured substance needs to be placed<br>"
									...
									+ "Error details are available from the menu command Help/Error Messages",
							"Format Error");
				}
			});
		} else {
				...
				for (Iterator<String> plantIt = experimentData.getPlantOrLineIterator(myData, 1, row); plantIt
					...
					for (Iterator<TimeAndPlantName> sampleIt = experimentData.getSampleTimeIterator(myData, plantOrLine,
						...
						for (ReplicateDouble rd : measurements) {
							...
							// ADD DATA ENTRY
							NumericMeasurementInterface mesEntry = Experiment.getTypeManager()
									.getNewMeasurement(sampleEntry);
