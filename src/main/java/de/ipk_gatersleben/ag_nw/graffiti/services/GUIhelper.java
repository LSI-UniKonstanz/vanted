...
public class GUIhelper {
		...
		return new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (optIntroText != null) {
					...
					t.start();
					showMessageDialog(optIntroText, optIntroDialogTitle, new String[] { "OK" });
				} else {
					AttributeHelper.showInBrowser(url);
					if (opt_local_folder != null && opt_local_folder.length() > 0)
						AttributeHelper.showInBrowser(opt_local_folder);
	...
	public static JComponent getWebsiteDownloadButton(final String title, final String optUrlManualDownloadWebsite,
		...
		res.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (res.getClientProperty("X-CANCEL") != null) {
					Boolean cancel = (Boolean) res.getClientProperty("X-CANCEL");
					if (cancel)
						return;
				}
	...
	public static boolean performDownload(String downloadURL, String opt_local_folder,
		...
		if (status == null)
			status = new BackgroundTaskConsoleLogger("downloading: " + downloadURL,
					"target folder: " + opt_local_folder, false);
		...
		try {
			...
			try {
				...
				if (ftp) {
					...
					status.setCurrentStatusText1("Received " + (numWritten / 1024) + " KB");
					status.setCurrentStatusText2("Finished: " + fileName);
				}
			} catch (Exception e) {
				ErrorMsg.addErrorMessage(e);
	...
	private static boolean processDownload(final BackgroundTaskStatusProviderSupportingExternalCallImpl status,
		...
		try {
			...
			if (!result) {
				...
				System.out.println("Download completed (" + f.getAbsolutePath() + ", " + (f.length() / 1024) + " KB, "
						+ (int) ((f.length() / 1024d / (tB - tA) * 1000d)) + " KB/s).");
			}
			BackgroundTaskHelper.executeLaterOnSwingTask(10000, new Runnable() {
				public void run() {
					try {
