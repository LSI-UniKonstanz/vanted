...
public class TableLayout implements java.awt.LayoutManager2, java.io.Serializable, TableLayoutConstants {
	...
	/**
	 * Offsets of rows in pixels. The left boarder of row n is at rowOffset[n] and
	 * the right boarder is at rowOffset[n + 1] for all rows including the last one.
	 * rowOffset.length = rowSize.length + 1
	 */
	 ...
	 * 
	 * @param size
	 *            widths of columns and heights of rows in the format, {{col0, col1,
	 *            col2, ..., colN}, {row0, row1, row2, ..., rowM}} If this parameter
	 *            is invalid, the TableLayout will have exactly one row and one
	 *            column.
	 */
	 ...
	 * Adjusts the number and sizes of rows in this layout. After calling this
	 * method, the caller should request this layout manager to perform the layout.
	 * This can be done with the following code: <code>
	 *     layout.layoutContainer(container);
	 *     container.repaint();
	 * </code> or
	 * 
	 ...
	 * 
	 * @param i
	 *            zero-based index of column to set. If this parameter is not valid,
	 ...
	 * be done with the following code: <code>
	 *     layout.layoutContainer(container);
	 *     container.repaint();
	 * </code> or
	 * 
	...
	public void insertColumn(int i, double size) {
		...
		if ((i < 0) || (i > columnSpec.length))
			throw new IllegalArgumentException(
					"Parameter i is invalid.  i = " + i + ".  Valid range is [0, " + columnSpec.length + "].");
	...
	public void insertRow(int i, double size) {
		...
		if ((i < 0) || (i > rowSpec.length))
			throw new IllegalArgumentException(
					"Parameter i is invalid.  i = " + i + ".  Valid range is [0, " + rowSpec.length + "].");
	...
	public void deleteColumn(int i) {
		...
		if ((i < 0) || (i >= columnSpec.length))
			throw new IllegalArgumentException(
					"Parameter i is invalid.  i = " + i + ".  Valid range is [0, " + (columnSpec.length - 1) + "].");
	...
	public void deleteRow(int i) {
		...
		if ((i < 0) || (i >= rowSpec.length))
			throw new IllegalArgumentException(
					"Parameter i is invalid.  i = " + i + ".  Valid range is [0, " + (rowSpec.length - 1) + "].");
	...
	/**
	 * Draws a grid on the given container. This is useful for seeing where the rows
	 * and columns go. In the container's paint method, call this method.
	 * 
	 * @param container
	...
	/**
	 * Determines whether or not there are any hidden components. A hidden component
	 * is one that will not be shown with this layout's current configuration. Such
	 * a component is, at least partly, in an invalid row or column. For example, on
	 * a table with five rows, row -1 and row 5 are both invalid. Valid rows are 0
	 * through 4, inclusively.
	 * 
	...
	protected class Entry extends TableLayoutConstraints {
		 ...
		 * 
		 * @param object
		 *            object being compared to; must be a Component if it is equal to
		 *            this TableLayoutConstraints.
	...
	public static JComponent getDoubleRow(JComponent label, JComponent row2, Color color) {
		...
		result.add(label, "1,1");
		row2.setBorder(BorderFactory.createLineBorder(color));
		result.add(row2, "1,2");
		result.validate();
		return result;
	}
	...
	public static JComponent get4Split(JComponent left, JComponent middle1, JComponent middle2, JComponent right,
		...
		if (left != null)
			result.add(left, "1,1");
		if (middle1 != null)
			result.add(middle1, "3,1");
		if (middle2 != null)
			result.add(middle2, "5,1");
		if (right != null)
			result.add(right, "7,1");
		result.validate();
		return result;
	}
	...
	public static JComponent get3SplitVertical(JComponent top, JComponent middle, JComponent bottom, double sizeTop,
		...
		if (top != null)
			result.add(top, "1,1");
		if (middle != null)
			result.add(middle, "1,3");
		if (bottom != null)
			result.add(bottom, "1,5");
		result.validate();
		return result;
	}
	...
	public static JComponent get4SplitVertical(JComponent top, JComponent middle1, JComponent middle2,
		...
		JPanel result = new JPanel();
		result.setOpaque(false);
		double[][] size = { { border, TableLayoutConstants.FILL, border }, { border, sizeTop, spaceBetween, sizeMiddle1,
				spaceBetween, sizeMiddle2, spaceBetween, sizeBottom, border }, // row
		...
		if (top != null)
			result.add(top, "1,1");
		if (middle1 != null)
			result.add(middle1, "1,3");
		if (middle2 != null)
			result.add(middle2, "1,5");
		if (bottom != null)
			result.add(bottom, "1,7");
		result.validate();
		return result;
	}

	public static LayoutManager getLayout(int width, int height) {
		double[][] size = { { width }, { height }, // row
	...
	public static void setMultiSplitVerticalNonPrefHeight_Contents(Collection<?> jComponentList, double h,
			JPanel result) {
		double[] height = new double[jComponentList.size()];
		for (int i = 0; i < height.length; i++)
			height[i] = h;
	...
	public static LayoutManager getLayout(double width, double height, int columns, int rows) {
		...
		return new TableLayout(new double[][] { widths, heights });
	}
}
