		...
		if (new SettingsHelperDefaultIsTrue().isEnabled("Hierarchy commands")) {
			if (ReleaseInfo.getRunningReleaseStatus() != Release.KGML_EDITOR) {
				this.algorithms = new Algorithm[] {
						...
						/*
						 * some of the following algorithms will be removed from the final version of vanted
						 * since they seem to be very implemented for very special use
						 * 
						 * The latest vanted version will focus on GO/KEGG hierarchy including 
						 * cluster histograms
						 * 
						 * All disabled algorithms will be in a special category (See category list //TODO: to be implemented).
						 * If the user enables a global setting e.g. enable special/extra algorithms. they will reappear
						 * 
						 * So.. the algorithms will be listed in the global algorithm list but will not be added to the menu
						 * but will be available in the vanted classpath, to be called by other algorithms if needed
						 */
