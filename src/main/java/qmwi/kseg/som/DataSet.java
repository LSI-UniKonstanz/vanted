...
public class DataSet {
	 ...
	 * beim Training verwendet wurden, �bereinstimmen Z�hlt alle Eintr�ge die
	 * innerhalb der Spalte "group" den Wert "entry" haben
	 */
	...
	/**
	 * Zählt die verschiedenen Ausprägungen innerhalb einer Spalte Diese Gruppe kann
	 * z.B. unterschiedliche Länder enthalten Der Rückgabewert würde dann der Anzahl
	 * an unterschiedlichen Ländern innerhalb des Datenbestandes entsprechen. TODO:
	 ...
	 * 
	 * @param thedata
	 *           Vector mit den Eingabedaten (CSVdataEntry) oder null, dann werden
	 *           die zur Klasse gehörenden Daten verwendet
	 * @param learn
		...
		for (int wh = 0; wh < anzWdh; wh++) {
			...
			for (int j = 0; j < data.size(); j++) {
				...
				for (int ic = 0; ic < cols.length; ic++) { // ic zählt die
					 ...
					 * qmwi.kseg.som.diagram.Aus.a("eingaben[iCols[ic]])", ((CSVdataEntry)
					 * thedata.elementAt(j)).eingaben[iCols[ic]]); }
					 */
					/***********************************************************
					 * Anpassung für som.diagramme ***********************************
					 * *******************************************
					...
					} else if (binColumn) { // Binäre Spalte (ja/nein)
				...
				if (learn) {
					if (fehler) {
						...
						if (decreaseN > 0)
							som.analyzeNewInput(wh, anzWdh, nachbarF, Tools.myMax(maxNachbar - (wh / decreaseN), 0.0),
									data);
						else
