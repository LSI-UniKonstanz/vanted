...
/**
 * Deterministically generates test graphs. All graphs can be generated with varying node numbers.
 * <p>
 * Graphs that are called "random" are generated using a fixed seed, so repeated executions of these
 * generation methods will return the same graphs.
 */
public class GraphGeneration {
	...
	/**
	 * Generates a complete graph: all nodes are connected to all other node. The graph will have
	 * numberOfNodes nodes and (numberOfNodes(numberOfNodes-1))/2 edges.
	 *
	 * @param numberOfNodes
	 */
	...
	/**
	 * Generates a star graph: all nodes are connected to one central node. The graph will have
	 * numberOfNodes nodes and numberOfNodes edges.
	 *
	 * @param numberOfNodes
	 */
	...
	/**
	 * Generates a wheel graph: all nodes are connected to one central node and to the nodes next to
	 * each node on the outer circle. The graph will have numberOfNodes nodes and 2 * numberOfNodes
	 * edges.
	 *
	 * @param numberOfNodes
	 */
	...
	/**
	 * Generates a Barabási–Albert graph: A graph that has similarities to some human made and
	 * natural networks, for example the internet or social networks (wikipedia).
	 *
	 * @param numberOfNodes
	 */
	 ...
	 * @param m0
	 *           Size of the initial connected component
	 * @param m
	 *           Desired degree of each node outside the initial component
	 * @param a
	 *           Parameter to control the degree distribution
	 */
	public Graph generateBarabasiAlbertNetwork(int N, int m0, int m, double a) {
		...
		for (int i = m0; i < N; i += 1) {
			...
			while (connectedNodes.size() < m) {
				...
				double Pj = Math.pow((nj.getDegree()) / (2.0 * graph.getNumberOfEdges()), a);
	 ...
	 *           How many levels of nested triangles shawl be generated. A value of 1
	 *           generates one simple triangle.
	 */
