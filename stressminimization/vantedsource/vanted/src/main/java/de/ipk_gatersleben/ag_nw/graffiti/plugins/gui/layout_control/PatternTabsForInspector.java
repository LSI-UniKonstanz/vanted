...
public class PatternTabsForInspector extends DBE_EditorPluginAdapter {
	...
	public PatternTabsForInspector() {
		...
		//
		// GravistoService.getInstance().getMainFrame().addSelectionListener(new
		// TabSubstrate());
	...
	/*
	 * private InspectorTab getSubtab(String title, Collection<InspectorTab>
	 * subtabs) { if (subtabs.size() == 1) return subtabs.iterator().next(); else
