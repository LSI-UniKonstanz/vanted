...
public class ChartOptions {
	...
	public ChartOptions(GraphElement ge, Graph graph, BioStatisticalCategoryDataset dataset, String chartTitle,
			PlotOrientation orientation, boolean showLegend, String domainAxis, String rangeAxis, boolean showRangeAxis,
			boolean showCategoryAxis, float outlineBorderWidth, int axisRotation, double lowerBound, double upperBound,
			boolean showGridRange, boolean showGridCategory, double gridWidth, double axisWidth, Color gridColor,
			Color axisColor, int axisFontSize, boolean showStdDevAsT, double stdDevTopWidth,
			boolean showStdDevAsFillRange, float shapeSize, float stdDevLineWidth, boolean showOnlyHalfErrorBar,
			boolean showShapes, boolean showLines, boolean usePieScaling, boolean connectPriorItems, int plotAxisSteps,
			boolean useLogYscale, int cpIdxA, int cpIdxC, Color cpColA, Color cpColBackground, Color cpColC,
			boolean useCustomRangeSteps, double customRangeSteps, boolean removeEmptyConditions) {
		this.ge = ge;
		this.graph = graph;
	...
	public void readAttributes(GraphElement ge) {
		...
		axisRotation = ((Double) AttributeHelper.getAttributeValue(graph, "", "node_plotAxisRotation", mNull, mNull))
				.intValue();

		boolean plotHor = ((Boolean) AttributeHelper.getAttributeValue(graph, "", "node_plotOrientationHor", mTrue,
				mTrue)).booleanValue();
		if (plotHor)
			orientation = PlotOrientation.VERTICAL;
		else
			orientation = PlotOrientation.HORIZONTAL;
		domainAxis = (String) AttributeHelper.getAttributeValue(ge, "charting", "domainAxis", new String("[unit]"),
				new String("[unit]"));
		...
		showCategoryAxis = ((Boolean) AttributeHelper.getAttributeValue(graph, "", "node_showCategoryAxis", mFalse,
				mFalse)).booleanValue();
		showRangeAxis = ((Boolean) AttributeHelper.getAttributeValue(graph, "", "node_showRangeAxis", mFalse, mFalse))
				.booleanValue();
		...
		cpIdxA = ((Integer) AttributeHelper.getAttributeValue(graph, "", "node_categoryBackgroundColorIndexA", mim1,
				mim1)).intValue();
		;
		cpIdxC = ((Integer) AttributeHelper.getAttributeValue(graph, "", "node_categoryBackgroundColorIndexC", mim1,
				mim1)).intValue();
		int c1 = 255;
		int c2 = 225;
		...
		fillTimeGaps = ((Boolean) AttributeHelper.getAttributeValue(graph, "", "node_lineChartFillTimeGaps", mTrue,
				mTrue)).booleanValue();
