...
public class KeggExpressionReader {
	 ...
	 * in the organism. Second column Y-axis coordinate information of the ORF on
	 * the microarray. Third column X-axis coordinate information of the gene. The
	 * second and third columns are used for specifying the location of the ORF in
	 * the schematic of the DNA microarray in the ArrayViewer application. Fourth
	 * column Intensity of the signal of the control channel. Fifth column Intensity
	 * of the background of the control channel. Sixth column Intensity of the
	 * signal of the target channel. Seventh column Intensity of the background of
	 * the target channel. When the subtraction of the background from the signal
		...
		if (askUserForRelevantColumns) {
			...
			for (TextFileColumnInformation ci : result) {
				JCheckBox cb = new JCheckBox(
						ci.getName() + " - Sign./Det. Col. " + ci.getSignalColumn() + "/" + ci.getDetectionColumn(),
						false);
				cb.setOpaque(false);
			...
			Object[] res = MyInputHelper.getInput(
					"<html>" + "Please select the relevant data columns. If no selection is made,<br>"
							+ "all columns will be processed. Depending on the amount of data,<br>"
							+ "this process might take a few moments.",
					"Data-Column Selection", new Object[] { "", checkBoxFp });
			if (res == null)
				result.clear();
			else {
