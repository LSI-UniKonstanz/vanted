...
public class PasswordDialog {
	...
	public static boolean DBE_global_login_valid() {
		return false;
		/*
		 * try { if (pdc==null || pd==null) { pdc = Class.forName(
		 * "de.ipk_gatersleben.ag_nw.expdb.application.picture_gui.PasswordDialog"); pd
		 * = pdc.newInstance(); } String passAndLogin = pd.toString(); String[] pal =
		 * passAndLogin.split("§", 2); return (!passAndLogin.equals("null§null")) &&
		 * pal[0].length()>0 && pal[1].length()>0; } catch(ClassNotFoundException cnfe)
		 * { ErrorMsg.addErrorMessage(cnfe); } catch (SecurityException e) {
		 * ErrorMsg.addErrorMessage(e); } catch (InstantiationException e) {
		 * ErrorMsg.addErrorMessage(e); } catch (IllegalAccessException e) {
