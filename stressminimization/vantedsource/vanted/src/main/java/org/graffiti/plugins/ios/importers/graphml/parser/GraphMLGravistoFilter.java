...
//
//==============================================================================
// $Id: GraphMLGravistoFilter.java,v 1.7 2011/01/16 16:39:59 klukas Exp $
 ...
 * nested graphs, hyperedges and locators as specified in the
 * <a href="http://graphml.graphdrawing.org/specification">graphML
 * specification</a>.
 *
 * @author ruediger
 */
class GraphMLGravistoFilter extends XMLFilterImpl {
	...
	private static final Logger logger = Logger.getLogger(GraphMLGravistoFilter.class.getName());
	...
	/**
	 * A stack containing the qualified name of element from foreign namespaces used
	 * for filtering.
	 */
	...
	/**
	 * Filter a character data event. Checks if the element that is currently being
	 * processed is ignorable and if not forwards the character data event to the
	 * parent parser.
	 *
	 * @param ch
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (qName.equals("graph")) {
			--this.nesting;
			assert this.nesting >= -1 : "nesting level less than -1 (" + this.nesting + ").";
			logger.fine("end of " + ((this.nesting == 0) ? "" : "embedded ") + "graph detected");

			return;
		} else if (qName.equals("hyperedge")) {
			logger.fine("end of " + (isIgnorable() ? "ignorable " : "") + "element hyperedge detected");
			this.hyperedge = false;

			return;
		} else if (qName.equals("locator")) {
			logger.fine("end of " + (isIgnorable() ? "ignorable " : "") + "element locator detected");
			this.loc = false;

			return;
		} else if (qName.equals("graphml")) {
			logger.fine("end of " + (isIgnorable() ? "ignorable " : "") + "element graphml detected");

			return;
		} else if (qName.equals("desc")) {
			logger.fine("end of " + (isIgnorable() ? "ignorable " : "") + "element desc detected");
			this.desc = false;
	...
	/**
	 * Filter a start element event. Filters elements that are not supported as well
	 * as foreign namespaces and nested graphs. The rest is passed to the parent
	 * filter.
	 *
	 * @param uri
	 *            the element's Namespace URI, or the empty string.
	...
	public void startElement(String uri, String localName, String qName, Attributes atts) throws SAXException {
		logger.fine("start " + (isIgnorable() ? "ignorable " : "") + "element " + qName + "\n\tnamespace: " + uri
				+ "\n\tqName: " + qName + "\n\tstack: " + this.foreignNSElts.toString());
		...
		// processed
		if (uri.equals(NAMESPACE) || uri.equals(OLD_NAMESPACE)) {
			...
			if (qName.equals("graph")) {
				...
				logger.fine(((this.nesting == 0) ? "" : "embedded ") + "graph detected");
			}
	 ...
	 *
	 * @return <code>true</code> if the corresponding data can be ignored, otherwise
	 *         <code>false</code>.
	 */
	...
	/**
	 * This stack is designed in order to keep track of the nesting of elements from
	 * foreign namespaces.
	 */
	private class ElementStack {
		...
		/**
		 * Returns a <code>String</code> representation of the current content of the
		 * stack - only for debugging purposes!
		 *
		 * @return a <code>String</code> representation of the current content of the
		 *         stack.
		 */
		@Override
		public String toString() {
			if (this.stack.isEmpty()) {
				...
				for (Iterator itr = this.stack.iterator(); itr.hasNext();) {
					s += ((String) itr.next() + " ");
				}
		...
		/**
		 * Removes and returns the top element of the stack, returns <code>null</code>
		 * if the stack is empty
		 *
		 * @return the top element of the stack, <code>null</code> if the stack is
		 *         empty.
		 */
		String pop() {
			if (this.stack.isEmpty()) {
