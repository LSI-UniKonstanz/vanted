...
public class ViewTypeChooser extends JDialog
	...
	/**
	 * Constructs a new view type chooser, which is needed if there are several view
	 * types available.
	 * 
	 * @param parent
	...
	public ViewTypeChooser(Frame parent, String title, String[] views) {
		...
		getContentPane().add(TableLayout.getSplitVertical(frameCheckbox, buttonsPanel, TableLayoutConstants.PREFERRED,
				TableLayoutConstants.PREFERRED), BorderLayout.SOUTH);
	...
	class NameListCellRenderer extends JLabel implements ListCellRenderer {
		...
		/**
		 * Return a component that has been configured to display the view name without
		 * class path. That component's <code>paint</code> method is then called to
		 * "render" the cell.
		 * 
		 * @param list
