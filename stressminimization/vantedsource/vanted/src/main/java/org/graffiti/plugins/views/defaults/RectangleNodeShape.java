...
//
// ==============================================================================
// $Id: RectangleNodeShape.java,v 1.14 2010/12/22 13:06:19 klukas Exp $
...
public class RectangleNodeShape extends RectangularNodeShape {
	...
	public static Point2D getIntersectionOfRoundRectangleAndLine(Line2D line, Rectangle2D rect, double rounding) {
		...
		if (left.intersectsLine(line) && (left.getX1() != line.getX1())) {
			return checkEllipsesIntersections(GraphicHelper.getIntersection(left, line), line, Line.LEFT, rect,
					rounding);
		} else if (bottom.intersectsLine(line) && (bottom.getY1() != line.getY1())) {
			return checkEllipsesIntersections(GraphicHelper.getIntersection(bottom, line), line, Line.BOTTOM, rect,
					rounding);
		} else if (right.intersectsLine(line) && (right.getX1() != line.getX1())) {
			return checkEllipsesIntersections(GraphicHelper.getIntersection(right, line), line, Line.RIGHT, rect,
					rounding);
	...
	private static Point2D checkEllipsesIntersections(Point2D intersection, Line2D line, Line position,
		...
		if (iy >= rect.getMinY() + rounding && iy <= rect.getMaxY() - rounding)
			return intersection;
		// System.out.println("INTERSECT "+rect.toString()+" with "+line.toString()+" =>
	...
	public void buildShape(NodeGraphicAttribute graphics) {
		...
		} else if (Math.abs(rounding) < 0.0001 && (rect2D instanceof RoundRectangle2D)) {
			rect2D = new Rectangle2D.Double(x, y, w, h);
		} else if (rect2D instanceof RoundRectangle2D)
