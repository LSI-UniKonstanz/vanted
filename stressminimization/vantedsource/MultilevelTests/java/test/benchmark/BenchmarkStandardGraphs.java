...
/**
 * Abstract class for standard benchmarks. Number of nodes can be specified by
 * sub class;
 */
public abstract class BenchmarkStandardGraphs extends AlgorithmBenchmarkSuite {
	...
	protected List<Quadtuple<Graph, String, Integer, Integer>> createGraphs() {
		...
		if (benchmarkStarGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(starGraph, "star graph (n = " + n + ")",
		...
		if (benchmarkWheelGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(wheelGraph, "wheel graph (n = " + n + ")",
					WARMUP_ROUNDS, ROUNDS));
		...
		if (benchmarkCompleteGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(completeGraph, "complete graph (n = " + n + ")",
					WARMUP_ROUNDS, ROUNDS));
		...
		if (benchmarkLineGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(lineGraph, "line graph (n = " + n + ")",
					WARMUP_ROUNDS, ROUNDS));
		...
		if (benchmarkBarabasisAlbertGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(barabasiAlbertGraph,
					"barabasis albert graph (n = " + n + ")", WARMUP_ROUNDS, ROUNDS));
		...
		if (benchmarkWattsStrogatzGraph()) {
			...
			cases.add(new Quadtuple<Graph, String, Integer, Integer>(wattsStrogatzGraph,
					"watts strogatz graph (n = " + n + ")", WARMUP_ROUNDS, ROUNDS));
