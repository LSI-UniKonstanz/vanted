	...
	public void execute() {
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			for (int q = 0; q < pointSize; q++) {
				...
				pointsArray[q] = (new Point2DTimePoint(percentageTime * animationDuration,
						nextPos.getX() * (Math.random() * 0 + 1), nextPos.getY() * (Math.random() * 0 + 1)));
				...
				outlineColour = new Color(outlineColour.getRed() / 2, outlineColour.getGreen() / 2,
						outlineColour.getBlue() / 2);
				outlineArray[q] = new ColorTimePoint(percentageTime * animationDuration, outlineColour);
				int shade = (int) (123 + 122 * Math.sin(sinX));
				labelColourArray[q] = new ColorTimePoint(percentageTime * animationDuration,
				...
				sizeArray[q] = new Point2DTimePoint(percentageTime * animationDuration,
			...
			Position2DAnimation posAnim = new Position2DAnimation(node, Arrays.asList(pointsArray), animationDuration,
					0, 1, new StandardLooper(), new CubicInterpolator());
			FillColorAnimation fillAnim = new FillColorAnimation(node, Arrays.asList(fillArray), animationDuration, 0,
					1, new ForwardLooper());
			SizeAnimation sizeAnim = new SizeAnimation(node, Arrays.asList(sizeArray), animationDuration, 0, 0,
					new ForwardLooper(), new CubicInterpolator());
			OutlineColorAnimation outlineColourAnim = new OutlineColorAnimation(node, Arrays.asList(outlineArray),
					animationDuration, 0, 1, new ForwardLooper());
			LabelColorAnimation labelColourAnim = new LabelColorAnimation(node, Arrays.asList(labelColourArray),
					animationDuration, 0, 1, new ForwardLooper(), new LinearInterpolator(), ColorMode.HSB, 1);
			FrameThicknessAnimation thicknessAnim = new FrameThicknessAnimation(node, Arrays.asList(thicknessArray),
					animationDuration, 0, 1, new ForwardLooper());
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			// OutlineColorAnimation outlineColourAnim = new
			// OutlineColorAnimation(edge,Arrays.asList(colours), animationDuration, 0, 1,
			// new ForwardLooper());
			// FrameThicknessAnimation thicknessAnim = new
			// FrameThicknessAnimation(edge,Arrays.asList(thickness), animationDuration, 0,
			// 1, new ForwardLooper());
			// animator.addAnimation(thicknessAnim);
			// animator.addAnimation(outlineColourAnim);
		}
