...
public class SBML_Species_Reader {
	...
	public void addSpecies(Graph g, ListOf<Species> speciesList, PositionGridGenerator pgg,
		...
		if (!speciesList.isEmpty()) {
			...
			while (itSpecies.hasNext()) {
				...
				AttributeHelper.setAttribute(speciesNode, SBML_Constants.SBML, SBML_Constants.SBML_ROLE,
						SBML_Constants.ROLE_SPECIES);
				...
				if (Double.isNaN(initialAmount)) {
					SBML_Logger.addErrorMessage("Attribute initialAmount of species " + id
				...
				if (Double.isNaN(initialConcentration) && !(speciesJSBML.isSetInitialAmount())) {
					SBML_Logger.addErrorMessage("Attribute initialConcentration of species " + id
				...
				if (speciesJSBML.isSetInitialConcentration()) {
					speciesHelper.setInitialConcentration(speciesNode, initialConcentration);
				}
				...
				if (speciesJSBML.isSetHasOnlySubstanceUnits()) {
					speciesHelper.setHasOnlySubstanceUnits(speciesNode, hasOnlySubstanceUnits);
				}
				// if (speciesJSBML.isSetBoundaryCondition()) {
				speciesHelper.setBoundaryConsition(speciesNode, boundaryCondition);
				// }
				if (speciesJSBML.isSetConstant()) {
					speciesHelper.setConstant(speciesNode, constant);
				}
				if (speciesJSBML.isSetConversionFactor()) {
					speciesHelper.setConversionFactor(speciesNode, conversionFactor);
				}
				...
				if (speciesJSBML.isSetAnnotation()) {
					if (speciesJSBML.getAnnotation().isSetRDFannotation()) {
						speciesHelper.setAnnotation(speciesNode, speciesJSBML.getAnnotation());
					}
					if (speciesJSBML.getAnnotation().isSetNonRDFannotation()) {
						speciesHelper.setNonRDFAnnotation(speciesNode,
								speciesJSBML.getAnnotation().getNonRDFannotation());
					}
				}
	...
	// speciesJSBML.getModel().getExtension(SBMLHelper.SBML_LAYOUT_EXTENSION_NAMESPACE);
	// if (layoutModel != null) {
	...
	// Iterator<SpeciesGlyph> speciesGlyphListIt =
	// layout.getListOfSpeciesGlyphs().iterator();
		...
		if (layoutModel != null) {
			...
			while (speciesGlyphListIt.hasNext()) {
				...
				if (speciesGlyph.getSpecies().equals(speciesID)) {
					...
					AttributeHelper.setAttribute(_speciesNode, SBML_Constants.SBML, SBML_Constants.SPECIES_GLYPH_ID,
							speciesGlyph.getId());
					BoundingBox boundingBox = speciesGlyph.getBoundingBox();
					if (boundingBox != null) {
						Dimensions dimensions = boundingBox.getDimensions();
						...
						if (position != null) {
							...
							AttributeHelper.setAttribute(_speciesNode, SBML_Constants.SBML,
									SBML_Constants.SBML_LAYOUT_ID, "dummyLayoutID");
						} else if (AttributeHelper.hasAttribute(_speciesNode, SBML_Constants.SBML,
								SBML_Constants.SBML_LAYOUT_ID))
							AttributeHelper.deleteAttribute(_speciesNode, SBML_Constants.SBML,
									SBML_Constants.SBML_LAYOUT_ID);
					}
					i++;
				}
