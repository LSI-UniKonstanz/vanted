	...
	public void addFunktionDefinition(ListOf<FunctionDefinition> functionDefinitionList, Graph g) {
		...
		while (itFunctionDefinition.hasNext()) {
			try {
				...
				String internHeadline = new StringBuffer(SBML_Constants.SBML_FUNCTION_DEFINITION).append(functionCount)
						.toString();
				String presentedHeadline = new StringBuffer("SBML Function Definition ").append(functionCount)
						.toString();
				...
				if (functionDefinition.isSetNotes()) {
					functionDefinitionHelper.setNotes(functionDefinition.getNotesString(),
							functionDefinition.getNotes());
				}
					...
					if (functionDefinition.getAnnotation().isSetNonRDFannotation()) {
						if (functionDefinition.getAnnotation().isSetNonRDFannotation()) {
							functionDefinitionHelper
									.setNonRDFAnnotation(functionDefinition.getAnnotation().getNonRDFannotation());
						}
					}
				}
