...
public class MyTools {
	...
	public static void initNodeCache(ArrayList<NodeCacheEntry> nodeArray, HashMap<Node, NodeCacheEntry> nodeSearch,
			Graph graph, Selection selection, ArrayList<Graph> patternGraphs) {
		...
		for (int i = 0; i < nodeList.size(); i++) {
			...
			org.Vector2d storeP = new org.Vector2d(cn.getX(), cn.getY());
			...
			if (newInfo.patternType != null) {
				...
				try {
					...
					if (patternGraphs != null && patternGraphs.size() > graphNum
							&& patternGraphs.get(graphNum) != null) {
						List<?> patternNodeCollection = (patternGraphs.get(graphNum)).getNodes();

						newInfo.patternNode = (Node) patternNodeCollection.get(indexInPattern);
					}
	...
	public static void initNodeCache3d(ArrayList<NodeCacheEntry3d> nodeArray,
			HashMap<Node, NodeCacheEntry3d> nodeSearch, Graph graph, Selection selection) {
		nodeArray.ensureCapacity(graph.getNodes().size());
		boolean foundPattern = false;
		int patternCount = 0;
		...
		for (int i = 0; i < graph.getNodes().size(); i++) {
			...
			ArrayList<?> patternGraphs = GravistoService.getInstance().getPatternGraphs();
			...
			if (newInfo.patternType != null) {
				...
				try {
					...
					if (patternGraphs != null && patternGraphs.size() > graphNum
							&& patternGraphs.get(graphNum) != null) {
						List<?> patternNodeCollection = ((Graph) patternGraphs.get(graphNum)).getNodes();
	...
	private static String getLabelPath() {
		...
		if (GraphicAttributeConstants.LABEL_ATTRIBUTE_PATH.equals("")) {
			...
			LABEL_PATH = GraphicAttributeConstants.LABEL_ATTRIBUTE_PATH + Attribute.SEPARATOR
