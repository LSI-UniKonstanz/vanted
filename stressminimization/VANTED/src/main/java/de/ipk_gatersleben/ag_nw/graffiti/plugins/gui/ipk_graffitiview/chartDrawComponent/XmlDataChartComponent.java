...
public class XmlDataChartComponent extends JComponent {
	...
	public XmlDataChartComponent(String preferredChartType, Graph graph, org.graffiti.graph.GraphElement ge) {
		...
		for (int index = 0; index < mappedDataListSize; index++) {
			...
			// axisRotation, lowerBound, upperBound, showGridRange, showGridCategory,
			// gridWidth, axisWidth, gridColor,
			// axisColor, axisFontSize, showStdDev, errorBarLen, showStdDevRangeLine,
			// shapeSize, stdDevLineWidth,
			// showOnlyHalfErrorBar, showShapes, showLines, usePieScaling,
			// connectPriorItems, plotAxisSteps,
			// useLogYscale, categoryPaintBackgroundColorIdxA,
			// categoryPaintBackgroundColorIdxC,
	...
	public static String getChartTitle(org.graffiti.graph.GraphElement ge, int idx) {
		...
		if (chartTitle.length() == 0)
			chartTitle = null;
		else if (chartTitle.equalsIgnoreCase("[substancename]")) {
	...
	protected void setCategoryAxisOptions(CategoryAxis axis, ChartOptions co) {
		if (co.orientation == PlotOrientation.VERTICAL) {
			...
			axis.setCategoryLabelPositions(
					CategoryLabelPositions.createUpRotationLabelPositions(Math.PI / 180 * co.axisRotation));
			/*
