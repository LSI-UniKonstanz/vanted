 ...
 * Contributor(s): -;
 * $Id: TimePeriodValues.java,v 1.3 2010/12/14 07:02:06 morla Exp $
 * Changes
 * -------
...
public class TimePeriodValues extends Series implements Serializable {
	...
	public boolean equals(final Object object) {
		...
		for (int i = 0; i < count; i++) {
			if (!getDataItem(i).equals(tpvs.getDataItem(i))) {
				return false;
			}
		}
		return true;
	...
	public TimePeriodValues createCopy(final int start, final int end) throws CloneNotSupportedException {
		...
		if (this.data.size() > 0) {
			for (int index = start; index <= end; index++) {
				final TimePeriodValue item = (TimePeriodValue) this.data.get(index);
