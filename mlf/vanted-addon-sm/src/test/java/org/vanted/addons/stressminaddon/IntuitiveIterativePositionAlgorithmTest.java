...
public class IntuitiveIterativePositionAlgorithmTest {
    ...
    public void nextIterationOnePosition(){
        ...
        NodeValueMatrix weights = GRAPH_OnePosition_DISTANCES.clone().apply(x-> 1/(x*x));

        List<Vector2d> position = alg.nextIteration(GRAPH_OnePosition_NODES, GRAPH_OnePosition_DISTANCES,weights);
        Vector2d[] expected = new Vector2d[]{
                new Vector2d(0,0),
                new Vector2d(0,0),
                new Vector2d(0,0),
        ...
        for (int pos = 0; pos < GRAPH_OnePosition_POSITIONS.size(); pos++){
            Vector2d expectedPos = expected[pos];
            Vector2d myPos = position.get(pos);
