...
public class NodeValueMatrix implements Cloneable {
    ...
    public NodeValueMatrix(final int dimension) {
        ...
        // initialize the values (internally only a (d-1)*(d-1) half matrix is saved
     ...
     *
     * @throws IndexOutOfBoundsException if {@code row} or {@code col} are out of bounds.
     * @throws IllegalArgumentException if {@code row} and {@code col} are equal.
     */
    public void set(final int row, final int col, final double value) {
        ...
        if (row == col)
            throw new IllegalArgumentException("The diagonal may not be set (row==col)");
        // set the value
    ...
    @Override
    protected NodeValueMatrix clone() throws CloneNotSupportedException {
