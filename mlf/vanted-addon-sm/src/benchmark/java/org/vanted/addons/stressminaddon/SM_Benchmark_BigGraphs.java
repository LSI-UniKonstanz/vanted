...
public class SM_Benchmark_BigGraphs {
    ...
    public void benchmark() throws InterruptedException, InvocationTargetException, NoSuchMethodException, NoSuchFieldException, IllegalAccessException {
        ...
        System.out.println(Paths.get("benchmark/java/test_graphs/BigGraphs").toAbsolutePath());
        Graph graph = null;
        ...
        for(int graphIndex = 0; graphIndex < graphen.length;graphIndex++) {
            try {
                graph = MainFrame.getInstance().getGraph(Paths.get("benchmark/java/test_graphs/BigGraphs/" + graphen[graphIndex]).toFile());
        ...
        save_csv(csv_singleStepsBigGraphs, "csv_singleStepsBigGraphs");
        save_csv(csv_averageBigGraphs, "csv_averageBigGraphs");
