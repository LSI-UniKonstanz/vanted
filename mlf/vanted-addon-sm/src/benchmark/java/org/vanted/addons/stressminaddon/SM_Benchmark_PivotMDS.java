...
/**
 * Run benchmarks for PivotMDS in VANTED.
 * @author Rene
 */

public class SM_Benchmark_PivotMDS {

    /**
     *Sets the parameter for the test
     */
    private final static int REPEAT_TIMES = 5;
    private final static String getPath = "benchmark/java/test_graphs/PivotMDS_Graphs/";
    private final static String outPath = "benchmark/java/test_graphs/PivotMDS_Graphs/output_PivotMDS/";
    ...
    public void benchmark() throws InterruptedException{
        ...
        String[] graphen = {"crack.gml","sierpinski_7.gml"};
        ...
        for(int graphIndex = 0; graphIndex < graphen.length;graphIndex++) {
            try {
                ...
                for (int pivotFactor = 1; pivotFactor <= 20; pivotFactor++) {
                    ...
                    for (int repeat = 0; repeat < REPEAT_TIMES; repeat++) {
                        ...
                        ((PivotMDS) state.initialPlacer).percentPivots = 5 * pivotFactor;
                        ...
                        csv_singleSteps.append(runningTimeMs).append(",").append(iterations).append(",").append(stress)
                                .append(",").append(lineCrossings).append(",\"").append(pivotFactor).append(graphen[graphIndex]).append("\"\n");

                        //  save the calculated graph
                        try {
                            MainFrame.getInstance().saveGraphAs(graph, outPath +
                                    repeat + "_" + pivotFactor + "_" + graphen[graphIndex], graph.getFileTypeDescription());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    csv_average.append(averageRunningTimeMs / REPEAT_TIMES).append(",").append(averageIterations / REPEAT_TIMES)
                            ...
                            .append(",\"").append(pivotFactor).append(graphen[graphIndex]).append("\"\n");
        ...
        save_csv(csv_singleSteps.toString(), "csv_singleSteps_PivotMDS_Graphs");
        save_csv(csv_average.toString(), "csv_average_PivotMDS_Graphs");
